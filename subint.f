      SUBROUTINE SUBINT(IQ,NCPERT,ICPERT,I10)
*
*
...
*       Sort the time-step list sequentially in KBLIST.
              IF (NNTB.GT.1) THEN
...
*       See whether the smallest KS time falls due before next block step.
   ...
   10         CALL KSINT(I1)
*
*       Check for multiple calls of #I1 (avoids CALL INSERT).
...
*       Reset decision indicator and make new list on change of KS sequence.
              ...
              END IF
*
*       Check for change of perturbed state (ignore collisions: ISEND < 0).
              IF (NP0.EQ.0.AND.LIST(1,I1).GT.0) THEN
                  ICM = N + KVEC(I1)
*       Increase membership and add #ICM to perturbed binary list.
                  NCPERT = NCPERT + 1
                  ICPERT(NCPERT) = ICM
*
              ELSE IF (NP0.GT.0.AND.LIST(1,I1).EQ.0) THEN
*       Remove body #ICM from list of active KS and reduce membership.
                  ICM = N + KVEC(I1)
                  DO 20 L = 1,NCPERT
                      IF (ICPERT(L).EQ.ICM) THEN
                          DO 15 LL = L,NCPERT-1
                              ICPERT(LL) = ICPERT(LL+1)
   15                     CONTINUE
                          NCPERT = NCPERT - 1
                      END IF
   20             CONTINUE
              END IF
*
*       Continue integration cycle.
              GO TO 5
          END IF
*
*       Copy original block time at end of KS cycle.
          TIME = TBLOCK
          NBPREV = NPAIRS
...
*       Update unperturbed size of subsystem and copy c.m. step.
              ...
              END IF
*
*       Check for termination (set TPREV < TIME and set IQ < 0 & ISEND > 0).
