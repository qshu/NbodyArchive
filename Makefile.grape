...
# make mpich_grape                 (Beowulf PC clusters with GRAPE's, standard mpif77)
...
mpich_ensemble:
	...
	$(MAKE) $(RESULT) "FC = mpif77" "FFLAGS = -O4 -D ENSEMBLE" \
		"SOURCE = energy_mpi.f fpoly1_mpi.f fpoly2_mpi.f $(SOURCE)"
#----------------------------------------------------------------------------
# Workstation parallel CPU f77 Linux (Beowulf generic mpif77 with GRAPE's)
#----------------------------------------------------------------------------
mpich_grape:
	cp mpif.mpich.h mpif.h
	$(MAKE) $(RESULT) "FC = mpif77" "FFLAGS = -O4 -D PUREMPI -D PARALLEL -D GRAPE" \
		"LFLAGS = -L/usr/src/g6apkg/lib/ -lg6a $(LFLAGS)" \
		"SOURCE = util_grape.f energy_mpi.f fpoly1_mpi.f fpoly2_mpi.f $(SOURCE)"
#----------------------------------------------------------------------------
...
INC = params.h common6.h common_grape.h
...
fpoly1.f fpoly2.f freeze.f gntage.f hcorr.f hiarch.f hidat.f himax.f \
...
clean: 
	\rm *.o adjust.f  cloud0.f  escape.f  regint.f   hotsys.f  intide.f  modify.f  output.f bindat.f  cputim.f  file_init.f  input.f   ksint.f   mydump.f  scale.f binpop.f  data.f    hipop.f      intgrt.f  lagr.f    nbody6.f  start.f
