     ...
     &                             F8.1,2I6,I4,1P,4E10.2)
      ...
      ELSE
          ...
          IF (MC2.LE.1.0D-10.OR.MC2.GT.M0) THEN
              ...
              IF(KW.EQ.9) MC2 = MIN(0.3,0.95*M0)
              CM(2,IC) = MC2/ZMBAR
          END IF
          ZDUM = 2.0D0
...
*
  ...
  790         FORMAT (' BEGIN    TROT P TK R*   ',2F8.2,2F10.2,2F7.1)
          END IF
      END IF
*
...
*       Choose the step from smallest time-scale (! time0 < time possible).
      taux = max(abs(1.0/udot1),abs(1.0/udot2),abs(1.0/udot3))
...
*
      ...
      WRITE (6,800) LIST(1,I1), NAME(I1),es0, omeq, meanmotion, spin1,
     &              spin2
  800 FORMAT (' SPIRAL    NP NM es0 omeq <n> spin ',
...
*       Ensure that no overshooting takes place.
      IF (spin10.LT.1.0.and.spin1.gt.1.0) THEN
          IF (MOD(IWARN,100).EQ.0) THEN
              WRITE (66,805) TTOT, NAME(J1), KSTAR(J1), nstep, ecc,
     &                       RP0*SU/(1.0-es0), spin1-1.0
  805         FORMAT (' SPIN WARNING   T NM K* # E A S-1 ',
     &                                 F8.1,I7,I4,I6,F7.3,1P,2E10.1)
      ...
      IF (spin20.LT.1.0.and.spin2.gt.1.0) THEN
          IF (MOD(IWARN,100).EQ.0) THEN
              WRITE (66,805) TTOT, NAME(J2), KSTAR(J2), nstep, ecc,
...
*       Ensure the dominant star has synchronous rotation.
   ...
   26     FORMAT (' ENFORCE    NM(J1) Q1 QC DOM/OM ',I8,2F8.2,F7.3)
...
*
          ECC2 = (1.0-R(IPAIR)/SEMI)**2 + TDOT2(IPAIR)**2/(BODY(I)*SEMI)
          WRITE (6,22)  ECC, SQRT(ECC2), TIME-TIME0, SEMI,
