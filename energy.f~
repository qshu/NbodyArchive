...
*       Store potential in shared vector first (R.Sp.)
      PHIDBL(I) = POTI-SMBH/DSQRT(X(1,I)**2+X(2,I)**2+X(3,I)**2)  
      POT = POT + BODY(I)*POTJ 
   20 CONTINUE
*
      EBLCKHL=0.D0
...
*       Sum the kinetic energy (include c.m. bodies but not components).
     ...
     &                                          XDOT(3,I)**2)
*       Accumulation of Potential Energy provoked by
*              Massive Central Objecty
*
      EBLCKHL=EBLCKHL-SMBH*BODY(I)/DSQRT(X(1,I)**2+
     &        X(2,I)**2+X(3,I)**2)  
*
   40 CONTINUE
      ZKIN = 0.5D0*ZKIN
