...
*
      ...
      REAL*8  B2(3,4),U2(4),Q2(4),FPI(3),FPJ(3),FPID(3),FPJD(3)
      REAL*8  XI1(3),XDI1(3),XI2(3),XDI2(3),ETAU1,RIP
*
*
...
*
   ...
   40 CONTINUE
*
*       Add Pertubation of Central Star
*
      DO K=1,3
       FPI(K) = 0.0
       FPJ(K) = 0.0
...
*          /Ch.Om. July 2005
      ETAU1=ETAU
      RIP=SQRT(XI1(1)**2.0+XI1(2)**2.0+XI1(3)**2.0)
      IF(RIP.LE.0.3) THEN
      ETAU1=ETAU*0.01
      END IF
*
      CALL MASSCENTROBJECT(I1,XI1,XDI1,FPI,FPID)
      CALL DRAGFORCE(I1,XI1,XDI1,FPI,FPID,FPI,2)
*
      CALL MASSCENTROBJECT(I2,XI2,XDI2,FPJ,FPJD)
      CALL DRAGFORCE(I2,XI2,XDI2,FPJ,FPJD,FPJ,2)
*
      DO K = 1,3
       FP(K) = FP(K)+FPI(K)-FPJ(K)
       FPDOT(K) = FPDOT(K)+FPID(K)-FPJD(K)
      ENDDO
*
*
...
*       Form higher derivatives by boot-strapping (Q2 added 11/1/98).
     ...
     &                        TDOT3(IPAIR)*A(K+4))
          ...
          FUDOT3(K,IPAIR) = FUDOT3(K,IPAIR) + 0.5*TDOT2(IPAIR)*Q2(K)
          HDOT3(IPAIR) = HDOT3(IPAIR) + 2.0*UDOT(K,IPAIR)*Q2(K)
...
*       Assign a conservative value of the initial step (except IMOD > 1).
      ...
      DTU = FAC*ETAU1*SQRT(A2)/(1.0 + 1000.0*GAMMA(IPAIR))**0.333
*
