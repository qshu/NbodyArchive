...
*
      ...
      REAL*8  B2(3,4),U2(4),Q2(4),FRELP(6),FRELD(6),XI(6),VI(6)
*
...
*       See whether c.m. approximation applies.
     ...
     &                                      (X(3,J) - X(3,I))**2
              ...
              K = J - N
              IF (RIJ2.LT.rs2fac*CMSEP2*R(K)**2
     &             .AND.LIST(1,2*K-1).GT.0) THEN
                  JDUM = 2*K - 1
                  J = JDUM
              END IF
          END IF
...
******Relativistic modification for hard binaries (Kupi Gabor)
        DO 112, II =1,3
        XI(II) = X(II,I1)
        XI(II+3) = X(II,I2)
        VI(II) = XDOT(II,I1)
       ...
       FRELD(II + 3) = 0.0
  112   CONTINUE
        CALL KSREL(IPAIR,I1,XI,VI,FRELP,FRELD,VWHOLE)
        FRP2=SQRT(FRELP(1)**2 + FRELP(2)**2 + FRELP(3)**2)
	GREL = FRP2*R(IPAIR)**2/(BODY(I1)+BODY(I2))
	GAMMA(IPAIR) = GAMMA(IPAIR) + GREL
******
...
!*     There are not any unperturbed binaries in relatvistic case (Kupi Gabor)
