...
*       Predict current coordinates & velocities of the perturbers.
    ...
    5 CONTINUE
*
*       Ensure current values of KS solution.
      IF (IPHASE.EQ.0) THEN
          CALL RESOLV(IPAIR,1)
      END IF
*
...
*       Form regularized force & two derivatives of time & binding energy.
   ...
   60 CONTINUE
*
*       Include safety procedure to avoid TDOT2 < 0 near pericentre.
      IF (KSTAR(I).EQ.-2) THEN
          SEMI = -0.5*BODY(I)/H(IPAIR)
          IF (R(IPAIR).LT.SEMI.AND.TDOT2(IPAIR).LT.0.0) THEN
              ECC2 = (1.0 - R(IPAIR)/SEMI)**2 +
     &                                   TDOT2(IPAIR)**2/(SEMI*BODY(I))
              DR = (R(IPAIR) - SEMI*(1.0 - SQRT(ECC2)))/R(IPAIR)
              IF (ABS(DR).LT.0.03) THEN
                  TDOT2(IPAIR) = 0.0
              END IF
          END IF
      END IF
...
*       Include convergence criterion DH = H'*DTU + H''*DTU**2/2 = 0.001*|H|.
      ...
      END IF
*
*       Initialize reference energy and Stumpff coefficients.
      H0(IPAIR) = H(IPAIR)
...
*       Ensure that c.m. step is not exceeded.
      ...
      END IF
*
*       Add an arbitrary member for unperturbed motion (integrates to apo).
