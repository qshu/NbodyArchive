...
*       Use semi-major axis and/or 4*RMIN for perturber selection.
      IF (EB.LT.EBH) THEN
          IF (IPHASE.EQ.6) THEN
              RAP = SQRT(SEMI*RMIN)*(1.0 + ECC)
          ELSE
              RAP = SEMI*(1.d0 + ECC)
              RAP = MIN(RAP,4.d0*RMIN)
...
*       Adopt cutoff from n(r) = N/2*(r/RSCALE)**2 to limit GRAPE list < LMAX.
      ...
      ELSE
          RCRIT2 = MIN(RCRIT2,0.25d0*RSCALE**2)
      END IF
*
*       Impose minimum conditions for hyperbolic or soft binding energy.
      IF (EB.GT.0.0) THEN
          RCRIT2 = MIN(400.d0*RMIN2,0.25d0*RSCALE**2)
      ELSE IF (EB.GT.0.5*EBH) THEN
          RCRIT2 = MIN(RCRIT2,CMSEP2*RAP2)
      END IF
*
