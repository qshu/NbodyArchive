...
*
* Check if binary is relativistic, if so, no unperturbed period
* See define.f for meaning of each KZ(J)
             IF (KZ(41).LT.5) THEN
*                IF(((KZ(42).EQ.1).OR.((KZ(42).EQ.2).AND.((NAME(I1).EQ.1
*     &               .AND.NAME(I2).EQ.2).OR.(NAME(I1).EQ.2.AND.NAME(I2)
*     &               .EQ.1))).OR.(KZ(42).EQ.3.AND.(NAME(I1).EQ.1.OR.
*     &               NAME(I2).EQ.1)))
                IF ((KZ(43).EQ.0).OR.(KZ(43).EQ.1.AND.(VOVERC.GT.
     &               BETAREL))) GO TO 1222
             END IF
*
* If not relativistic, Specify one unperturbed period at apocentre
* (NB! check STEP(I)).
             STEP(I1) = TWOPI*SEMI*SQRT(SEMI/BODY(I))
             STEP(I1) = MIN(STEP(I1),STEP(I))
*
* If relativistic there is no unperturbed period.
 1222        CONTINUE
*
...
*       Adopt orbital period since current value could be small many times.
               ...
               ELSE
*
* Again there are no unpeturbed particles in the PN case so we skip
* calculating unperturbed timesteps.
* See define.f for meaning of KZ(J)
                  IF (KZ(41).LT.5) THEN
*                     IF (((KZ(42).EQ.1).OR.((KZ(42).EQ.2).AND.((NAME(I1)
*     &                    .EQ.1.AND.NAME(I2).EQ.2).OR.(NAME(I1).EQ.2
*     &                    .AND.NAME(I2).EQ.1))).OR.(KZ(42).EQ.3.AND.
*     &                    (NAME(I1).EQ.1.AND.NAME(I2).EQ.1))).AND.
                     IF ((KZ(43).EQ.0).OR.(KZ(43).EQ.1.AND.(VOVERC
     &                    .GT.BETAREL))) GO TO 1224
                  END IF
*
* If binary is non-relativistic calculate a new STEP(I1)...
                  STEP(I1) = TWOPI*SEMI*SQRT(SEMI/BODY(I))
                  STEP(I1) = MIN(STEP(I1),STEP(I))
*
* ...but don't if the binary is relativistic
 1224             CONTINUE
*
                  GAMMA(IPAIR) = 0.5*GMIN
               END IF
