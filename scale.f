...
*
      IF (KZ(22).EQ.2) THEN
          WRITE(6,*)' SCALE POT KIN ',POT,ZKIN
          QV = SQRT(Q*VIR/ZKIN)
...
*       Scale tidal radius given from King model.
      IF (KZ(22).GE.3) THEN
*       In this case Rtide is actually the maximum radius for the model. 
 ...
 72       FORMAT (/,12X,' KING MODEL: NEW RBAR =',F6.2)
      ...
      ELSEIF (KZ(14).GE.3.AND.KZ(22).NE.2) THEN
          RTIDE = 1.2*RIJ2
      ELSEIF (KZ(14).GT.1) THEN
          RTIDE = RTIDE/SX
*         RBAR = SX
      END IF
...
*       Check for general binary search of initial condition.
      ...
      END IF
*
*       Obtain actual half-mass radius.
      IF (KZ(7).GT.0) THEN
          CALL LAGR(RDENS)
      END IF
*
