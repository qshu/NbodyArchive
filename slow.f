...
*       Determine chain index and largest inverse distance.
          ...
          END DO
*
*       Sum the perturbing forces m/r^3 next to #i1.
          sum = 0.0
          do i = 1,n-1
...
*       Determine chain index for slow-down binary (otherwise from above).
      IF (KSLOW) THEN
          ...
          END DO
*       See whether a closer particle pair is present.
          I2 = 0
          DO I = 1,N-1
              IF (I.NE.I1.AND.RINV(I).GT.RINV(I1)) THEN
                  I2 = I
              END IF
          END DO
      END IF
...
*
*       Ensure selection of closest binary if N > 3.
      IF (.NOT.KSLOW.AND.N.GT.3) THEN
          RM2 = 0.0
          DO I = 1,N-1
              IF (I.NE.I1.AND.RINV(I).GT.RM2) THEN
                  RM2 = RINV(I)
                  I2 = I
              END IF
          END DO
*       Include twice the smallest separation for eccentricity effect.
          IF (RM2.GT.0.5*RM) THEN
              K1 = INAME(i2)
              K2 = INAME(i2+1)
              CALL EREL(i2,EB2,SEMI2)
              IF (SEMI2.GT.0.0.AND.SEMI2.LT.SEMI) THEN
*       Switch to the closest binary.
                  SEMI = SEMI2
                  i1 = i2
*       Sum the perturbing forces m/r^3 next to #i1.
                  sum = 0.0
...
*       Exit if no current binary (set KSLOW = .false. just in case).
      ...
      END IF
*
*       Check for switching to a second smaller binary.
      IF (KSLOW) THEN
          IF (I2.GT.0) THEN
...
*       Switch off the present pair to prepare re-activation.
      ...
      END IF
*
*       Sum the perturbations next to #i1 (done above if KSLOW not active).
      IF (KSLOW) THEN
          sum = 0.0
          do i = 1,n-1
              if (iabs(i-i1).eq.1) then
