...
*
         IF(KW.EQ.14)PRINT*,' MDOT: M0,MC,AGE,TN=',M0,MC,AGE,TN
*
...
*
       IF(KW.GE.10)PRINT*,'MDOT2:KW,I,IKICK,M0,MC,AGE,TN,DM,DMSUN=',
     *   KW,I,IKICK,M0,MC,AGE,TN,DM,DMSUN

*       Perform total force & energy corrections (delay dF if DMSUN > 0.1).
            IF (DMSUN.LT.0.05.AND.(KW.LT.10.OR..NOT.IKICK)) THEN
               IF(KW.GE.10)PRINT*,' goto ficorr '
                CALL FICORR(I,DM)
            ELSE
               IF(KW.GE.10)PRINT*,' goto fcorr '
                CALL FCORR(I,DM,KW)
            END IF
*
