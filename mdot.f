...
*       Update the masses (single body, standard KS or merger).
          IF (IGHOST.EQ.0) THEN
              BODY(I) = M1/ZMBAR
              IF (KW.LE.1.OR.KW.EQ.7) BODY0(I) = M0/ZMBAR
          ELSE
              J = 2*KSPAIR - 2 + IHDOT
              BODY(J) = BODY(J) - DM
              IF (KW.LE.1.OR.KW.EQ.7) BODY0(J) = M0/ZMBAR
          END IF
*
