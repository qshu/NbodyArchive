...
*       Update evolution times.
          ...
          TEV0(I2) = TEV(I2)
*
*       Copy new semi-major axis (RIJ0 if SEMI < 0) and specify mass loss.
          SEMI = SEP/SU
          IF (SEMI.LT.0.0) SEMI = RIJ0
          BODY(I1) = M1/ZMBAR
          BODY(I2) = M2/ZMBAR
          ZMB = BODY(I1) + BODY(I2)
          ...
          ZMASS = ZMASS - DM
*
*       Send all variables to GRAPE (the suppressed lines have been tested).
*         CALL GPSEND
*
*       Impose a small eccentricity for perturbed pericentre procedure.
          EMIN = ECC
          EFAC = SQRT((1.0 - EMIN)/(1.0 + EMIN))
*
*       Set new coordinates and velocities for relative & absolute motion.
          DO 20 K = 1,3
              XREL(K) = XREL(K)*SEMI/RIJ0
              XREL(K) = (1.0 + EMIN)*XREL(K)
...
*  22 CONTINUE
*
*       Obtain neighbour list for force corrections.
*     RS2 = RSCALE**2/FLOAT(N)**0.66667
*     CALL NBLIST(I1,RS2,NNB)
*
*       Remove the two dominant chain members (I1 & I2 located first).
*     DO 40 L = 1,2
*         DO 30 K = 2,NNB+1
*             IF (ILIST(K).EQ.JLIST(L)) THEN
*                 DO 25 KK = K,NNB
*                     ILIST(KK) = ILIST(KK+1)
*  25             CONTINUE
*                 NNB = NNB - 1
...
*       Add potential energy contributions to yield final correction.
          ECDOT = ECDOT + DM*POTJ
*
          WRITE (6,60)  NCH, (NAME(JLIST(K)),K=1,4), KSTAR(I1),
     &                  KSTAR(I2), KW1, KW2, M1, M2, DM*ZMBAR, R1, R2,
     &                  SEMI0*SU, SEMI*SU
   60     FORMAT (' CHAIN CE    NCH NAM K0* K* M1 M2 DM R1 R2 A0 A ',
     &                          I3,4I6,4I3,3F5.1,2F6.1,2F8.1)
*
*      Note that rare case of NS formation & kick is not treated (see EXPEL).
          KSTAR(I1) = KW1
          KSTAR(I2) = KW2
          IPHASE = 0
      END IF
