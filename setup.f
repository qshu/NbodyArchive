...
*
      ...
      REAL*4  RAN2
*
      REAL*8 RSTAR(NMAX),XMSTAR,VSTAR1(NMAX)
...
*       Scale coordinates & velocities to analytical expectation values.
      ...
      DO 50 I = 1,N
	      ...
	      XDOT(K,I) = XDOT(K,I) - CMRDOT(K)/ZMASS
              X(K,I) = SX*X(K,I)
*     Introduce  XPREV for Calculation of Black Hole Energy	      
              XPREV(K,I)=X(K,I)
              XDOT(K,I) = SV*XDOT(K,I)
   45     CONTINUE
*   
   50 CONTINUE 
      PRINT*,'88888888888888888',NAME(10)    
*
      DO I=1,N
      RSTAR(I)=SQRT(X(1,I)*X(1,I)+X(2,I)*X(2,I)+X(3,I)*X(3,I))
      INDEXVECTOR(I) = I
*     VSTAR1(I)=SQRT(XDOT(1,I)**2+XDOT(2,I)**2+X(3,I)**2)
      END DO
      CALL SORT1(N,RSTAR,INDEXVECTOR)
*     CALL SORT1(N,VSTAR,INDEXVECTOR)
*
      DO I=1,N
      L=INDEXVECTOR(I)
      XMSTAR=XMSTAR+BODY(L)/N
*     SV=SQRT(1.D0)
*      SV=SQRT(1.D0+CMBLHOLE/XMSTAR) /////SCALING FACTOR IN CASE OF BLACK HOLE
*     SX=SQRT(1.D0+CMBLHOLE/XMSTAR)
*     XDOT(K,L)=XDOT(K,L)*SX
      PRINT*,'N=',L,RSTAR(I),STEPR(I),STEP(I)
*     PRINT*,L,VSTAR1(I)
      DO K=1,3
