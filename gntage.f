...
* Set the minimum CHeB core mass using M = Mflash
         ...
         if(mc.le.mcy) kw = 3
         if(mc.le.mcy) WRITE(38,*)' GNTAGE4: changed to 3'
      endif
*
...
*
         if(aj.lt.0.d0.or.aj.gt.1.d0)then
            WRITE(6,*)' FATAL ERROR! GNTAGE4: fage out of bounds'
            WRITE(6,*)' FAGE ',aj
            CALL gpfree
            STOP
         endif
...
* allowed mass for a CHeB star and fage > 0.
     ...
     &             aj*mcagbf(mmid) - mc
            ...
            if(j.eq.jmax)then
               WRITE(6,*)' FATAL ERROR! GNTAGE4: root not found'
               CALL gpfree
               STOP
            endif
 10      continue
...
* First we double check that we don't have a GB star for M => Mfgb
         mcy = mcheif(zpars(3),zpars(2),zpars(9))
         if(mc.ge.mcy)then
            WRITE(6,*)' GNTAGE3: star too big for GB'
            CALL gpfree
            STOP
         endif
...
* Use Newton-Raphson to find m0 from Lbgb
 ...
 30         continue
            ...
            j = j + 1
            if(j.eq.jmax)then
               WRITE(6,*)' FATAL ERROR! GNTAGE3: root not found'
               CALL gpfree
               STOP
            endif
            goto 30
...
*
         ...
         if(mcx.ge.mc)then
            WRITE(6,*)' FATAL ERROR! GNTAGE9: mmin too big'
            CALL gpfree
            STOP
         endif
         f = mcx - mc
         ...
         do 50 , j = 1,jmax
            ...
            mmax = 2.d0*mmax
            if(j.eq.jmax)then
               WRITE(6,*)' FATAL ERROR! GNTAGE9: mmax not found'
               CALL gpfree
               STOP
            endif
 50      continue
...
* Use the bisection method to find m0
         if(f*fmid.ge.0.d0)then
            WRITE(6,*)' FATAL ERROR! GNTAGE9: root not bracketed'
            CALL gpfree
            STOP
         endif
         m0 = mmin
         ...
         do 70 , j = 1,jmax
            ...
            if(j.eq.jmax)then
               WRITE(6,*)' FATAL ERROR! GNTAGE9: root not found'
               CALL gpfree
               STOP
            endif
 70      continue
