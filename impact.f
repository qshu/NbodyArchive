...
*       Search c.m. neighbours if binary has at most two perturbers.
      ...
      NNB2 = LIST(1,I1) + 1
*
*       Find the dominant body (JCOMP) and nearest perturber (JCMAX).
      DO 10 L = 2,NNB2
          J = LIST(L,I1)
          NP = NP + 1
          JLIST(NP) = J
...
*       Compare with existing subsystem of same type (if any).
      IF (NSUB.GT.0.AND.KCHAIN.EQ.0) THEN
          ...
          IF (IGO.GT.0) WRITE (6,19)
   19     FORMAT (' IMPACT    TERMINATION REQUEST ')
...
*       Include any close single perturber (cf. routine SETSYS).
      IF (JCMAX.NE.JCOMP.AND.SQRT(RMAX2).LT.2.0*RIJ.AND.
     &    JCMAX.LE.N.AND.NAME(JCMAX).GT.0) THEN
          WRITE (6,22)  NAME(JCOMP), NAME(JCMAX), RSUM, SQRT(RMAX2)
...
*       Set relevant variables in DELAY before terminating inner binary.
          CALL DELAY(KCHAIN,KS2)
          CALL DELAY(IPHASE,-1)
*       Initialize new chain of the 4 members JCMAX, JCLOSE & KS components.
          ISUB = 0
          CALL CHAIN(ISUB)
...
*       Skip merger if an outer binary is fairly perturbed or not hard.
      IF (JCOMP.GT.N) THEN
          IF (GAMMA(JPAIR).GT.1.0E-04.OR.EB2.GT.EBH) GO TO 100
      END IF
*
...
*       Check whether the main perturber dominates the outer component.
      RIJ2 = (X(1,JCMAX) - X(1,JCOMP))**2+(X(2,JCMAX) - X(2,JCOMP))**2+
     &                                    (X(3,JCMAX) - X(3,JCOMP))**2
      FMAX = (BODY(JCMAX) + BODY(JCOMP))/RIJ2
...
*    &        (NMARG.GT.100000.OR.NMARG*TK.GT.0.5*DTADJ)) THEN
     ...
     &                      YFAC*PCRIT
   45         FORMAT (' NEW HIERARCHY    # ALPHA Q YF PM PC1 PC2 ',
...
*       Check Zare exchange stability criterion and create diagnostics.
   ...
   48         FORMAT (' ZARE TEST    T Q E E1 A PM PCR YF SP ',
     &                               F8.2,F5.1,2F7.3,1P,3E9.1,0P,2F6.2)
              GO TO 100
          END IF
          WRITE (73,49)  TIME, Q, ECC, ECC1, SEMI, PMIN, PCRIT, SP,
     &                   ANGLE, KSTAR(I)
...
*       Include diagnostics for double hierarchy or optional standard case.
     ...
     &                  BODY(I), BODY(JCOMP), PERT4, RIJ, PMIN,
     &                  EB1/EB, LIST(1,I1)
*       Note rare case of two hierarchies merging.
           IF (NAME(I).LT.0.AND.NAME(JCOMP).LT.0) THEN
               WRITE (6,60)  NAME(I), NAME(JCOMP), ECC, ECC1, R(JPAIR)
   60          FORMAT (' HI MERGE    NM E E1 RJ ',2I7,2F7.3,1P,E10.2)
           END IF
