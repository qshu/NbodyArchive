...
*         IF (SEMI.GT.20.0*SEMI0.AND.RIJ.GT.MAX(R(IPAIR),R(JPAIR))) THEN
          IF (PMIN.LT.SEMI.AND.RIJ.GT.2.0*MAX(R(IPAIR),R(JPAIR))) THEN
              IF (MAX(GAMMA(IPAIR),GAMMA(JPAIR)).LT.0.1) GO TO 100
          END IF
      END IF
*
...
*       Save initial energy in binaries for routine SETSYS.
          EBCH0 = EBT - EB1
*         WRITE (6,15)  I, JCOMP, SEMI, SEMI1, RIJ, RSUM, RGRAV, EBT
*  15     FORMAT (' IMPACT:   I J A A1 RIJ RSUM RG EBT ',2I4,6F8.4)
*         IF (RSUM.GT.3.0*RGRAV.AND.RIJ.LT.3.0*RGRAV) THEN
*         WRITE (6,16) I, J, SEMI, SEMI1, RGRAV, RIJ/(3.0*RGRAV)
*  16     FORMAT (' FAILED CHAIN:   I J A A1 RG D/3*RG ',
*    &                              2I5,1P,3E9.1,0P,F6.2)
*         END IF
...
*
*       Skip chain for almost stable triples (factor 1.2 is experimental).
      IF (JCOMP.LE.N.AND.PMIN.GT.3.0*SEMI) THEN
          CALL HISTAB(IPAIR,JCOMP,PMIN,RSTAB)
*         WRITE (6,12) ECC1, PMIN, RSTAB
*  12     FORMAT (' HISTAB    E1 PM RSTAB ',F8.4,1P,4E10.2)
          IF (PMIN.GT.1.2*RSTAB) GO TO 30
*         IF (PMIN.LT.1.2*RSTAB) GO TO 30
      END IF
*
...
*       Compare with existing subsystem of same type (if any).
      IF (NSUB.GT.0.AND.KCHAIN.EQ.0) THEN
          ...
          IF (IGO.GT.0) THEN
              NWARN = NWARN + 1
              IF (rank.eq.0.and.NWARN.LT.50) WRITE (6,19)  PERIM
   19         FORMAT (' IMPACT    TERMINATION REQUEST    PERIM',1P,E9.1)
              GO TO 100
          END IF
      END IF
*
...
*     CALL KSTERM
*
*       Suppress experimental procedure for the present.
...
*       Skip if outer pericentre is below the practical stability limit.
      IF (PMIN.LT.1.5*SEMI.OR.RIJ.GT.RMIN) GO TO 100
*
...
*         WRITE (6,44)  NMARG, ALPH, YF, PMIN, YF*PCRIT, TK
...
*    &                          I7,F7.1,F7.3,1P,3E10.2)
     ...
     &        (NMARG.GT.10000.OR.NMARG*TK.GT.0.5*DTADJ)) THEN
              ...
              if(rank.eq.0)
     &        WRITE (6,45)  NMARG, NAME(I), NAME(JCOMP), ALPH, Q,
...
*             WRITE (6,44)  ECC1, PERT, RP, RSI, TK1
*  44         FORMAT (' WATCH!    E1 G RP RSI TK1 ',F8.4,F8.4,1P,5E10.2)
          END IF
      END IF
*
...
*       Check Zare exchange stability criterion and create diagnostics.
   ...
   48         FORMAT (' ZARE TEST    T Q E E1 A PM PCR YF SP ',
     ...
     &                   SP, 360.0*ANGLE/TWOPI, KSTAR(I)
   49     FORMAT (' STAB    T Q E E1 A PM PCR SP IN K* ',
     &                      F8.2,F5.1,2F7.3,1P,3E9.1,0P,F6.2,F7.1,I4)
          CALL FLUSH(73)
      ELSE
*       Subtract hyperbolic flybys to give number of stable systems in MERGE.
          NMERG = NMERG - 1
      END IF
*
...
*
      IF (NMERGE.EQ.MMAX) THEN
          if(rank.eq.0)WRITE (6,50)  NMERGE
...
*     IF (NAME(I).LT.0.AND.NAME(JCOMP).GT.N) GO TO 100
*
      DO 55 ISUB = 1,NSUB
...
*
      IF (NAME(I).LT.0) THEN
*       Avoid frequent second-order mergers due to short look-up times.
          IF (KZ(19).GE.3) THEN
              TM = MIN(TEV(I1),TEV(I2),TEV(JCOMP))
              IF (TM - TIME.LT.0.05) GO TO 100
          END IF
          CALL HIMAX2(I1,ECC,SEMI,ECC1,SEMI1,EMAX,EMIN,ZI,TG,EDAV)
          PCRIT2 = stability(BODY(I1),BODY(I2),BODY(JCOMP),ECC,EMAX,
     &                                                        ANGLE)
          PCRIT2 = PCRIT2*SEMI
          PM = SEMI1*(1.0 - EMAX)
          IF (PM.LT.PCRIT2) THEN
              if(rank.eq.0)
     &        WRITE (6,58)  ECC1, EMAX, PM, PCRIT, PCRIT2, TG
   58         FORMAT (' WATCH HIMAX2    E1 EX PM PC PC2 TG ',
     &                                  2F7.3,1P,4E10.2)
          ELSE
              if(rank.eq.0)
     &        WRITE (6,59)  ECC1, EMAX, PM, PCRIT, PCRIT2, TG
   59         FORMAT (' CHECK HIMAX2    E1 EX PMX PC PC2 TG ',
     &                                  2F7.3,1P,4E10.2)
          END IF
      END IF
*
...
*       Note rare case of two hierarchies merging and identify ghost names.
   ...
   60         FORMAT (' HI MERGE    NM E E1 RJ ',2I6,2F7.3,1P,E10.2)
     ...
     &                  EB1/EB, LIST(1,I1)
      END IF
*
*       Extra Output RS Nov. 2003
      WRITE(45,445)TIME,PMIN/PCRIT,SEMI1,ECC1,PMIN,PCRIT,SEMI,ECC,
     &  ANGLE*180./3.14159,YFAC,IPAIR,H(IPAIR),RI2,
     &  NAME(I),NAME(I1),NAME(I2),BODY(I),BODY(I1),BODY(I2),
     &  EB,PERT,NAME(JCOMP),BODY(JCOMP),EB1,PERTA
 445  FORMAT(' stab t,ratio,aout,eout,pmin,pcrit,ain,ein,inc,yfac=',
     &  1p,10d13.5,
     &' inner i,h,r,n,ni1,ni2,m,mi1,mi2,eb,pert=',i8,2d13.5,3i8,5d13.5,
     &' outer n,m,eb,perta=',i8,3d13.5)
      CALL FLUSH(45)
*
