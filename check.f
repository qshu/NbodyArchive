...
*       See whether output intervals should be increased (at main output).
      IF (KZ(32).GT.0.AND.TIME.GT.TNEXT - 20.0*DTMIN) THEN
          IF (ABS(E(3)).LT.0.12.AND.KZ(32).EQ.1.OR.
     &        ABS(E(3)).LT.0.06.AND.KZ(32).EQ.2.OR.
     &        ABS(E(3)).LT.0.03.AND.KZ(32).EQ.3) THEN
              DTADJ = 2.0*DTADJ
              DELTAT = 2.0*DELTAT
              KZ(32) = KZ(32) + 1
              WRITE (6,5)  DTADJ, DELTAT
    5         FORMAT (/,5X,'NEW INTERVALS:   DTADJ =',F6.2,
     &                                    '  DELTAT =',F6.2)
          END IF
      END IF
*
...
*       Control variable NDUMP used to prevent a third restart.
      ...
      WRITE (6,20)  TIME, ETAI, ETAR, ETAU
...
*       Increase the time-step parameters (up to initial value only).
          IF (TIME.GT.0.0D0) THEN
              ...
              DTMIN = SQRT(ETACOR)*DTMIN
              SMIN = SQRT(ETACOR)*SMIN
              IF (ETACOR.GT.1.01) WRITE (6,40)  ETAI, ETAR, ETAU
          END IF
      END IF
*
   ...
   50 IF (ETACOR.LT.1.0.AND.KZ(2).GT.2) THEN
          DO 60 I = IFIRST,NTOT
              ...
              IF (DMOD(T0R(I),0.5D0*STEPR(I)).EQ.0.0D0) THEN
                  IF (T0R(I) + 0.5*STEPR(I).GE.TIME) THEN
                      STEPR(I) = 0.5*STEPR(I)
                  END IF
              END IF
   60     CONTINUE
