...
*
      ...
      ECC = (1.0-R(IPAIR)/SEMI)**2 + TDOT2(IPAIR)**2/(BODY(ICM)*SEMI)
      ECC = SQRT(ECC)
*
*       Save regular c.m. step and form irregular time-step (cf. ADJUST).
      CMSTEP = STEPR(ICM)
      DTCL = 0.4*SQRT(ETAI*R(IPAIR)**3/BODY(ICM))
...
*       Include error of the bilinear relation (cf. Book Eqn.(4.30)).
   ...
   15     FORMAT (/,' END KSREG    TIME =',F11.5,1P,2E9.1,0P,F6.3,1P,
...
*       Check option for external force.
          ...
          END IF
*
*       Skip timestep calculation for multiple system initialization
          IF (IPHASE.EQ.4.OR.IPHASE.EQ.5.OR.IPHASE.EQ.8) GO TO 149
*
