...
*       Set index of KS pair & first component of c.m. body #I.
      ...
      JC = LIST(2,I1)
*
*       Find the closest body (JCOMP) and copy perturbers to KLIST.
      NNB = LIST(1,I1)
      DO 10 L = 2,NNB+1
          J = LIST(L,I1)
          KLIST(L-1) = J
...
*       Skip rare case of merged binary or chain c.m. (denoted by NAME <= 0).
      IF (NAME(I).LE.0.OR.NAME(JC).LE.0) GO TO 20
*
      RDOT = (X(1,I) - X(1,JC))*(XDOT(1,I) - XDOT(1,JC)) +
     &       (X(2,I) - X(2,JC))*(XDOT(2,I) - XDOT(2,JC)) +
     &       (X(3,I) - X(3,JC))*(XDOT(3,I) - XDOT(3,JC))
*
      VREL2 = (XDOT(1,I) - XDOT(1,JC))**2 + 
     &        (XDOT(2,I) - XDOT(2,JC))**2 +
     &        (XDOT(3,I) - XDOT(3,JC))**2
 
*       Evaluate outer semi-major axis, eccentricity & impact parameter.
      RIJ = SQRT(RJMIN2)
      SEMI1 = 2.0/RIJ - VREL2/(BODY(I) + BODY(JC))
      SEMI1 = 1.0/SEMI1
      ECC1 = SQRT((1.0D0 - RIJ/SEMI1)**2 +
     &                     RDOT**2/(SEMI1*(BODY(I) + BODY(JC))))
...
*
      I2 = I1 + 1
      RJI1 = (X(1,JC) - X(1,I1))**2 + (X(2,JC) - X(2,I1))**2 +
     &                                (X(3,JC) - X(3,I1))**2
      RJI2 = (X(1,JC) - X(1,I2))**2 + (X(2,JC) - X(2,I2))**2 +
     &                                (X(3,JC) - X(3,I2))**2
*
*       Identify the dominant interaction (#I1 or #I2).
      FJI1 = (BODY(I1) + BODY(JC))/RJI1
      FJI2 = (BODY(I2) + BODY(JC))/RJI2
      NNB = NNB + 1
...
*       Evaluate radial velocity (routine SEARCH requires RD < 0).
      ...
      VJJ = 0.0
      DO 15 K = 1,3
          RD = RD + (X(K,JC) - X(K,J))*(XDOT(K,JC) - XDOT(K,J))
          RJJ = RJJ + (X(K,JC) - X(K,J))**2
          VJJ = VJJ + (XDOT(K,JC) - XDOT(K,J))**2
   15 CONTINUE
*
*       Determine vectorial perturbation on intruder and closest component.
      CALL FPERT(JC,J,NNB,KLIST,PERT)
      GJ = PERT*RJ2/(BODY(JC) + BODY(J))
      RJ = SQRT(RJJ)
*     WRITE (6,18) NAME(J),NAME(JC),GAMMA(IPAIR),GJ,R(IPAIR),RJ,RD,
*    &             RDOT
...
*       Increase the cross section for quadruples.
      IF (JC.GT.N) THEN
          ...
          ELSE
              HJ = H(JPAIR)
          END IF
          SEMI2 = -0.5*BODY(JC)/HJ
          APO = APO + ABS(SEMI2)
      END IF
*
      SEMIJ = 2.0/RJ - VJJ/(BODY(JC) + BODY(J))
      SEMIJ = 1.0/SEMIJ
...
*       Terminate for chain candidate but exclude H > 0 (avoids NEW KS).
      ELSE IF (GJ.GT.0.7*GAMMA(IPAIR).AND.RD.LE.0.0.AND.
     &         H(IPAIR).LT.0) THEN
          ITERM = 2
      END IF
*
