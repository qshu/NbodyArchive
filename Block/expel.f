...
*       Save current semi-major axis.
      ...
      RP1 = R(IPAIR)
      ISKIP = 0
*       Skip possible further calls for hyperbolic motion.
      IF (ECC.GT.1.0) GO TO 60
*
...
*       Conserve total mass and specific energy (H = const here).
          ...
          R1 = RADIUS(I1)*SU
          R2 = RADIUS(I2)*SU
          DM11 = DM1              ! Save mass-loss for diagnostics.
...
*       Transform to positive radial velocity and obtain global coordinates.
              JPAIR = -IPAIR
              CALL KSAPO(JPAIR)   ! Negative argument randomizes the phase.
...
*
     ...
     &                  KW1, KW2, M1, M2, DM1, ECC0, ECC, R1, R2, RP1,
     &                  SEMI0*SU, SEMI*SU
...
*       Re-initialize the KS solution with new perturber list.
              CALL KSLIST(IPAIR)
              CALL KSPOLY(IPAIR,1)
*        Assign a random phase transformation for unperturbed orbit.
              IF (LIST(1,2*IPAIR-1).EQ.0) THEN
                  JJ = -IPAIR      ! Staying at pericentre is difficult.
                  CALL KSAPO(JJ)
              END IF
