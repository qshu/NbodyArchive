...
*
      IF (LKSINT(IPAIR)) THEN
          JPHASE = IPHASEX(IPAIR)
      ELSE
          JPHASE = IPHASE
      END IF
*
!$omp critical (chaos_critical)
      DO 1 K = 1,NCHAOS
...
*       Restore case of former merger with KSTARM < 0 to chaos table.
          ...
          NAMEC(NCHAOS) = NAME(I)
      END IF
!$omp end critical (chaos_critical)
*
...
*       Update orbital parameters after merger, mass loss or radius change.
          IF (DMR.GE.0.0D0) THEN
              ...
              ELSE
                  JPHASE = IPHASE
              END IF
              IF (JPHASE.LT.0.OR.KSTAR(I).GT.0) GO TO 30
          END IF
*
...
*       Check Roche radius but skip RESET call (no second EMERGE correction).
              IF (RADIUS(J1).GT.RL1.AND.JPHASE.NE.7) THEN
...
*       Obtain KS variables at pericentre and enforce collision or CE.
                  ...
                  IF (LKSINT(IPAIR)) THEN
                      KSPAIRX(IPAIR) = IPAIR
                      IF (JPHASE.EQ.0) IPHASEX(IPAIR) = -1
                  ELSE
                      KSPAIR = IPAIR
                      IQCOLL = 2
                      CALL CMBODY(QPERI,2)
