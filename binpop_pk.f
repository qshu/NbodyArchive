...
C
C P. Kroupa   1.04.94
C Edited in Aug.1998 for nb6++ to be in accord with binpop.f which
C           uses quantities NBGR and REDUCE. 
C
C NOTE: This routine is an amended version of binpop_new_new.f, and
C works for <= 100 per cent binaries as the initial
C set up.
C This routine is as binpop_new_new.f in that it allows choice of an
...
*
*
      READ (5,*)  NBIN, SEMI0, ECC0, RATIO, NBGR, REDUCE, 
     &            RANGE, NSKIP, IDORM
      NBIN0 = NBIN
      NBIN1 = NBIN + 1
      WRITE (6,1)  NBIN, SEMI0, ECC0, RATIO, NBGR, REDUCE, 
     &             RANGE, NSKIP, IDORM
...
* exp = logP(days)
             exp1 = SQRT(RAN2(IDUM1)*2.0/0.03438)
* Period in yrs:
             exp1 = 10**exp1 /365.25
...
*========= BK(2) = 2 ========
             ...
             exp1 = EXP(2.0*RAN2(IDUM1)/3.50) - 1.
             exp1 = SQRT(exp1*100.)
* exp = logP(days)
* Period in yrs:
             exp1 = 10**exp1 /365.25
...
*========= BK(2) = 3 ========
             ...
             exp1 = EXP(2.D0*RAN2(IDUM1)/2.3D0) - 1.D0
...
* Solar radius in AY is 4.6523D-3.
...
*
C For consistency with binpop.f as used in nb6++ until now add
C the following lines: (Aug.1998, P.Kroupa)
          if (BK(2).EQ.-1) then
             IBIN = IBIN + 1
...
*
      ...
      write(6,*)
      write(6,*)' ZMASS and BODY0(i) updated.'
      write(6,'(a,F10.4)')' ZMASS= ',ZMASS
      write(6,*)
*
c+++ for testing:
c      write(6,*)
