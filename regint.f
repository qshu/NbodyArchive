...
*       August 2001: NNBOPT is not working properly, went back to
*                    old algorithm by SJA using NNBMAX
   40 CONTINUE
*
      FAC = 1.D0
*     IF (KZ(40).GT.0) THEN
...
*       Calculate the radial velocity with respect to at most 3 neighbours.
      IF (NNB.LE.3.AND.RI2.LT.100.0*RSCALE**2) THEN
          A1 = 2.0*RS(I)
*
          DO 45 L = 1,NNB
              J = ILIST(L+1)
...
*
*       Call Drag Force
*     CALL DRAGFORCE(I,FIRR,FREG,FD,FDR)
*
...
*
*/////////////////////////////////////////////////////////
*      FRG=SQRT(FREG(1)**2+FREG(2)**2+FREG(3)**2)
*      FDG=SQRT(FDR(1)**2+FDR(2)**2+FDR(3)**2)
*      FRR=SQRT(FIRR(1)**2+FIRR(2)**2+FIRR(3)**2)
*      FDD=SQRT(FD(1)**2+FD(2)**2+FD(3)**2)
*      CALL DRAGBLCKHL(I,W0,W1)
*      FBL=SQRT(FBLACKH(1)**2+FBLACKH(2)**2+FBLACKH(3)**2)
*
*      IF(FBL.GT.FRR*0.01D0) THEN
*      DTNEW=STEP(I)
*      TTMP=DTNEW
*      END IF      
*/////////////////////////////////////////////////////////
...
*     PRINT*,' New Step = ',TTMP,' Old ',STEPR(I),' Quot ',TTMP/STEPR(I)
      STEPR(I) = TTMP
*///////////////////////////////////////////////////////////
*      IF(STEPR(I).LT.1.E-04)THEN
*      PRINT*,' I',I,'  TIME=',TIME,'  STEPREG=',STEPR(I),
*     *   '  RADIUS=',SQRT(X(1,I)**2+X(2,I)**2+X(3,I)**2),
*     *   '  FORCE=',SQRT(FREG(1)**2+FREG(2)**2+FREG(3)**2)
*      END IF
*///////////////////////////////////////////////////////////
      RETURN                               
