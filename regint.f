...
*       Calculate potential with little extra cost.
      ...
      REAL*8  W0(4),W1(4),W2(4),W3(4),XZNEW(NMAX),R_XY
...
*      CALL DRAGFORCE(I,XI,XIDOT,FIRR,FD,FREG,1)
*
...
*                   quantized step)
      XZNEW(I)=XN(3,I)+STEPR(I)*XNDOT(3,I)
...
*     IF(DABS(XZNEW(I)).LT.HZ*1.8.AND.R_XY.LE.D_SCALE.AND.
*     &       DABS(XN(3,I)).GT.HZ*2.2) THEN
      IF(DABS(XZNEW(I)).LT.HZ*1.8) THEN
      STEPR(I) = 0.5 * (0.5 * (0.5 *STEPR(I)))
      TTMP=STEPR(I)
      END IF
      END IF
*
