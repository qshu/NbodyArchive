...
*
      ...
      INTEGER io,ireg,iq,icall,lq
*
      REAL*8 xgp(3,nmax),vgp(3,nmax)
...
*
     ...
     &                tcl,stepcl,tbig,dtbig,ncl,newcl
      INTEGER G6_PRINT_CHIP_STATUS,G6_CHANGED_INDEX
      EXTERNAL G6_PRINT_CHIP_STATUS
      EXTERNAL short
...
*
      ...
      time = t0(i) + step(i)
      ierr = 0
*     IF (TIME.GT.10.975) THEN
*     WRITE (6,17) I, NXTLEN, NSTEPU, TIME, STEP(I)
*  17 FORMAT (' INT   I LEN # T DT ',I6,I4,I9,F12.6,1P,E10.2)
*     CALL FLUSH(3)
*     END IF
*
* Re-determine list if current time exceeds the boundary.
      if(time.gt.tlistq) goto 10
      if(step(i).lt.1.0d-14)then
         WRITE(6,*)' BAD STEP ',i,step(i),t0(i)/step(i)
         CALL gpfree
         STOP
      endif
*
...
*
* Predict the first block of particles to be integrated (order FDOT or D2). 
...
* Predict next block (if any) while GRAPE is busy (order FDOT or D2).
     ...
     &                                               + x0(kk,j)
               xdot(kk,j) = (fdot(kk,j)*s1 + f(kk,j))*s2 + x0dot(kk,j)
               xgp(kk,k) = d2(kk,j)*s4 + x(kk,j)
               vgp(kk,k) = d2(kk,j)*s3 + xdot(kk,j)
            enddo
         enddo
*
...
* --------------------------------
      IF (N.GT.10) GO TO 7777
         JJ = g6_changed_index(0)
         IF (JJ.GE.0) THEN
         WRITE (6,777) JJ, NSTEPI
  777    FORMAT (' WATCH!    JJ NSTEPI ',I7,I12)
         DO 780 LL = 1,np
         jnext = jnext + 1
         j = nxtlst(jnext)
         ERR = 0.0
         FD2 = 0.0
         DO KK = 1,3
         ERR = ERR + (6.0*FDOT(KK,J) - gpjerk(KK,LL))**2
         FD2 = FD2 + 36.0*FDOT(KK,J)**2
         ENDDO
         ERR = SQRT(ERR/FD2)
         IF (ERR.GT.0.1) THEN
         WRITE (6,778) J,(6.0*FDOT(kk,j),kk=1,3),(gpjerk(kk,ll),kk=1,3),
     &                 STEP(J)
  778    FORMAT (' J FD jerk DT ',I6,1P,3E10.2,2X,3E10.2,E10.1)
         END IF
  780    CONTINUE
         CALL FLUSH(3)
         CALL GPFREE
...
* Check for hardware error.
         if(gperr.ne.0)then
            WRITE(6,*)'GRAPE HARDWARE ERROR: RESET REQUIRED 1',time
            WRITE(6,*)'g6calc_lasthalf returned ',gperr,' in intgrt'
            WRITE(6,*)' II NP NXTLEN ',ii,np,nxtlen
            NCHIP = G6_PRINT_CHIP_STATUS(0)
            WRITE(6,75)NCHIP
   75       FORMAT(' NCHIP   ',I5)
...
* Send corrected X & XDOT and prediction variables & mass to GRAPE.
      ...
      do ii = 1,nxtlen
         ...
         gpaddr(ii) = i - ifirst
         gpindx(ii) = i
* Copy all corrected coordinates & velocities (only at the end).
         do kk = 1,3
            x(kk,i) = x0(kk,i)
     ...
     &                          x0dot(1,i),x0(1,i))
      enddo
*
*     IF (TIME.GT.34.775.AND.TIME.LT.34.783) THEN
*     WRITE (6,600)  NXTLEN, IQ, TBLOCK-TPREV
* 600 FORMAT (' WATCH!    NXTL IQ T-TB  ',I5,I4,1P,4E10.2)
*     CALL FLUSH(3)
*     END IF
* Update time of current block at end of cycle.
      tprev = time
*
