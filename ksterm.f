...
*       Select neighbours for components inside the modified c.m. sphere.
   20 NNB1 = 1
     ...
     &                                    (X(3,ICM) - X(3,J))**2
          CALL rs2modf(rs2fac,BODY(ICM),BODY(J))
*       Ensure that at least the predicted neighbour number is reached.
          IF (RIJ2.LT.(rs2fac*RS2).OR.L + NBP.GT.NNB1 + NNB) THEN
              NNB1 = NNB1 + 1
              ILIST(NNB1) = J
          END IF
   25 CONTINUE
...
*       Check that there is space for adding dominant component later.
      ...
      END IF
*
*Deleting of every relativistic variables connected to the pair (Kupi Gabor)
 
            PREVRELENERGY = PREVRELENERGY - RELENERGY(IPAIR)
	    IF(COLLISION.EQ.1) THEN
	     DO 373 II = 1, KMAX
              IF(ESCRELCOR(II).EQ.0.0)
&              ESCRELCOR(II) = RELENERGY(IPAIR)
  373        CONTINUE
            END IF	
             COLLISION = 0
