...
*
* PN variables
      REAL*8  SAVE(14),HOLD,CEPAR(5),CEMAX
      INTEGER*4 CENAM(4)
      LOGICAL CETYPE
      CHARACTER*8 WHICH1
      CHARACTER*11 XSTR
      COMMON /CLOSENC/ CEMAX,CENAM,CEPAR,CETYPE
*
...
*
   10 IF (KZ(10).GT.1) THEN
     ...
     &              (X(3,ICM) - RDENS(3))**2)
          if (rank.eq.0)
     &    WRITE (6,15)  TIME+TOFF, NAME(I1),NAME(I1+1),BODY(I1), 
     &         BODY(I1+1), DTAU(IPAIR),R(IPAIR), RI, H(IPAIR), IPAIR,
     &         GAMMA(IPAIR),STEP(I1), LIST(1,I1), LIST(1,ICM)
   15     FORMAT (/,' END KSREG    TIME =',1PE10.2,2I8,0P2F8.4,F8.3,
     &         1PE10.1,0PF7.2,F9.2,I5,F8.3,1PE10.1,2I5)
      END IF
*
...
*
* ------------------------------------------------------
* Delete all relativistic variables connected to KS pair
* ------------------------------------------------------
*
      IF (KZ(41).LT.5.AND.RFLAG(IPAIR).EQ.1) THEN
         PREVRELENERGY = PREVRELENERGY - RELENERGY1(IPAIR) 
     &        - RELENERGY2(IPAIR) - RELENERGY2_5(IPAIR)
*         IF (ICOLLISION.EQ.1) THEN
*            DO 373 II = 1,KMAX
*               IF(ESCRELCOR(II).EQ.0.0) THEN
*                  ESCRELCOR(IPAIR) = RELENERGY1(IPAIR) 
*     &                 + RELENERGY2(IPAIR) + RELENERGY2_5(IPAIR)
*                  write (*,373) TIME+TOFF,ETOT,PREVRELENERGY,
*     &                 ESCRELCOR(IPAIR),RELENERGY1(IPAIR),
*     &                 RELENERGY2(IPAIR),RELENERGY2_5(IPAIR)
* 373              format('PN merger energy check: TIME',f10.2,
*     &                 ' ETOT = ',e15.6,' PREVRELENERGY = ',e15.6,
*     &                 ' ESCRELCOR = ',e15.6,' PN1_energy = ',e15.6,
*     &                 ' PN2_energy = ',e15.6,' PN2.5_energy = ',e15.6)
*               END IF
* 373           CONTINUE
* Add the relativistic energy back in order to see if this takes care
* of the energy conservation problem at merger
*               PREVRELENERGY = PREVRELENERGY + ESCRELCOR(IPAIR)
*
         WRITE (*,*) ' '
         WRITE (6,273) TIME+TOFF,RELENERGY1(IPAIR),RELENERGY2(IPAIR),
     &        RELENERGY2_5(IPAIR),PREVRELENERGY
 273     FORMAT ('Rel Energy: TIME = ',f12.4,' PN_1 = ',e15.6,
     &        ' PN_2 = ',e15.6,' PN_2.5 = ',e15.6,' PREVRELENERGY = ',
     &        e15.6)
*
         ICOLLISION = 0
*
         DO 374 II = IPAIR,NPAIRS-1
            RFLAG(II) = RFLAG(II+1)
            RELENERGY1(II) = RELENERGY1(II+1)
            RELENERGY2(II) = RELENERGY2(II+1)
            RELENERGY2_5(II) = RELENERGY2_5(II+1)
            RELENERGY0(II) = RELENERGY0(II+1)
            RELTSTEP(II) = RELTSTEP(II+1)
            ILL(II) = ILL(II+1)
            ESCRELCOR(II) = ESCRELCOR(II+1)
 374     CONTINUE
*
         RFLAG(NPAIRS) = 0
         RELENERGY1(NPAIRS) = 0.0
         RELENERGY2(NPAIRS) = 0.0
         RELENERGY2_5(NPAIRS) = 0.0
         RELENERGY0(NPAIRS) = 0.0
         RELTSTEP(NPAIRS) = 0.0
         ILL(NPAIRS) = 0.0
         ESCRELCOR(NPAIRS) = 0.0
*
      END IF
*
...
*       See whether to include new c.m. or single components in NLIST.
      ...
      END IF
*
* Reset flag for a relativistic event
      KSTHEFIRST = 0
*
      RETURN
*
