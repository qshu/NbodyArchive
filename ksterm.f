...
*      NBP = MIN(ALPHA*SQRT(FLOAT(NNB)*RS(ICM))/(RS(ICM)**2),ZNBMAX)
      NBP = MIN(NNB + 0.1*(FLOAT(NNBOPT) - FLOAT(NNB)),ZNBMAX)
...
*       Improve force polynomials of strong perturber after rectification.
          IF (JMIN.GE.IFIRST) THEN
              NNB1 = LIST(1,JMIN)
              call xvpred(JMIN, NNB1)
              CALL FPOLY1_KS(JMIN,JMIN,0)
              CALL FPOLY2_KS(JMIN,JMIN,0)
          END IF
      END IF
*
...
c$$$         do i=ifirst,ntot
c$$$            if(name(i).eq.560) then
c$$$           print*,rank,'kse',i,name(i),x0(1,i),x0dot(1,i),
c$$$     *      step(i),t0(i),f(1,i),fdot(1,i),time
c$$$           end if
c$$$        end do
c$$$      end if
c$$$      if(name(jcomp).eq.560) then
c$$$         print*,rank,'ksterm',jcomp,x0(1,jcomp),x0dot(1,jcomp),time,
c$$$     *        iphase,f(1,jcomp),fdot(1,jcomp)
c$$$         call flush(6)
c$$$      end if
c$$$      itest=0
c$$$      print*,body(itest)
