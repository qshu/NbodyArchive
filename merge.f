...
*       Increase merger counter and set current merger index.
      ...
      IMERGE = NMERGE
*
*       Save component masses and define the reduced mass.
...
*       Ensure that outer binary components are resolved.
      ...
      END IF
*
*       Also improve the inner c.m. since coordinates may not be accurate.
      I = N + IPAIR
      CALL XVPRED(I,0)
*
...
*       Check for sufficient perturbers (RMAX = apocentre set in IMPACT).
      IF (SQRT(CMSEP2)*RMAX.GT.2.0*RS(ICOMP)) THEN
          FAC = ZNBMAX/FLOAT(NNB)
