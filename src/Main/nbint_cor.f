...
*       Assume small mass at centre for special case of no neighbours.
      IF (NNB0.EQ.0) THEN
          ...
          FIJ = 0.01D0*BODYM/(RI2*SQRT(RI2))
          RDOT = 3.D0*(XI(1)*XIDOT(1) + XI(2)*XIDOT(2) +
     &                                 XI(3)*XIDOT(3))/RI2
          DO 10 K = 1,3
              FIRR(K) = -FIJ*XI(K)
...
*     --03/03/14 20:51-lwang-end----------------------------------------*
      DO 80 K = 1,3
         ...
         SUM = FID + FD(K)
         AT3 = 2.D0*DF + DT*SUM
         BT2 = -3.D0*DF - DT*(SUM + FID)
*       Use here new variables for consistency in parallel execution (R.Sp.)
          XN(K,I) = XI(K) + (0.6D0*AT3 + BT2)*DTSQ12
          XNDOT(K,I) = XIDOT(K) + (0.75D0*AT3 + BT2)*DT13
*
          FI(K,I) = FIRR(K)
          FIDOT(K,I) = FD(K)
...
*
      ...
      DT0 = TTMP
*
* ---lwang debug 2013 --- updated RSp 2020 ------------------*
      if(step(i).gt.1.d2*dt0) then
...
*            DTJ = STEP(I)*(1.0D-06*STEP(I)**2*F2/DV2)**0.1d0
*            TTMP = MIN(TTMP,DTJ)
*         END IF
*      END IF
*
*       Select discrete value (increased by 2, decreased by 2 or unchanged).
      IF (TTMP.GT.2.D0*STEP(I)) THEN
          IF (MOD(TIME,2.D0*STEP(I))<1.D-14) THEN
              TTMP = MIN(2.D0*STEP(I),SMAX)
          ELSE
              TTMP = STEP(I)
          END IF
