...
*              CALL JPRED(J,TIME,TIME)
    ...
    2         CONTINUE
     ...
     &                                  RDOT**2/(A1*(BODY(I) + BODY(J)))
              IF (1.d0/A1.GT.0.5d0/RMIN) THEN
                  ECC1 = SQRT(ECC2)
                  RP1 = A1*(1.d0 - ECC1)
                  ECC = 1.d0 - R(KSPAIR)/SEMI
                  RA = SEMI*(1.d0 + ECC)
                  SR = RP1/RA
                  GA = 2.d0*BODY(J)*(RA/RP1)**3/BODY(I)
*       Determine inclination (8 bins of 22.5 degrees).
                  FAC = A1A2/SQRT(A12*A22)
                  ANGLE = 360.d0*ACOS(FAC)/TWOPI
                  if(rank.eq.0)
...
*       Select closest single body inside 0.5*RMIN as KS component.
              IF (RIP.LT.0.5d0*RMIN.AND.J.LE.N) THEN
                  IF (JCL.GT.0) THEN
    ...
    5     CONTINUE
*
          IF (rank.eq.0.and.RX.LT.20.d0*RMIN) THEN
              WRITE (6,200)  NAME(JX), ECCX, RX, SEMIX*(1.d0 - ECCX),
     &                       RDX, SEMI
...
*       Ensure orbit is at pericentre (perturbed hyperbolic case is OK).
          ...
          IF (R(KSPAIR).GT.SEMI.AND.SEMI.GT.0.d0) THEN
              CALL KSAPO(KSPAIR)
              CALL KSPERI(KSPAIR)
...
*       Save collision distance and VINF before any common envelope stage.
          RCOLL = R(KSPAIR)
          VINF = 0.d0
          IF (H(KSPAIR).GT.0.d0) VINF = SQRT(2.d0*H(KSPAIR))*VSTAR
          ECC = 1.d0 - R(KSPAIR)/SEMI
*
...
*       Note JLIST(1->NCH) contains global indices (JLIST(4)=0 for NCH=3).
          ...
          KSTARI = 0
          VINF = 0.d0
          ECC = 1.d0 + 2.d0*EBS*DMINC/(BODY(I1)*BODY(I2))
          ECC = MAX(ECC,0.001D0)
          IF (EBS.GT.0) THEN
...
*       Check for coalescence (one or two bodies of zero mass).
                  IF (((BODY(I1))<1.D-14).OR.((BODY(I2))<1.D-14)) 
     &            THEN
*       Include rare case of two massless stars (defined by KSTAR = 15).
                      IF ((BODY(I1) + BODY(I2))<1.D-14) THEN
...
*       Create new body from c.m. and initialize zero mass ghost in #I2.
      ...
      BODY(I1) = ZM
      BODY(I2) = 0.0D0
      SPIN(I1) = (SPIN(I1) + SPIN(I2))*(1.d0 - DM/ZM)
      T0(I2) = TADJ + DTADJ 
      IF (KZ(23).EQ.0.OR.RTIDE.GT.1000.d0*RSCALE) T0(I2) = 1.0D+10
      DTMAX = DTK(1)
      CALL DTCHCK(TIME,DTMAX,DTK(40))
...
*       Ensure that ghost will escape next output (far from fast escapers).
          X0(K,I2) = 1000.d0*RSCALE*(X(K,I2) - RDENS(K))/RI
          X(K,I2) = X0(K,I2)
...
*       Determine neighbour list for chain member.
          IF (NSYS.GT.2) THEN
              RSI = RSCALE*(10.d0/DBLE(N - NPAIRS))**0.3333d0
              CALL NBLIST(I1,RSI)
          END IF
*
          NNB = LIST(1,I1)
...
*     Add J particle into NLSTDELAY
          ...
          DO 35 K = 1,3
              X0(K,I1) = 1000.d0*RSCALE*X(K,I1)/RI
              X(K,I1) = X0(K,I1)
              X0DOT(K,I1) = SQRT(0.004d0*ZMASS/RSCALE)*XDOT(K,I2)/VI
...
*       Include case of two ghost stars.
          IF (((BODY(I2))<1.D-14).AND.(I1.NE.I2)) THEN
              I1 = I2
              GO TO 32
          END IF
...
*       Print cluster scaling parameters at start of the run.
          if((rank==0).and.(ABS(TTOT)<1.D-14))then
