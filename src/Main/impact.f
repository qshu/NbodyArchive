...
*       Only accept inward motion or small secondary perturbation.
      PERT3 = 2.d0*R(IPAIR)**3*PERT2/BODY(I)
...
*     --03/17/14 21:11-lwang-end----------------------------------------*
      IF ((RDOT.GT.0.d0).OR.(PERT3.GT.100.d0*GSTAR)) GO TO 100
*
*       Include impact parameter test to distinguish different cases.
     ...
     &     (XDOT(3,I) - XDOT(3,JCOMP))**2
      RIJ = SQRT(RJMIN2)
      A3 = 2.d0/RIJ - A2/(BODY(I) + BODY(JCOMP))
      SEMI1 = 1.d0/A3
...
*     ks MPI communication
                  call ksparmpi(K_store,K_int,K_KSTAR,
     &              I,0,DBLE(KSTAR(I)))
              END IF
          END IF
      END IF
...
*       Form binding energy of inner & outer binary.
      ...
      EB1 = -0.5d0*BODY(JCOMP)*BODY(I)/SEMI1
*
...
*       Choose maximum of dominant scalar & total vectorial perturbation.
      ...
      PERT4 = 2.d0*RJMIN2*RIJ*PERT2/(BODY(I) + BODY(JCOMP))
...
*       Use combined semi-major axis for binary-binary collision.
      IF (JCOMP.GT.N) THEN
          ...
          J1 = 2*JPAIR - 1
          EB2 = -0.5d0*BODY(J1)*BODY(J1+1)/SEMI2
...
*       Do not allow negative or soft cross section
          IF ((1.d0/SEMI).LT.(0.5d0/RMIN)) GO TO 100
*       Consider merger for PMIN > SEMI and large semi-major axis ratio.
          IF ((PMIN.GT.SEMI).AND.(SEMI2.GT.20.d0*SEMI0)) GO TO 30
*      END IF
...
*     &         PMIN.GT.2.0*RMIN) GO TO 30
          IF (RIJ.GT.MAX(3.d0*RGRAV,RMIN).OR.RSUM.GT.2.d0*RMIN) GO TO 30
          GI = 2.d0*BODY(JCOMP)*(RI/RIJ)**3/BODY(I)
*       Enforce KS orbit using MERGE for high eccentricity if PMIN > 10*RI.
          IF ((ECC1.GT.0.99d0).AND.(PMIN.GT.10.d0*RI).AND.
     &        (PERTM.LT.GMAX)) GO TO 40
          IF ((KZ(27).GT.0).AND.(JCOMP.GT.N)) THEN
              IF (SEMI0.LT.SEMI2) J1 = I1
              RT = 4.d0*MAX(RADIUS(J1),RADIUS(J1+1))
*       Do not allow large distance ratio for nearly synchronous binary.
              IF (SEMI0.GT.RT.AND.RI.GT.25.d0*SEMI0) GO TO 30
              IF (MIN(SEMI0,SEMI2).LT.0.05d0*RIJ) THEN
              IF (MAX(SEMI0,SEMI2).LT.0.1d0*RIJ) GO TO 30
              END IF
          END IF
      END IF
*
*       Include special case of strong interraction and large ECC1.
      IF ((ECC1.GT.0.9d0).AND.(GAMMA(IPAIR).GT.0.01d0)) THEN
          IF ((APO.LT.0.01d0*RMIN).AND.(PMIN.LT.2.5d0*APO)) GO TO 16
      END IF
*
...
*     IF ((APO.GT.0.01*RMIN.OR.JCOMP.GT.N).AND.PMIN.GT.APO) GO TO 30
      IF ((APO.GT.0.01d0*RMIN.OR.JCOMP.GT.N).AND.PMIN.GT.1.5d0*APO) 
     &   GO TO 30
*     IF (APO.LE.0.01*RMIN.AND.PMIN.GT.2.0*APO) GO TO 30
      IF ((RIJ.GT.RMIN.AND.SEMI1.GT.0.d0).OR.RIJ.GT.2.d0*RMIN) GO TO 100
      IF (PERTM.GT.100.d0*GSTAR) GO TO 30
   16 IF (JCOMP.GT.N.AND.PMIN.GT.0.1d0*RMIN) THEN
          IF (PMIN.GT.A0 + SEMI2) GO TO 30
      END IF
      IF (JCOMP.GT.N.AND.PMIN.GT.4.d0*SEMIX.AND.
     &   ((ECC1.GT.0.9d0).AND.(ECC1.LT.1.d0))) GO TO 30
*
*       Check almost stable triples (factor 1.2 is experimental).
      IF (JCOMP.LE.N.AND.PMIN.GT.2.5d0*SEMI) THEN
          CALL HISTAB(IPAIR,JCOMP,PMIN,RSTAB)
          RA = SEMI1*(1.d0 + ECC1)
          IF (SEMI1.LT.0.d0) RA = RIJ
          GI = PERT*(RA/RIJ)**3
*       Use estimated apocentre perturbation for decision-making.
          IF (PMIN.GT.1.2d0*RSTAB) THEN
              IF (GI.LT.0.05d0) GO TO 30
*       Choose chain for critical case of highly eccentric outer orbit.
              IF (ECC1.LT.0.95d0) GO TO 100
          ELSE IF (PMIN.GT.0.7d0*RSTAB) THEN
*       Treat marginally stable triple according to external perturbation.
              IF (GI.LT.0.05d0) GO TO 30
              IF ((GI.LT.1.d0).OR.(ECC1.LT.0.9d0)) GO TO 100
          END IF
          IF ((PMIN.GT.0.6d0*RSTAB).AND.(PMIN.LT.0.9d0*RSTAB)) GO TO 100
*       Delay for large distance ratio outside 0.5*RMIN.
          IF (RIJ.GT.MAX(10.d0*APO,0.5d0*RMIN)) GO TO 100
          IF (RIJ.GT.10.d0*APO) GO TO 100
          IF (PMIN.GT.2.5d0*APO) GO TO 40
      END IF
      IF (PMIN.GT.3.d0*SEMI.AND.JCOMP.LE.N) GO TO 40
*
      IF (JCOMP.GT.N) THEN
          IF (RIJ.GT.10.d0*APO) GO TO 100
      END IF
...
*
      IF ((rank.eq.0).and.(H(IPAIR).GT.0.d0)) THEN
...
*     ks MPI communication JCMAX
               call ksparmpi(K_store,K_int,K_JCMAX,
     &          0,0,DBLE(JCMAX))
            END IF
         END IF
      END IF
...
*       Check B-B interaction for switch of IPAIR & JPAIR or inert binary.
   ...
   22     FORMAT (' CHAIN B-B    NAME(I1-I4)',4I10,'  K*(INCM)',I3,
     ...
     &         '  PERIM',E10.2,0P)
          RT = 4.d0*MAX(RADIUS(I1),RADIUS(I2))
          IF ((SEMI0.LT.4.d0*RT).AND.(LIST(1,J1).EQ.0).OR.
     &        (MIN(SEMI0,SEMI2).LT.0.01d0*RIJ)) THEN
...
*     ks MPI delay
      call ksparmpi(K_store,K_int,K_DELAY,0,0,0.D0)
      flag_delay=.true.
c$$$*
...
*       Begin check for merger of stable hierarchical configuration.
   30 RA = SEMI1*(1.d0 + ECC1)
      IF (SEMI1.LT.0.d0) RA = RIJ
*
*       Identify formation of wide quadruple before merger is accepted.
      IF (JCOMP.GT.N.AND.ECC1.LT.1.d0.AND.SEMI1.LT.0.1d0*RSCALE) THEN
          NNB = LISTQ(1) - 1
          K = 0
...
*       Generate diagnostics of first five outer orbits every half period.
          IF (K.LE.5.AND.TIME.GT.QCHECK.AND.KZ(15).GE.3) THEN
              ...
              QCHECK = TIME + MIN(0.5d0*TWOPI*TK,0.1d0*TCR)
              TK = DAYS*TK
...
*       Do not allow merger if Roche overflow or mass loss during next orbit.
          ...
          IF (KZ(34).GT.0.AND.TM - TIME.LT.STEPX) THEN
              RT = 5.d0*MAX(RADIUS(I1),RADIUS(I2))
...
*       Ensure SLEEP for circularizing binary with TCIRC > 1.0E+06.
          QPERI = A0*(1.d0 - ECC)
          RM = MAX(RADIUS(I1),RADIUS(I2))
          IF (KSTAR(I).EQ.-2.AND.QPERI.GT.10.d0*RM) THEN
              ICIRC = -1
              CALL INDUCE(IPAIR,JCOMP,EMAX,EMIN,ICIRC,TC,ANGLE,TG,EDAV)
...
*       Beware possible termination by routine HMDOT using QPERI < 3*RADIUS.
              IF (TC.LT.2000.d0.AND.ECC.GT.0.002d0) GO TO 100
          END IF
          IF (KZ(19).GE.3) THEN
...
*       Allow highly eccentric outer orbit (estimated PERT may be excessive).
      IF (ECC1.GT.0.98d0.AND.RIJ.LT.0.1d0*SEMI1) THEN
          PERT = PERT4
          GO TO 40
      END IF
*
...
*       Switch to direct integration for planetary systems if GI > 1D-04.
      ...
      END IF
      IF (PERT4.GT.GMAX.OR.GI.GT.0.05d0.OR.RIJ.GT.6.d0*RMIN) GO TO 100
 
*       Skip merger if an outer binary is fairly perturbed or not hard.
 40   IF (JCOMP.GT.N) THEN
          IF (GAMMA(JPAIR).GT.1.0d-03.OR.EB2.GT.EBH) GO TO 100
      END IF
*
*       Estimate relative perturbation at apocentre from actual value.
      GI = PERT*(SEMI1*(1.d0 + ECC1)/RIJ)**3
      IF (PERT.GT.GMAX.OR.GI.GT.0.02d0) GO TO 100
...
*       Note: moved down from label 30 with 0.1*SEMI1 replacing 2*PMIN.
          IF (ECC1.GT.0.95d0.AND.RIJ.LT.0.1d0*SEMI1) THEN
              GO TO 100
          END IF
      END IF
*
...
*       Adopt 10% fudge factor with linear dependence on smallest ratio.
          YFAC = 1.d0 + 0.1d0*MIN(SEMI2/SEMI,SEMI/SEMI2)
      ELSE
          YFAC = 1.d0
      END IF
*
...
*     --10/08/14 21:40-lwang-end-debug----------------------------------*
          IF (NST.EQ.0) THEN
              PCRIT = 0.98d0*PMIN*(1.d0 - PERT)
              PCR = stability(BODY(I1),BODY(I2),BODY(JCOMP),ECC,EOUT,
     &                                                          ANGLE)
              PCR = PCR*SEMI
*       Specify reduced peri if old criterion < PCRIT/2 (avoids switching).
              IF (PCR.LT.0.5d0*PCRIT) THEN
                  PCRIT = 0.75d0*PCRIT
              END IF
              IF (PCRIT.LT.YFAC*PCR.AND.PERT.LT.0.02d0.AND.
     &            NMTRY.LT.10) THEN
                  ALPH = 360.d0*ANGLE/TWOPI
...
*     ks MPI COMMUNICATION tev
                  ...
                  END IF
              ELSE
                  IF (EMAX.GT.0.8d0.OR.DE.GT.0.2d0.OR.DT.LT.0.1d0) 
     &               GO TO 100
              END IF
          END IF
      END IF
...
*       Check circularization and dissipation time (exclude Roche stages).
      IF (KZ(27).EQ.2.AND.KSTAR(I).LT.10.AND.ECC1.LT.1.d0.AND.
     &    NAME(I).GT.0) THEN
          ECC0 = ECC
...
*         EK = EK*(1.0 + ECC)/(1.0 + ECC0)
          PCRIT = PCRIT*(1.d0 + ECC)/(1.d0 + ECC0)
      END IF
*
*       Perform safety check on radii for case of no circularization.
      IF (KZ(27).EQ.0) THEN
          IF (SEMI*(1.d0 - ECC).LT.2.d0*MAX(RADIUS(I1),RADIUS(I2))) THEN
              IF (KZ(19).EQ.0) THEN
                  GO TO 100
              END IF
          END IF
...
*       Note inclination is determined by routine INCLIN for ECC < 0.1.
   ...
   48         FORMAT (' ZARE TEST  Time[NB] M(I3)/M(INCM) ECC0 ECC1 ',
     ...
     &                   TG, SP, 360.d0*ANGLE/TWOPI, KSTAR(I)
...
*       Specify the final critical pericentre using the fudge factor.
      PCRIT = YFAC*PCRIT
*
      IF (NAME(I).GT.N.AND.NAME(JCOMP).GT.N.AND.ECC1.GT.1.d0) GO TO 100
*       Skip if #JCOMP is a chain c.m. but allow bound double hierarchy.
      IF (NAME(JCOMP).EQ.0) GO TO 100
      IF (ECC1.GT.1.d0.AND.MIN(NAME(I),NAME(JCOMP)).LT.0) GO TO 100
      DO 55 ISUB = 1,NSUB
...
*       Check for diagnostic output of quadruples.
      IF (SEMI1.GT.0.d0.AND.JCOMP.GT.N.AND.KZ(15).GE.3) THEN
          ZMB = BODY(I) + BODY(JCOMP)
...
*       Save KS indices and delay merger until end of block step.
      CALL DELAY(KS2,KS2)
*     ks MPI delay
      if(.not.flag_delay) call ksparmpi(K_store,K_int,K_DELAY,0,0,0.D0)
      if(.not.flag_delay) call ksparmpi(K_store,K_int,K_DELAY,0,0,0.D0)
*
