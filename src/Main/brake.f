...
*      Check for circularized case. 
      IF(KSTAR(I).LT.10.AND.ECC1.LT.0.002d0)THEN
         ECC1 = 0.001D0
         KSTAR(I) = 10
      ENDIF
*
...
*       Include GR coalescence criterion for compact objects.
      ...
      IF (KSX.GE.13.AND.KZ(28).GT.0) THEN
         RCOAL = 6.d0*BODY(I)/CLIGHT**2
      ELSE
         RCOAL = R1 + R2
      END IF
      IF (rank.eq.0.and.KZ(28).GT.1.AND.DSEP.GT.0.d0) THEN
...
*       Specify KS coordinate & velocity scaling factors at arbitrary point.
         C2 = SQRT(SEMI1/SEMI)
*        V2 = 0.5d0*(BODY(I) + H(IPAIR)*SEMI1*(1.0 - ECC))
         V2 = 0.5d0*(BODY(I) + H(IPAIR)*R(IPAIR)*(SEMI1/SEMI))
         C1 = SQRT(V2/V20)
*
