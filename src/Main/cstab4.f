...
*       Use perturbed triple stability instead for small middle distance.
      IF (1.d0/RINV(2).LT.0.5d0*RSUM) THEN
          CALL CSTAB3(ITERM)
          GO TO 40
      END IF
*
...
*       Evaluate orbital elements for inner and outer motion.
      ...
      RB2 = SQRT(R2(I3,I4))
      SEMI2 = 2.d0/RB2 - VREL34/MB2
      SEMI2 = 1.d0/SEMI2
*
*       Skip if innermost triple is not stable (including ECC > 1).
      IF (SEMI.LT.0.d0.OR.ECC.GT.1.d0) THEN
          ITERM = 1
          GO TO 40
      END IF
...
*       Use the general stability formula for the widest binary.
      ...
      ELSE
          ...
          PCRIT = 0.d0
      END IF
      PCRIT = PCRIT*(1.d0 + 0.1d0*ABS(SEMI/SEMI2))*AIN
*
*       Check hierarchical stability condition for bound close pair.
      ITERM = 0
      PMIN = SEMI1*(1.0D0 - ECC1)
      IF (PMIN.GT.PCRIT.AND.AIN.GT.0.d0.AND.SEMI1.GT.0.d0.AND.
     &    RB.GT.SEMI) THEN
          ITERM = -1
          ALPHA = 180.d0*ALPHA/3.14d0
          if(rank.eq.0)
