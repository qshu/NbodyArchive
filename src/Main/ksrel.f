      SUBROUTINE KSREL(IPAIR,I1,M1,M2,XI,VI,DT,FRELP,FRELD,VWHOLE,
     &    APPLY,KSAK,KSBK)
*     
...
C     &                LISTC(LMAX)
      ...
      REAL*8 TIME,XI(6),VI(6),FRELP(3),FRELD(3),DT
...
*     variables such as NVEC and RIJ are recycled in the perturber sum at 101
      REAL*8 M1,M2
      REAL*8 L2,ECC2,SEMI,TZ,GE,ETEMP
...
*     GAMREL for switch-on criterion
     ...
     &        C2_5(3)*C2_5(3))/CL4/MOR*RIJ
         ...
         A(1) = A(1)+ C1_5(1)/CL2 + (C2(1)+C2_5(1))/CL4
!     
         A(2) = MOR*(KSAK*NY + KSBK*KSV(2))/RIJ
         A(2) = A(2)+ C1_5(2)/CL2 + (C2(2)+C2_5(2))/CL4

         A(3) = MOR*(KSAK*NZ + KSBK*KSV(3))/RIJ
         A(3) = A(3)+ C1_5(3)/CL2 + (C2(3)+C2_5(3))/CL4
...
************* Paulina Spin OUTPUT
*         L2 = LABS*LABS
*         ETEMP = (0.5*V1_V22 - MASAS/RIJ)
*         SEMI = -MASAS/(2.*ETEMP)
*         ECC2 = 1.+(2.*ETEMP*L2)/(MASAS*MASAS)   
*     Merging timescale estimation
*         GE = (1.0 - ECC2)**3.5
*     &        /(1.0 + (73.0/24.0 + 37.0*ECC2/96.0)*ECC2)
*         TZ = 5.0/64.0*(CLIGHTPN)**5.*GE*(SEMI)**4./(M1*M2*MASAS)
*         IF(TZ.LE.5.0E-04)THEN
*         WRITE (52,*) TIME+TOFF,TZ,M1,M2,SPN(1,SPNI1),SPN(2,SPNI1),
*     &        SPN(3,SPNI1), SPN(1,SPNI2),SPN(2,SPNI2),SPN(3,SPNI2),
*     &        KSX(1),KSX(2),KSX(3),KSV(1),KSV(2),KSV(3),SPNI1,SPNI2
*         END IF
*         CALL FLUSH(52)
