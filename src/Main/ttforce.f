...
*** FlorentR - new subroutine
      ...
      IF(TTDX(1)<1.D-14) TTDX(1) = 1.d0
      TTDX(2) = TTDX(1)
      TTDX(3) = TTDX(1)

* update the timestep only for the guiding centre. Else, use the stored
* value
      IF(ABS(DT) > 1.D-14) THEN
        TTDX(4) = DT
      ENDIF

      DO K=1,4
        TT2DX(K) = 2.d0 * TTDX(K)
        TT12DX(K) = 12.d0 * TTDX(K)
      ENDDO
...
* Get the galactic force at XI:
      ...
      FM(1) =(TTPHI(17)-8.D0*TTPHI(5)+8.d0*TTPHI(4)-TTPHI(16))/TT12DX(1)
      FM(2) =(TTPHI(20)-8.D0*TTPHI(6)+8.d0*TTPHI(3)-TTPHI(13))/TT12DX(2)
      FM(3) =(TTPHI(25)-8.D0*TTPHI(7)+8.d0*TTPHI(2)-TTPHI(8))/TT12DX(3)
...
* if the potential is time dependent
      IF((TTTDEP .GT. 0) .AND. (ABS(TTDX(4)) > 1.D-14)) THEN
