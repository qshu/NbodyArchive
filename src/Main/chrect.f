...
*     Skip on call from RESET/MDOT with arbitrary phase (only updating).
         IF (ABS(TDOT2(IPAIR)).GT.1.0D-12.AND.DMR.GE.0.d0) THEN
...
*     Restrict lookup time to TC/2 for exit from possible merger.
         TEV(I1) = MIN(TEV(I1),TIME + 0.5d0*TC/TSTAR)
*     ks MPI communication TEV
...
*     Determine index of most recent degenerate object formation.
         ...
         END IF
         IF (TIME - TEV0(J1).LT.2.d0*STEPX.AND.TC.GT.3000.d0) THEN
            SLEEP = .TRUE.
         END IF
      END IF
*     
...
*     Re-initialize all chaos parameters after expansion or check SLEEP.
      IF (DMR.GT.0.01d0.OR.KSTAR(I).EQ.-1.OR.SLEEP) THEN
         IF (rank.eq.0.and.DMR.GT.0.01d0.AND.TC
     &       .GT.100.d0.AND.KM.LT.5) THEN
...
c$$$            CALL CMBODY(2)
      ...
      ELSE IF (KSTAR(I).EQ.-2) THEN
         I1 = 2*IPAIR - 1
         I2 = I1 + 1
         IF (QPERI.LT.2.d0*MAX(RADIUS(I1),RADIUS(I2))) THEN
...
*     Define mass ratio and evaluate Roche radius for the primary.
            ...
            Q1 = Q0**0.3333d0
            Q2 = Q1**2
            RL1 = 0.49d0*Q2/(0.6d0*Q2 + LOG(1.D0 + Q1))*SEMI
