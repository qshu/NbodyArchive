...
*    changed RS/MG Oct 2017 test
      IF (MASS(1) + MASS(2).GT.10.d0) ACRIT = 1000.d0
*
     ...
     &            MASS(1).GT.0.35d0.AND.MASS(1).LT.1.25d0) THEN
...
* Alternatively use condition of Hjellming & Webbink, 1987, ApJ, 318, 794.
      ...
      ENDIF
*
      IF(KSTAR(J1).EQ.0.AND.Q(1).GT.0.695d0)THEN
*
...
* otherwise it will become younger still.
            IF(MASS(2).LT.0.35d0.OR.MASS(2).GT.1.25d0)THEN
...
*           jkick = 0
         ...
         EPOCH(J2) = TPHYS0 - AJ(2)
*
      ELSEIF((KSTAR(J1).GE.10).AND.(KSTAR(J1).LE.12)
     &  .AND.(Q(1).GE.0.628d0))THEN
*
...
* RSp updated output July 2020
            IF (rank.eq.0.and.DJGR.GT.1.d-06)
...
*
     ...
     &             TSCLS,LUMS,GB,ZPARS)
         ...
         ELSEIF((MASS(2).LT.0.35d0.OR.MASS(2).GT.1.25d0).AND.
     &      KSTAR(J2).NE.7)THEN
...
*       unless both stars have zero MASS.
          II = J2
          K = 2
          IF((MASS(1)<1.D-14).AND.(MASS(2)>1.D-14))THEN
             II = J1
             K = 1
          ENDIF
          BODY0(II) = 0.D0
...
*     Impose 2.5 % limit on change of JORB (Chris Tout 1/2013).
               IF (ABS(DJORB).GT.0.1d0*JORB.AND.ITRO.LE.5) THEN
                   DTM0 = 0.1d0*JORB*DTM0/ABS(DJORB)
                   DELET = 0.1d0*JORB*DELET/ABS(DJORB)
                   ITRO = ITRO + 1
...
*       Form new star by combining the components inelastically.
         ...
         END IF
*
         IF (IWARN.LT.100.AND.RAD(1).GT.2.d0*ROL(1)) THEN
            IWARN = IWARN + 1
            if(rank.eq.0)
...
*       Produce diagnostics for cases involving degenerate objects.
     ...
     &                           DM1/DTM/1.0d+06, DM2/DTM/1.0d+06
...
*         DTMAX = MIN(DTMAX,STEPX)
          ...
          TEV(J1) = TEV(I) + 2.d0*STEPX + STEP(I1)
          TEV(J2) = TEV(J1)
          TMDOT = MIN(TMDOT,TEV(I))
          Z = (1.d0 - R(IPAIR)/SEMI)**2 + TDOT2(IPAIR)**2/(BODY(I)*SEMI)
          ECC = SQRT(Z)
...
*       Include consistency check on mass difference.
      ...
      IF(ABS(DM).GT.0.0001d0*BODY(I))THEN
          if(rank.eq.0)
