...
*        add into ghost list
            ...
            DO 12 K = 1,3
               X0(K,I) = MIN(1.0d+04+X(K,I),1000.d0*RSCALE*X(K,I)/RI)
               X(K,I) = X0(K,I)
               X0DOT(K,I) = SQRT(0.004d0*ZMASS/RSCALE)*XDOT(K,I)/VI
               XDOT(K,I) = X0DOT(K,I)
               F(K,I) = 0.0D0
               FDOT(K,I) = 0.0D0
               D2(K,I) = 0.0D0
...
*       Avoid negative radial velocity at pericentre (bug fix 12/08).
              IF (TDOT2(IPAIR).LT.0.0D0) TDOT2(IPAIR) = 0.d0
*
