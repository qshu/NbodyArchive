...
*       Reduce chain if close binary is hierarchical and at beginning or end.
                  IF (1.d0/RINV(JM).GT.50.d0*SEMI) THEN
                      IESC = INAME(IM)
                      JESC = INAME(IM+1)
                  END IF
...
*       Identify index of the maximum end separation.
                  IF (1.d0/RINV(JM).LT.1.d0/RINV(N-1)) JM = N - 1
                  IF (1.d0/RINV(JM).GT.50.d0*SEMI) THEN
                      IESC = INAME(JM)
                  END IF
              END IF   
...
*       Employ parabolic escape criterion (reduce if RI > RSUM/2 & RDOT > 0).
              IF (RI.GT.RESC.AND.RDOT.GT.0.d0) THEN
                  IF ((RDOT**2).LT.(2.d0*MASS/RI)) THEN
                      IESC = 0
                  END IF
              ELSE
                  IESC = 0
...
*       Check approximate escape criterion outside RESC (allow RX > 2*RESC).
              IF (RM.GT.RESC.AND.RDOT.GT.0.d0) THEN
                  IF ((RDOT**2).LT.(2.d0*MASS/RI)) THEN
                      ER = 0.5d0*RDOT**2 - MASS/RI
                      RX = -MASS/ER
                      IF ((ER.LT.0.d0).AND.(RX.LT.2.d0*RESC)) THEN
                          IESC = 0
                      END IF
                  END IF
              ELSE
