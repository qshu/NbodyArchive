...
*       Determine the smallest two-body time-scale from parabolic orbit.
      ...
      I2 = IJ(2)
      RM = SQRT(R2(I1,I2))
      VP2 = 2.d0*(M(I1) + M(I2))/RM
      TP = RM/SQRT(VP2)
      TSTAR = MIN(TP,TCR)
      STEPIN = EPS**0.2d0*TSTAR*ALAG
...
*       Set pericentre search distance (twice 4*r_max; discrete intervals).
      RSTAR = 8.d0*RSTAR  ! no longer used.
      KSLOW = .false.
      KCOLL = .false.
      EJUMP = 0.0D0
      stopB = .false.
...
*
   ...
   15     FORMAT (' NEW CHAIN   NCH NP ENERGY RSUM RGRAV TCR RMAXS ',
     &         'NAME(1-NM) [ALL in NB UNIT]',2I4,F10.5,1P,4E9.1,0P,6I10)
          call flush(6)
      END IF
      IF (RSUM.LT.1.0d-05) EPS = 1.0D-12
*
...
*       Ensure termination if only two particles left or positive energy.
          IF (N.EQ.2.OR.ENERGY.GT.0.0d0) GO TO 70
      END IF
*
...
*       Set slightly large interval to avoid small steps and check STEPS.
          DT = 1.01d0*TMAX - CHTIME
          IF (DT.LT.0.d0) DT = ABS(DT)
          DT = MIN(1.01d0*STEPS(ISUB),DT)
...
*       Increase step by 10% during expansion and check convergence value.
          IF (TPR.GT.TPR0) DTAU = 1.1d0*DTAU
...
*       Check time-step rejuvenation during standard integration.
      IF (MOD(NSTEP1,1000).EQ.0.AND.ICALL.EQ.0.AND.ICOLL.EQ.0) THEN
          CALL CONST(X,V,M,N,ENER0,G0,ALAG)
          SMALL = 1.0D-04*EPS**0.2d0*TSTAR*ALAG
...
*
      IF (STEP.GT.10.d0*STEP0.AND.ICOLL.EQ.0) THEN
          STEP = 10.d0*ABS(STEP0)
      ELSE IF (ABS(STEP)<1.D-14) THEN
...
*       Check for capture, circularization or collision.
      IF (QPERI.LT.4.d0*MAX(SIZE(K1),SIZE(K2))) THEN
*
...
*       Avoid tidal dissipation for circularized orbit.
          ICIRC = 0
          IF (KZ27.GT.0.AND.EBS.LT.0.d0) THEN
              SEMI = -0.5d0*M(K1)*M(K2)/EBS
              ECC = 1.d0 - QPERI/SEMI
              IF (ECC.GT.0.0022d0) ICIRC = 1
          END IF
*
*       Adopt collision criterion of Kochanek (Ap.J. 385, 684, 1992)
          IF (KZ27.LE.2) THEN
              FAC = 0.5d0*(M(J1) + M(J2))/M(J1)
              RCR = 1.7d0*FAC**0.3333d0*SIZE(J1)
          ELSE
              CLIGHT = 3.0D+05/VSTAR1
              RCR = 6.d0*(M(J1) + M(J2))/CLIGHT**2
          END IF
*
...
*       Check termination or strong perturbation (T > TMAX or GPERT > 0.01).
      IF (CHTIME.GT.TMAX.OR.GPERT.GT.0.01d0) THEN
          CALL YSAVE(Y)
          CALL TRANSX
          ECH = ENERGY
          TIMEC = CHTIME
...
*         IF (rank.eq.0.and.KZ30.GT.2) THEN
          ...
          RDOTV(K) = 0.D0
          DO 306 KK = 1,3
          RI2(K) = RI2(K) + XCH(KK,K)**2
          VI2(K) = VI2(K) + VCH(KK,K)**2
          IF(K.LT.N)THEN
              RSEP2(K) = RSEP2(K) + (XCH(KK,K+1)-XCH(KK,K))**2
              VSEP2(K) = VSEP2(K) + (VCH(KK,K+1)-VCH(KK,K))**2
              RDOTV(K) = RDOTV(K) + 
     &               (XCH(KK,K+1)-XCH(KK,K))*(VCH(KK,K+1)-VCH(KK,K))
          END IF
  306     CONTINUE
          RI(K) = SQRT(RI2(K))
          VI(K) = SQRT(VI2(K))
          IF(K.LT.N)THEN
              ...
              RSEP(K) = SQRT(RSEP2(K))
              VSEP(K) = SQRT(VSEP2(K))
              SEMICH(K) = 1.D0/(2.0D0/RSEP(K) - VSEP2(K)/XMCH)
              ECCCH(K) = SQRT((1.0D0 - RSEP(K)/SEMICH(K))**2 + 
...
*   Bound Orbit
         IF(SEMICH(K).GT.0.D0)THEN
              ...
              A_EIN(K) = 3.d0*TWOPI*XMCH/(SEMICH(K)*CL2*(1.D0-ECC2))
     ...
     &                   SEMICH(K)**4/FE1*FE2
         ...
         GE3 = (ECC2 - 1.D0)**0.5D0*(301.D0/144.D0 + 673.D0*ECC2/288.D0)
              GE4 = DACOS(-1.D0/ECCCH(K))*2.D0/TWOPI
              GEE = GE4*GE2+GE3
         A_EIN(K) = 3.d0*TWOPI*XMCH/(DABS(SEMICH(K))*CL2*(ECC2-1.D0))
              DLGADT = 32.D0/5.D0/CL5*M(K)*M(K+1)*XMCH/
     &                   SEMICH(K)**4/GE1*GEE
              TGRCH(K) = 1.D0/DLGADT
         END IF
...
*       Terminate on large step number and tiny STEP > 0.
          IF (NSTEP1.GT.100000.AND.STEP.LT.SMALL.AND.STEP.GT.0.d0) THEN
              if(rank.eq.0)
     &        WRITE (6,58)  NSTEP1, N, STEP, (1.d0/RINV(K),K=1,N-1)
...
*       Check hierarchical stability condition for triple or quad.
   60 IF (N.EQ.3) THEN
          IF (RSUM.GT.4.d0*RM) THEN
              CALL CHSTAB(ITERM)
              IF (ITERM.LT.0) GO TO 70
          END IF
      ELSE IF (N.EQ.4) THEN
          IF (RM.LT.0.1d0*RSUM) THEN
...
*       Activate termination index for routine INTGRT.
      ITERM = -1
*
      IF (rank.eq.0.and.KZ30.GT.1.AND.QPERI.LT.1.d0) THEN
