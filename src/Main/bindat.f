...
*       Determine merger & ghost index for negative c.m. name (skip ghost).
          IF (NAME(ICM).LT.0.AND.BODY(ICM).GT.0.d0) THEN
...
*       Include separate diagnostics for the hierarchy (inner comps J1 & J).
     ...
     &                                 TDOT2(JPAIR)**2/(BODY(ICM)*SEMI1)
              ...
              E1 = SQRT(ECC1)
              IF (J.LT.0) J = J1
              RM = SEMI*(1.d0 - E0)/MAX(RADIUS(J1),RADIUS(J),1.0D-20)
C              RM = MIN(RM,99.9D0)
...
c$$$    9     CONTINUE
          ECM(JPAIR) = 0.5d0*VJ2 - phidbl(ICM)
...
*       Copy relevant binary diagnostics to single precision.
      ...
      AS(4) = RC
      AS(5) = TPHYS
      AS(6) = -1.5d0*(TIDAL(1)*ZMASS**2)**0.3333d0
      AS(7) = 0.d0
      DO 20 K = 1,10
          AS(K+7) = E(K)
   20 CONTINUE
...
*
      DO 50 JPAIR = 1,NPAIRS
          ...
          END IF
          ICM = N + JPAIR
          SEMI = -0.5d0*(BODY(J1) + BODY(J2))/H(JPAIR)
...
*       Include fast skips on each dimension to reduce the effort.
 ...
 70      CONTINUE
         ...
         SEMI = 2.d0/RIJ - VREL2/ZMB
         SEMI = 1.d0/SEMI
         IF (SEMI.GT.0.d0.AND.SEMI.LT.RB1) THEN
*     Exclude duplicates by examining current list of NEWI components.
 ...
 72         CONTINUE
            ...
            JLIST(NEWI+2) = JMIN
            NEWI = NEWI + 2
            ECC2 = (1.d0 - RIJ/SEMI)**2 + RDOT**2/(ZMB*SEMI)
            ECC1 = SQRT(ECC2)
