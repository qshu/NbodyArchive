...
*       Include test on fast escaper approaching a perturber having RDOT > 0.
      IF (GPERT.GT.0.05d0.AND.RDOT.GT.0) THEN
*       Bypass test and repeated diagnostics for large perturbation.
          IF (GPERT.GT.0.5d0) GO TO 5
          RDX = 0.d0
...
*       See whether the intruder is approaching a chain member.
             IF (RDI.LT.0.d0.AND.RJX2.LT.RJX**2) THEN
                RJX = SQRT(RJX2)
                RDX = RDI/RJX
             END IF
 3        CONTINUE
...
*       Bypass RDOT < 0 test for approaching ejection candidate.
          IF ((RDX.LT.0.d0).AND.(RJX.LT.2.d0*RSUM/DBLE(NN-1))) THEN
              if(rank.eq.0)
...
*       Include conditions for skipping (large RIJ & size or small GPERT).
      IF (RIJ.GT.3.d0*MIN(RSUM,RMIN)) GO TO 10
      IF (RDOT.GT.0.d0.AND.GPERT.LT.0.05d0) GO TO 10
      IF (RSUM.GT.5.d0*RMIN.AND.GPERT.LT.1.d0) GO TO 10
...
*       Perform impact parameter test: a*(1 - e) < RSUM.
     ...
     &      (XDOT(3,ICH) - XDOT(3,JCLOSE))**2
      AINV = 2.d0/RIJ - VR2/(MASS + BODY(JCLOSE))
      ECC2 = (1.d0 - RIJ*AINV)**2 + RDOT**2*AINV/(MASS + BODY(JCLOSE))
      ECC = SQRT(ECC2)
      SEMI1 = 1.d0/AINV
      PMIN = SEMI1*(1.d0 - ECC)
*       Widen the impact parameter test to be on safe side.
      IF (PMIN.GT.1.5d0*RSUM.AND.GPERT.LT.0.05d0) GO TO 10
*
*       Check option for increasing or decreasing regularization parameters.
      IF (KZ(16).GT.2) THEN
          IF (GPERT.GT.0.05d0.AND.NCH.LE.4.AND.KSMAG.LE.5) THEN
              KSMAG = KSMAG + 1
              RMIN = 1.2d0*RMIN
              RMIN2 = RMIN**2
              DTMIN = 1.3d0*DTMIN
  ...
  700             FORMAT (' INCREASE   Time[NB] KSMAG GPERT RMIN ',
     &                 'RIJ[NB] ',1P,E15.6,0P,I4,1P,3E15.6)
                  CALL FLUSH(77)
              END IF
          ELSE IF (GPERT.LT.0.001d0.AND.KSMAG.GT.1) THEN
              KSMAX = KSMAG - 1
              RMIN = 0.83d0*RMIN
              RMIN2 = RMIN**2
              DTMIN = 0.77d0*DTMIN
...
*       Include additional criteria (V^2 > VC^2/2; JCL > N & RIJ < RSUM).
     ...
     &    ((RIJ.LT.2.d0*RMIN).AND.(PMIN.LT.RMIN)).OR.
     &    ((RIJ.LT.RSUM).AND.((RDOT**2).GT.(0.5d0*VC2))).OR.
     &    ((JCLOSE.GT.N).AND.(RIJ.LT.RSUM)).OR.
     &    ((GPERT.GT.0.2d0).AND.(RDOT.LT.0.d0))) THEN
*
...
*       Include possible return after ejection in bound orbit.
          ELSE IF (RDOT.GT.0.d0) THEN
              GO TO 10
          ELSE IF (NN.GT.4.AND.JCLOSE.GT.N.AND.RSUM.GT.2.d0*RMIN) THEN
              GO TO 10
          END IF
*
          RSUM = RSUM + RIJ
          IF (KZ(30).GT.1) THEN
              IF (JCLOSE.GT.N) THEN
                  SEMI = -0.5d0*BODY(JCLOSE)/H(JCLOSE-N)
              ELSE
                  SEMI = 0.d0
              END IF
              if(rank.eq.0)
...
*       Treat extreme binary as inert on significant slow-down.
          IF (JCLOSE.GT.N.AND.FAC.GT.10.d0) THEN
              SEMI = -0.5d0*BODY(JCLOSE)/H(JCLOSE-N)
              RM = 1.d0
              DO 8 K = 1,NCH-1
                  RM = MIN(1.d0/RINV(K),RM)
    8         CONTINUE
              SMALL = 0.01d0*(RSUM - RIJ)
              IF (rank.eq.0.and.RM.LT.SMALL.AND.SEMI.LT.SMALL) THEN
                  WRITE (6,9)  NAME(JCLOSE), GPERT, SEMI, RIJ,
     &                                       (1.d0/RINV(K),K=1,NCH-1)
...
c$$$                call flush(6)
c$$$                IDTTB = INT(DTTB)
c$$$                DITTB = DTTB - DBLE(IDTTB)
c$$$                call STEPK(DITTB,DTTBN)
c$$$                DTTBN = DTTBN + DBLE(IDTTB)
c$$$             else 
...
*       Try escape check if middle distance is largest.
     ...
     &    1.d0/RINV(ISORT(1)).GT.2.d0*RMIN) THEN
          R1 = 1.d0/RINV(1)
          R2 = 1.d0/RINV(NN-1)
...
*       Ensure RSUM > 0.1*RMIN for reducing differential force corrections.
     ...
     &                    RSUM.GT.0.1d0*RMIN) THEN
          ...
          ELSE
              IF (HI.GT.0.d0) THEN
                  VINF = SQRT(2.d0*HI)*VSTAR
              ELSE
                  VINF = 0.d0
              END IF
              IF (KZ(30).GT.1.OR.VINF.GT.1.d0) THEN
                  if(rank.eq.0)
     &            WRITE (6,28)  IESC, JESC, NAMEC(IESC), NAMEC(JESC),
     &                          RI, RDOT**2, 2.d0*BODY(ICH)/RI, RB, VINF
...
*       Note that arithmetic mean tends to delay escape.
      RESC = MAX(5.d0*RGRAV,DESC)
      RESC = MIN(RESC,3.d0*RMIN)
      IF ((NN.EQ.3).AND.(RESC.LT.0.001d0*RMIN)) RESC = 5.d0*RESC
      IF (RM.GT.RESC.AND.RDOT.GT.0.d0) THEN
          IF ((RDOT**2).LT.(2.d0*BODY(ICH)/RI)) THEN
*     --10/29/13 13:53-lwang-bug-fix------------------------------------*
***** Note: BODY(IESC) here is incorrect-------------------------------**
C*              FAC2 = 2.d0*BODY(IESC)/(BODY(ICH) - BODYC(IESC))
             FAC2 = 2.d0*BODYC(IESC)/(BODY(ICH) - BODYC(IESC))
*     --10/29/13 13:53-lwang-end----------------------------------------*
              GI = FAC2*((RSUM - 1.d0/RINV(IM))/RI)**3
*       Do not permit large length contrast even for bound orbit (NN = 4).
              IF (NN.EQ.4.AND.GI.LT.0.01d0) THEN
                  IB = ISORT(NN-1)
*       Prevent NN = 4 termination near small pericentre (subject to safety).
                  IF (((1.d0/RINV(IB)).GT.0.1d0*RGRAV)
     &              .OR.(RSUM.GT.RMIN)) THEN
                      KCASE = -1
                      GO TO 50
                  ELSE
                      KCASE = 0
                  ...
                  END IF
              END IF
              ER = 0.5d0*RDOT**2 - BODY(ICH)/RI
              RX = -BODY(ICH)/ER
              IF ((ER.LT.0.d0.AND.RX.LT.MIN(2.d0*RSUM,2.d0*RMIN)).OR.
*     --10/29/13 14:09-lwang-bug-fix------------------------------------*
***** Note:update in 10/26/13 in Sverre's Email------------------------**
C*     &            (NN.EQ.3.AND.GI.GT.0.1)) THEN
     &             (RSUM.LT.5.d0*RMIN.AND.GPERT.LT.0.01d0)) THEN
*     --10/29/13 14:10-lwang-end----------------------------------------*
                  KCASE = 0
                  GO TO 60
              END IF
              IF (RI.GT.MIN(0.5d0*RMAXS(ISUB),RMIN)) THEN
...
*       Write E = E_b + E_1 with E_b = -m1*m2/2*a and E_1 = MU3*ER.
                      SEMI = 0.5d0*RGRAV/
     &                             (1.d0 + ZF*(1.d0 + RGRAV*ER/MASS))
                      RY = 1.d0/RINV(IB)
                      IF (RY.LT.0.9d0*SEMI.AND.RI.LT.2.d0*RMIN) THEN
                          if(rank.eq.0)
     &                    WRITE (6,31)  NSTEP1, RY/SEMI, RI, RDOT**2,
     &                                  2.d0*BODY(ICH)/RI, SEMI
                          KCASE = 0
                          GO TO 60
                      END IF
                      KCASE = -1
...
*       Include three-body stability test for distant fourth body.
              IF (RBIG.GT.0.75d0*RSUM.AND.NN.EQ.4.AND.
     &            RSUM.GT.0.5d0*RMIN) THEN
                  CALL CHSTAB(ITERM)
                  IF (ITERM.LT.0) THEN
                      KCASE = -1
                      GO TO 60
...
*       Obtain semi-major axis from ENERGY = -(m1*m2 + (m1+m2)*m3)/RGRAV.
              ...
              ER = 0.5d0*RDOT**2 - BODY(ICH)/RI
*       Write E = E_b + E_1 with E_b = -m1*m2/2*a and E_1 = MU3*ER.
              SEMI = 0.5d0*RGRAV/(1.d0 + ZF*(1.d0 + RGRAV*ER/MASS))
              RY = 1.d0/RINV(IB)
*       Note RGRAV = MIN(0.5*RSUM,RGRAV) is OK and RGRAV adjusted on QPMOD.
              IF (RY.LT.0.9d0*SEMI.AND.RI.LT.2.d0*RMIN.AND.
     &            RI.LT.50.d0*SEMI) THEN
                  if(rank.eq.0)
     &            WRITE (6,31)  NSTEP1, RY/SEMI, RI, RDOT**2,
     &                          2.d0*BODY(ICH)/RI, SEMI
...
*       Check that escaper is well separated (ratio > 3).
          RR = RSUM - 1.d0/RINV(IM)
          RATIO = 1.d0/(RINV(IM)*RR)
          IF ((RATIO.LT.3.d0).AND.(RSUM.LT.RMIN)) THEN
              KCASE = 0
              GO TO 60
          END IF
*
   32     HI = 0.5d0*RDOT**2 - BODY(ICH)/RI
          IF (HI.GT.0.d0) THEN
              VINF = SQRT(2.d0*HI)*VSTAR
          ELSE
              VINF = 0.d0
          END IF
          IF (KZ(30).GT.1.OR.VINF.GT.2.d0) THEN
              if(rank.eq.0)
     &        WRITE (6,35)  IESC, NAMEC(IESC), RI, RDOT**2,
     &                      2.d0*BODY(ICH)/RI, VINF
