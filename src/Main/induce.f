...
***   ANGLE = FAC*360.d0/TWOPI
      IN = 1 + FAC*360.d0/(TWOPI*22.5d0)
*
*       Exit on hyperbolic orbit or outer eccentricity near 1.
      IF ((SEMI1.LT.0.d0).OR.(ECC1.GT.0.9999d0)) THEN
          EMAX = 0.d0
          TC2 = 999.d0
          TG = 1.0d+04
          EDAV = 1.0d+04
...
*       Set impact parameters and estimate eccentricity growth time-scale.
      ...
      TG = TK1**2*ZMB*(1.d0 - ECC1**2)**1.5d0/(BODY(JCL)*TK)
*
      CONST = PFAC(A,Z)
...
*       Determine circularization time for current & smallest pericentre.
      TC2 = 2000.d0
      TC = 2000.d0
      IF ((PMIN2.LT.10.d0*MAX(RADIUS(I1),RADIUS(I2))).AND.
     &    (KZ(27).EQ.2).AND.(ECC.GT.0.002d0)) THEN
...
*       Note: only use PMIN call because ICIRC may become >0 after first!
      ...
      ELSE IF (ECC.LT.0.002d0) THEN
          TC = 0.d0
          TC2 = 0.d0
      END IF
*
*       Print diagnostics for high inclinations and TC2 < 10**7 yrs.
      IF ((IN.GE.4).AND.(IN.LE.5).AND.(TC2.LT.10.d0).AND.
     &    (KSTAR(I).NE.-2).AND.EI2.GT.4.0D-06) THEN
