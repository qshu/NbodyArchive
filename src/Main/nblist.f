...
*       Note possible large RS0 set in routine MERGE at later times.
          IF (ABS(TIME)<1.D-14) THEN
              RS2 = RS2*(1.d0 + RI2)
          ELSE
              RSX = 0.3d0*(1000.d0/DBLE(N - NPAIRS))**0.3333d0
              RS2 = (RSX*RSCALE)**2*(1.d0 + RI2)
          END IF
      END IF
*
      VI2 = XDOT(1,I)**2 + XDOT(2,I)**2 + XDOT(3,I)**2
      IF (VI2.GT.0.d0) THEN
          DTR = 0.1d0*RS0/SQRT(VI2)
          VRFAC = -0.1d0*RS2/DTR
...
*       Include both components in case of primordial binaries.
    6     IF ((J.LE.2*NBIN0).AND.(ABS(TIME)<1.D-14)) THEN
              JPAIR = KVEC(J)
...
*       Reduce neighbour sphere but avoid possible looping.
          A1 = ZNBMAX/DBLE(NNB)
          IF (ABS(A1 - 0.5d0).LT.0.05d0) A1 = 1.2d0*A1
          RS2 = RS2*A1**0.66667d0
          NBFULL = NBFULL + 1
          GO TO 1
      END IF
*
