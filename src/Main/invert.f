...
*       Form semi-major axes for the two closest distances.
      ...
      do i = 1,n-1
          ...
          if (i.eq.i1) then
              ...
              mb = mc(i) + mc(i+1)
              amax = 2.d0*rinv(i) - w2/mb
              a1 = 1.d0/amax
              V21 = w2
*       Obtain the largest relative perturbation from nearest neighbour.
              if (i.eq.1) then
                  g1 = 2.d0*mc(3)/mb*(RINV(2)/RINV(1))**3
              else
                  g1 = 2.d0*mc(i-1)/mb*(RINV(i-1)/RINV(i))**3
                  if (i.lt.n-1) then
                      g1b = 2.d0*mc(i+2)/mb*(RINV(i+1)/RINV(i))**3
                      g1 = max(g1b,g1)
                  end if
              end if
*       Treat the second smallest distance similarly.
              ...
              dm = rinv(i)
              if (i.eq.1) then
                  g2 = 2.d0*mc(3)/mb*(RINV(2)/RINV(1))**3
              else
                  g2 = 2.d0*mc(i-1)/mb*(RINV(i-1)/RINV(i))**3
                  if (i.lt.n-1) then
                      g2b = 2.d0*mc(i+2)/mb*(RINV(i+1)/RINV(i))**3
                      g2 = max(g2b,g2)
                  end if
              end if
          end if
...
*       Obtain reliable semi-major axis for small pericentre or large EB.
      EB = -0.5d0*mc(i1)*mc(i1+1)/a1
      EB1 = EB/ENERGY
      IF (rm.lt.0.2d0*a1.or.EB1.gt.0.9d0) then
...
*
          ERR = (a1 - SEMI)/SEMI
          IF (ABS(ERR).GT.1.0d-05.AND.IWARN.LT.10) THEN
              IWARN = IWARN + 1
              ZK = 1.d0
              DO I = 1,N-1
                  ZK = MAX(KSCH(I),ZK)
...
*       Restrict the interval to a fractional period or crossing time.
      IF (a1.GT.0.d0) THEN
          TKS = 6.28d0*KSCH(i1)*a1*SQRT(a1/(mc(i1) + mc(i1+1)))
...
*       Check second closest pair if bound (small period; a1 < 0 possible).
      if (a2.gt.0.d0) then
          TKS = 6.28d0*KSCH(i2)*a2*SQRT(a2/(mc(i2) + mc(i2+1)))
          DT = MIN(0.5d0*TKS,DT)
      end if
*
...
*       Sum the constant part of the integral.
      SUM1 = SUM1 - 2.d0*MC(i1)*MC(I1+1)/(KSCH(i1)*RM)
...
*       Consider the second close interaction for small perturbation.
      IF (i2.ne.i1.and.g2.lt.0.01d0) THEN
          i1 = i2
          a1 = a2
          rm = dm
          GO TO 5
