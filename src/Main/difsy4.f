...
*
      ...
      YA(I)=Y(I)
      END DO
      CALL DERQP4(Y(1),Y(13),DZ(1),DZ(13))
      IF (JC.GT.0)H=DSC
   10 XN=X+H
      BO=.FALSE.
      M=1
      ...
      END DO
      DO K=1,M
      CALL DERQP4(YM(1),YM(13),DY(1),DY(13))
      DO I=1,N
      U=YL(I)+B*DY(I)
      YL(I)=YM(I)
      YM(I)=U
...
*       Use modified EPS for convergence test.
      ...
      Y(I)=TA
      END DO
      IF(ABS(FS)>1.D-14)THEN
      FA=FY
      K=L-1
      FY=(EP(K)/FS)**(1./(L+K))
      FA7=0.7d0*FA
      IF(L.EQ.2)FA7=0.d0
      FYBAD=.NOT.((FA7.GT.FY).OR.(FY.GT.0.7d0))
      IF(FYBAD)THEN
      H=H*FY
      JTI=JTI+1
      IF(JTI.GT.5)THEN
