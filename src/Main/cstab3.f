...
*       Evaluate orbital elements for inner and outer motion.
      ...
      SEMI1 = 2.d0/R3 - VREL21/MB1
      SEMI1 = 1.d0/SEMI1
      ECC1 = SQRT((1.0D0 - R3/SEMI1)**2 + RDOT3**2/(SEMI1*MB1))
      R4 = SQRT(R4)
      SEMI4 = 2.d0/R4 - V4/MB4
      SEMI4 = 1.d0/SEMI4
      ECC4 = SQRT((1.d0 - R4/SEMI4)**2 + RDOT4**2/(SEMI4*MB4))
*
...
*       Replace the EK criterion by the MA analytical stability formula.
      ...
      IF (ECC1.LT.1.d0) THEN
          XFAC = (1.d0 + Q0)*(1.d0 + ECC1)/SQRT(1.d0 - ECC1)
      ELSE
          XFAC = 40.d0*(1.d0 + Q0)
      END IF
      FE = 1.d0
      PCRIT = 2.8d0*FE*XFAC**0.4d0*SEMI
*
*       Obtain the inclination.
      CALL INCLIN(XX,VV,XCM,VCM,ALPHA)
*
*       Include fudge factor for inclination effect.
      YFAC = 1.d0 - 0.3d0*ALPHA/3.14d0
      PCRIT = YFAC*PCRIT
*
*       Check hierarchical stability condition for 3 + 1 configuration.
      ITERM = 0
      IF (PMIN.GT.PCRIT.AND.SEMI1.GT.0.d0.AND.SEMI4.GT.0.d0.AND.
     &    RB.GT.SEMI) THEN
          G4 = 2.d0*M(I4)/MB4*(R3/R4)**3
          PMIN4 = SEMI4*(1.d0 - ECC4)
          ZFAC = (1.d0 + M(I4)/MB1)*(1.d0 + ECC4)/SQRT(1.d0 - ECC4)
          ZCRIT = 2.8d0*FE*ZFAC**0.4d0*SEMI4
          PZ = PMIN4/ZCRIT
*
*       Ignore outermost body for perturbation test of inner triple.
          IF (N.EQ.5) THEN
              G4 = 2.d0*M(I4)/MB1*(R3/PMIN4)**3
              IF (G4.LT.0.1d0) THEN
                  if(rank.eq.0)
...
*       Continue chain integration if outer orbit unstable or large pert.
          IF (PMIN4.LT.ZCRIT.OR.G4.GT.0.2d0) GO TO 40
*
          ITERM = -1
          if(rank.eq.0)
   ...
   20     FORMAT (' CSTAB3    E =',F6.3,'  E1 =',F6.3,'  A =',1P,E8.1,
     ...
     &                   SEMI, SEMI1, PCRIT/PMIN, 180.d0*ALPHA/3.14d0
