...
*       Check for planetary systems - defined as HeWDs - and low-mass WDs!
      IF(K1.EQ.10.AND.M1.LT.0.05d0)THEN
         ICASE = K2
         IF(K2.LE.1)THEN
            ICASE = 1
            AGE1 = 0.D0
         ENDIF
      ELSEIF(K1.GE.11.AND.M1.LT.0.15d0.AND.ICASE.EQ.6)THEN
         ICASE = 1
      ENDIF
      IF(K2.EQ.10.AND.M2.LT.0.05d0)THEN
         ICASE = K1
         IF(K1.LE.1)THEN
            ICASE = 1
            AGE2 = 0.D0
...
*      Form binding energy of secondary and orbital kinetic energy.
      EB = -0.5d0*6.68D-08*4.0D+66*M2**2/(RADIUS(I2)*SU*7.0D+10)
      ZMU = SMU*BODY(I1)*BODY(I2)/(BODY(I1) + BODY(I2))
      ZKE = 0.5d0*2.0D+33*ZMU*1.0D+10*VIJ2*VSTAR**2
      IF (ZKE + EB.GT.0.d0.AND.RIJ.LT.0.5d0*(RADIUS(I1)+RADIUS(I2)).AND.
     &    MAX(K1,K2).LT.2) THEN
          ZM = ABS(ZKE/EB)
          FAC = 1.d0/SQRT(ZM)
...
*       Prepare spin diagnostics for fort.91 (rg2=0.1 is assumed).
          ...
          PD = MAX(PD,0.0D0)
          ECC2 = (1.d0 - RIJ/SEMI)**2 + RDOT**2/(SEMI*ZMB)
          ECC = SQRT(ECC2)
