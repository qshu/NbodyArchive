...
*     Remove tables and re-initialize on large epoch or standard binary.
         IF ((TIME - TOSC(IC).GT.1.d0).OR.(KSTAR(I).EQ.0)) THEN
            II = -I
            CALL SPIRAL(II)
            IC = 0
         END IF
...
*     Ensure next location contains zero core mass (avoids confusion).
         ...
         DO 5 K = 1,4
            EOSC(K,IC) = 0.0D0
 5       CONTINUE
         IF ((KZ(9).GE.2).AND.(H(IPAIR).LT.0.d0)) THEN
            CALL BINEV(IPAIR)
         END IF
...
*     Check termination for rare case of wide chaos (minimum 1000 calls).
      ...
      IF ((QP.GT.7.d0).AND.(NDEC.GT.1000)) THEN
...
*     Specify polytropic index for each star (n = 2, 3 or 3/2).
     ...
     &        KSTAR(IK).EQ.6.OR.KSTAR(IK).EQ.9) THEN
         ...
         ELSE
            ...
            W(J) = WW(IP+3)
         END IF
         ALF(K) = 2.d0*TDYN(K)/SQRT(W(K))
         ALF(J) = 3.d0*TDYN(K)/SQRT(W(J))
...
*     Save initial eccentricity, binding energy & J0 the first time.
      ...
      IF ((IOSC(IC).EQ.0).OR.(LIST(1,I1).GT.0)) THEN
         ...
         ECC0 = ECC
         EB0(IC) = ZMU*H(IPAIR)
         ZJ0(IC) = CJ*SQRT(QPERI*(1.d0 + ECC0))
         EDEC(IC) = 0.d0
         IOSC(IC) = 1
...
*     Print NEW CHAOS/CAPTURE the first time.
         IF (NDEC.EQ.0) THEN
            NCHA = NCHA + 1
            WHICH1 = ' CHAOS  '
            IF (SEMI.LT.0.d0) WHICH1 = ' CAPTURE'
...
*     Employ an emergency procedure for zero energy change (TIDES2 bug).
      IF ((ABS(DE2(1))<1.D-14).OR.(ABS(DE2(2))<1.D-14)) THEN
         EB = BODYM*H(IPAIR)
         DE2(1) = -0.001d0*EB*RAN2(IDUM)
         DE2(2) = -0.001d0*EB*RAN2(IDUM)
      END IF
*     
...
*     Adopt non-linear dissipation time scale of Kumar & Goodman 1995.
      ...
      TD(1) = (1.d0/QL(1) + 1.d0/QNL1)*TK
      TD(2) = (1.d0/QL(2) + 1.d0/QNL2)*TK
*     
      RM = MAX(RADIUS(I1),RADIUS(I2))
      IF ((NDEC.GT.10000).AND.(QPERI.GT.4.d0*RM)) THEN
         DO 18 K = 1,2
            DE2(K) = 1000.d0*DE2(K)
            DE3(K) = 1000.d0*DE3(K)
 18      CONTINUE
...
*     Sum old and new oscillation energies for all modes.
      ...
      DO 20 K = 1,2
         ...
         END IF
         EOSC(K,IC) = EOSC(K,IC) +
     &        2.d0*SQRT(EOSC(K,IC)*DE2(K))*COS(DELTA) + DE2(K)
         IF (IOSC(IC).EQ.-1) THEN
            IF (K.EQ.2) IOSC(IC) = 1
         ELSE
            DELTA = TWOPI*RAN2(IDUM1)
         END IF
         EOSC(J,IC) = EOSC(J,IC) +
     &        2.d0*SQRT(EOSC(J,IC)*DE3(K))*COS(DELTA) + DE3(K)
...
*     Activate spiral indicator and save time, pericentre & eccentricity.
            ...
            KICKS = 0
         ELSE
            EPS = (2.d0*ECRIT(IC) - EB0(IC) + EDEC(IC))/(ZMU*BODY(I))
            ECCM = (1.d0 + EPS*BR(IC))/(1.d0 - EPS*AR(IC))
...
*     Check for hierarchical configuration on first call.
 34   IF ((NDEC.LE.1).AND.(SEMI.GT.0.d0).AND.(SEMI.LT.2.d0*RMIN)) THEN
         NP1 = LIST(1,I1) + 1
         DO 39 L = 2,NP1
            J = LIST(L,I1)
            RIJ2 = 0.d0
...
*     Accept semi-major axis ratio below 25.
            IF (((1.d0/A1).GT.(0.04d0/SEMI)).AND.(IDIS.LE.0)) THEN
               RA = SEMI*(1.d0 + ECC)
               SR = PMIN/RA
               GA = 2.d0*BODY(J)*(RA/PMIN)**3/BODY(I)
*     Determine inclination (8 bins of 22.5 degrees).
               FAC = A1A2/SQRT(A12*A22)
               FAC = ACOS(FAC)
               IN = INT(1.d0 + FAC*360.d0/(TWOPI*22.5d0))
               if(rank.eq.0)
...
*     Skip output for unperturbed case (CALL KSTIDE from UNPERT).
         IF ((LIST(1,I1).GT.0).AND.(H(IPAIR).LT.0.d0)) THEN
            CALL BINEV(IPAIR)
         END IF
      END IF
*     
