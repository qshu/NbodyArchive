...
void GPUNB_devinit(int irank){
	...
	cudaGetDeviceCount(&numGPU);
        fprintf(stderr, " irank, rank, size, rank mod numGPU, numGPU %d %d %d %d %d\n",irank,rank,size,rank%numGPU,numGPU);
	assert(0 == size%numGPU);
		for(int i=0; i<numGPU; i++){
			devid[rank%numGPU] = rank%numGPU;
		}
    devinit = true;
    numGPU = 1;
}
...
void GPUNB_open(int nbmax,int irank){
	...
	int rank;int size;int nrGPU;
    MPI_Comm_size( MPI_COMM_WORLD, &size );
    MPI_Comm_rank( MPI_COMM_WORLD, &rank );

                cudaGetDeviceCount(&nrGPU);
		cudaSetDevice(devid[rank%nrGPU]);
    fprintf(stderr, " rank, rank mod nrGPU %d %d Set Dev %d\n", rank, rank%nrGPU, devid[rank%nrGPU]);
    char hostname[150];
    memset(hostname,0,150);
    gethostname(hostname,150);
    int len;char pbusid[13];
    len = 13;
      cudaDeviceGetPCIBusId ( pbusid, len, devid[rank%nrGPU] );
      cudaDeviceProp prop;
      cudaGetDeviceProperties(&prop, devid[rank%nrGPU]);
      fprintf(stderr, "# GPU Set Device - rank, rank mod nrGPU: %d %d; HOST %s; NGPU %d; device: %d %s Bus Id %s \n", rank, rank%nrGPU, hostname, nrGPU, devid[rank%nrGPU], prop.name, pbusid);
                        int tid = 0;
