...
*     Include step reduction for large de/dt or primary spin rate.
      IT = 0
 1    IF (ABS(udot(1))*dtau.GT.0.01d0*MAX(es0,0.01d0)) THEN
         dtau = 0.5D0*dtau
         nsteps = 2*nsteps
         IT = IT + 1
         IF (IT.LT.10) GO TO 1
    ...
    2 IF ((ABS(udot(2))*dtau.GT.0.01d0*u(2)).AND.(u(2).GT.0.1d0)) THEN
         dtau = 0.5D0*dtau
         nsteps = 2*nsteps
         IT = IT + 1
         IF (IT.LT.10) GO TO 2
...
*     Treat large eccentricity carefully for rapid spin change.
      IF ((es0.GT.0.9995d0).and.(ABS(udot(2))*dtau.GT.0.01d0*u(2))
     &     .AND.u(2).GT.0.1d0) THEN
         dtau = 0.5D0*dtau
         nsteps = 2*nsteps
         IT = IT + 1
      END IF
...
*     Note there are 4 calls to deriv2 when u(1) may go negative.
         IF ((u(1).lt.0.002d0).or.(u(1).gt.0.99999d0)) then
*     Enforce circularization by adopting e=0.00199 and copying spins.
            IF (u(1).LT.0.d0) WRITE (6,4) I, nsteps, u(1)
...
*     DATA IC /0/
      ...
      f2=1.d0+7.5d0*e2+5.625d0*e4+0.3125d0*e6
      f3=1.d0+3.75d0*e2+1.875d0*e4+0.078125d0*e6
      f4=1.d0+1.5d0*e2+0.125d0*e4
      f5=1.d0+3.d0*e2+0.375d0*e4

      semi=angmom0-rg2(1)*spin1-m21*r21**2*rg2(2)*spin2
      semi=(semi*(1.d0+m21)/m21/semi0**2)**2/fac
      oa = 1.d0/semi

      if((e.le.0.d0).or.(e.ge.1.d0).or.(oa.lt.0.d0))then
         udot(1) = 0.d0
         udot(2) = 0.d0
         udot(3) = 0.d0
      else
         udot(1)=-oa**8*(e/fac**6.5d0)*
     &        (C1*(f3-(11.d0/18.d0)*fac**1.5d0*f4*spin1/oa**1.5d0)+
     &        C2*(f3-(11.d0/18.d0)*fac**1.5d0*f4*spin2/oa**1.5d0))
         udot(2)=(oa/fac)**6*C3*(oa**1.5d0*f2-fac**1.5d0*f5*spin1)
         udot(3)=(oa/fac)**6*C4*(oa**1.5d0*f2-fac**1.5d0*f5*spin2)
      endif
      if ((e.lt.0.002d0).and.(semi.lt.0.0d0)) semi=semi1
