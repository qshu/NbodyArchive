...
*       Quit if active ROCHE or mass-losing star.
      ...
      IF(KSTARM(IM).EQ.11.OR.KSTARM(IM).EQ.13)THEN
          GO TO 30
      END IF
      IF (M1.GE.10.d0.OR.(KSTAR(I1).GT.1.AND.KSTAR(I1).LT.10))THEN
          GO TO 30
      END IF
      IF (M2.GE.10.d0.OR.(KSTAR(I2).GT.1.AND.KSTAR(I2).LT.10))THEN
          GO TO 30
      END IF
*
*       Skip braking if semi-major axis exceeds 10 solar radii.
      ITERM = 0
      ZMB = CM(1,IM) + CM(2,IM)
      SEMI = -0.5d0*ZMB/HM(IM)
      IF (SEMI*SU.GT.10.d0)THEN
          TGR = 1.0D+06
          TBR = 1.0D+06
          SEMI1 = SEMI
          GO TO 20
...
*       Obtain current separation, square velocity and t'' = 2*U*U'.
      ...
      DO 5 K = 1,4
          ...
          TD2 = TD2 + 2.d0*UM(K,IM)*UMDOT(K,IM)
    5 CONTINUE
*
*       Evaluate inner eccentricity and outer period.
      ECC2 = (1.d0 - RB/SEMI)**2 + TD2**2/(ZMB*SEMI)
      ECC = SQRT(ECC2)
      SEMI0 = -0.5d0*BODY(I)/H(IPAIR)
*
*       Identify the most magnetically active star (#J2).
      IF (RADIUS(I1).GE.RADIUS(I2)) THEN
          IF ((KSTAR(I1).EQ.1.AND.M1.GT.1.25d0).OR.
     &        (KSTAR(I1).EQ.0.AND.M1.LT.0.35d0)) THEN
              J2 = I2
          ELSE
              J2 = I1
          END IF
      ELSE
          IF ((KSTAR(I2).EQ.1.AND.M2.GT.1.25d0).OR.
     &        (KSTAR(I2).EQ.0.AND.M2.LT.0.35d0)) THEN
              J2 = I1
          ELSE
              J2 = I2
          END IF
...
*       Determine new semi-major axis from angular momentum relation.
      ...
      ADOT = 2.d0*SQRT(ACM/GMS)*ZJDOT/(1.989D+33*ZMBAR*ZMU)
*
...
*       Suppress magnetic braking for massive MS/low-mass or evolved star.
      IF (((M2.GT.1.25d0.OR.M2.LT.0.35d0).AND.KSTAR(J2).LE.2).OR.
     &      KSTAR(J2).GE.10) THEN
          ADOT = 0.d0
          TBR = 1.0D+06
      END IF
...
*       Include safety test on new semi-major axis.
      IF (ABS(SEMI1).LT.RADIUS(J2).OR.SEMI1.LT.0.d0) THEN
          SEMI1 = RADIUS(J2)
      END IF
*
*       Update binding energy.
      HI = HM(IM)
      HM(IM) = -0.5d0*ZMB/SEMI1
*
...
*       Specify KS coordinate & velocity scaling factors at general point.
      C2 = SQRT(SEMI1/SEMI)
      V2 = 0.5d0*(ZMB + HM(IM)*RB*(SEMI1/SEMI))
      C1 = SQRT(V2/V20)
*
