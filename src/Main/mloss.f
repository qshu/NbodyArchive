...
*       Save old velocity for FDOT correction.
    ...
    5 CONTINUE
*
      DE = 0.5d0*DM*(VI2- TIDAL(1)*X(1,IMAX)**2 - TIDAL(3)*X(3,IMAX)**2)
      A0 = 1.d0
      IF (ZMBAR*BMAX.LE.6.d0) GO TO 8
*
*       Assign a high velocity to neutron star (at least 4*rms velocity).
      A0 = 4.d0*SQRT(0.5D0*ZMASS/(RSCALE*VI2))
      IF (IMAX.GT.2*NPAIRS) GO TO 6
      A2 = 2.d0*(BODY(2*IPAIR-1) + BODY(2*IPAIR))/R(IPAIR)
...
*       Set option = 2 to skip next energy check (reduced by 1 in ADJUST).
      KZ(19) = 2
*
      IF (ZMBAR*BMAX.LE.6.d0) GO TO 50
*       Include correction to total energy from the increased velocity.
      DE = 0.5d0*BODY(IMAX)*VI2*(A0**2 - 1.d0)
      BE(3) = BE(3) + DE
      if(rank.eq.0)
