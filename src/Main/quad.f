...
*       Synchronize next time interval with c.m. step unless termination.
          ...
          IF (STEPS(ISUB).LE.0.0D0) DS = 0.01d0*DS
          GO TO 30
      END IF
*
...
*       Ensure that unperturbed boundary exceeds system size (+ 20 per cent).
      ...
      RMAX4 = MAX(RMAXS(ISUB),1.2d0*RMAX)
*
*       Specify local crossing time.
      TCR = (M(1) + M(2) + M(3) + M(4))**2.5d0/ABS(2.0D0*ENERGY)**1.5d0
*
...
*       Distinguish between tidal energy loss and collision (ICOLL holds IM).
      ...
      IF (QPERI.LT.4.d0*MAX(SIZE(K1),SIZE(K2))) THEN
          IF (QPERI.LT.0.75d0*(SIZE(K1) + SIZE(K2))) THEN
*
...
*       Estimate perturbations on smallest binary.
      GB = 2.d0*MB1/MB*(RB/RM)**3
      IF (EB1.LT.0.d0) GB = GB*M(IM)/MB1
      G4 = 2.d0*M(IMAX)/(MB + M(IM))*(RM/RMAX)**3
      DB = (EB - EB0)/EB0
...
*       Avoid new regularization of any binary near small pericentre.
      RATIO = RM/SEMI
      ISAVE = ITRY
   60 IF (RATIO.LT.5.d0.AND.ITRY.LE.5
     & .OR.RATIO.LT.3.d0.AND.ITRY.LE.10) THEN
          GO TO 30
      END IF
*
...
*       Form binary energy change (set in BBCOLL; routine START4).
      ...
      IF (EB1.GT.0.d0) CM(9) = CM(9) + EB1*ENERGY
*
