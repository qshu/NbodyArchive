...
*       Exclude unlikely stability configuration (ratio 1/4 - 4).
      ...
      DO 1 I = 1,N-2
          ...
          IF (RATIO.GT.0.25d0.AND.RATIO.LT.4.d0) THEN
              IR = IR + 1
          END IF
    1 CONTINUE
*
...
*
      ...
      IF (RATIO.GT.0.25d0.AND.RATIO.LT.4.d0) GO TO 40
*
...
*       Modify correction factor by widest binary (cf. routine IMPACT).
      IF (SEMI2.GT.0.d0) PCRIT = (1.d0 + 0.1d0*SEMI2/SEMI)*PCRIT
*
*       Check hierarchical stability condition for bound close pair.
     ...
     &    RB0.GT.SEMI0.AND.SEMI2.GT.0.d0) THEN
          ITERM = -1
          if(rank.eq.0)
