...
* Stop when 0.1% accuracy is achieved.
               IT = IT + 1
               IF(IT.EQ.25)THEN
                  IF(ABS(RR-RLS).GT.0.1d0)THEN
                     if(rank.eq.0)WRITE(38,*)' TRFLOW KW1: ',rr,rls,t,tm
                  ENDIF
               ENDIF
               IF(ABS(RR - RLS).LE.0.001d0*RR.OR.IT.GT.25) GOTO 200
               THIGH = T
               RHIGH = RR
            ENDIF
            GOTO 10
...
* then for the overflow time.
 ...
 30         IT = IT + 1
            DEL = RGBF(M1,LUM) - RLS
            IF(IT.EQ.25.AND.ABS(DEL).GT.0.1d0)THEN
               if(rank.eq.0)then
...
*
         LUM = (RLS/0.08D0)**(4.d0/3.d0)
         CM = 2.0D-03*M1**2.5d0/(2.D0 + M1**5)
         IF(CM*LUM.GE.100.D0)THEN
            DTR = 1.0D+10
            GOTO 210
         ENDIF
         RX = RZHEF(M1)
         RR = RX*(LUM/LUMS(2))**0.2d0 + 
...
* Stop when 0.1% accuracy is achieved.
               ...
               IF(IT.EQ.25)THEN
                  ...
                  ENDIF
               ENDIF
               IF((ABS(RR - RLS).LE.0.001d0*RR).OR.(IT.GT.25)) GOTO 95
               THIGH = LUM
               RHIGH = RR
            ENDIF
            GOTO 90
