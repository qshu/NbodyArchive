...
*       Check for unreasonable input parameters (initial & restart).
   ...
   10     FORMAT (/,5X,'FATAL ERROR!   BAD INPUT   N =',I6,
     &                                 ' NNBMAX =',I4' NNBOPT =',I4)
      ...
      END IF
*
      IF (ETAI.GT.0.08D0.OR.ETAR.GT.0.16D0) THEN
          if(rank.eq.0)
   ...
   20     FORMAT (/,5X,'FATAL ERROR!   BAD INPUT   ETAI =',F6.2,
     &                                                  '  ETAR =',F6.2)
      ...
      END IF
*
      IF (ETAU.GT.0.5D0.OR.GMIN.GT.0.0001D0.OR.GMAX.GT.0.10D0) THEN
          if(rank.eq.0)
...
*       Also check for zero or negative values.
      IF (N.LE.0.OR.NNBMAX.LE.0.OR.ETAI.LE.0.D0.OR.ETAR.LE.0.D0) THEN
          if(rank.eq.0)
   ...
   40     FORMAT (/,5X,'FATAL ERROR!   BAD INPUT   N =',I5,
     &                  '  NNBMAX =',I4,'  ETAI =',F6.2,'  ETAR =',F6.2)
      ...
      END IF
*
      IF (ETAU.LE.0.D0.OR.GMIN.LE.0.D0.OR.GMAX.LE.0.D0) THEN
          if(rank.eq.0)
     &    WRITE (6,30)  ETAU, GMIN, GMAX
          STOP
      END IF
*
      IF (DTADJ.LE.0.D0.OR.DELTAT.LE.0.D0.OR.QE.LE.0.D0) THEN
          if(rank.eq.0)
