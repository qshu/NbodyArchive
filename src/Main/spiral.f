...
*     Note: rg2 should really be given by k2 in HRDIAG. 
            rg2(k)= 0.1d0*(1.d0 - CM(K,IC)/BODY(IK))
         ELSE
            QL = 1.0D+04
            IP = 3
            ...
            Q(K) = QQ(IP)
            rg2(k)= 0.21d0
            IF (KSTAR(IK).LE.2.OR.KSTAR(IK).EQ.7) rg2(k) = 0.1d0
            IF (KSTAR(IK).EQ.4) rg2(k)= 0.1d0*(1.d0 - CM(K,IC)/BODY(IK))
         END IF
...
*     Convert from angular momentum to omega (denoted spin1 & spin2).
      ...
      ELSE 
         ...
         IF (MC1.LE.0.0D0.OR.MC1.GT.M0) THEN
            MC1 = 0.3d0 + 0.1d0*DBLE(KW - 3)
            IF(KW.EQ.9) MC1 = MIN(0.3D0,0.95d0*M0)
            CM(1,IC) = MC1/ZMBAR
         END IF
         ZDUM = 2.0D0
         RC1 = CORERD(KW,MC1,M0,ZDUM)/SU
         SPIN1 = SPIN(J1)/(rg2(1)*BODY(J1)*RADIUS(J1)**2 +
     &        0.21d0*MC1/SMU*RC1**2)
      END IF
     ...
     &           1P,E20.11,0P,2I12,I4,1P,4E14.5)
      ...
      ELSE
         ...
         IF (MC2.LE.1.0D-10.OR.MC2.GT.M0) THEN
            MC2 = 0.3d0 + 0.1d0*DBLE(KW - 3)
            IF(KW.EQ.9) MC2 = MIN(0.3D0,0.95d0*M0)
            CM(2,IC) = MC2/ZMBAR
         END IF
         ZDUM = 2.0D0
         RCS2 = CORERD(KW,MC2,M0,ZDUM)/SU
         spin2 = SPIN(J2)/(rg2(2)*BODY(J2)*RADIUS(J2)**2 +
     &        0.21d0*MC2/SMU*RCS2**2)
      END IF
*     
      IF (IONE.EQ.0) THEN
         IONE = IONE + 1
         IF (ABS(SPIN1).GT.0.D0.AND.ABS(SPIN2).GT.0.D0) THEN
            TS = 1.0D+06*365.d0*TWOPI*TSTAR
            if(rank.eq.0)
...
*     Evaluate damping coefficients (Mardling & SJA, M.N. 321, 398, 2001).
      cf = 54.d0*twopi/5.d0
      C1 = cf*(AT0(1)*(Q(1)/W(1))**2*(1.d0 + M21)*M21)/semi0**8
      C2 = cf*(AT0(2)*(Q(2)/W(2))**2*((1.d0 + M21)/M21**2)*R21**8)/
     &     semi0**8
      C3 = (cf/9.d0)*(AT0(1)*(Q(1)/W(1))**2*M21**2)/rg2(1)/semi0**6
      C4 = (cf/9.d0)*(AT0(2)*(Q(2)/W(2))**2/M21**2)*R21**6/rg2(2)/
     &     semi0**6
*     
...
*     Obtain dominant terms of the derivatives (cf. routine HUT/DERIV2).
      fac=1-es0**2
      udot1=-(es0/fac**6.5d0)*(C1 + C2)
      udot2=(1.d0/fac)**6*C3
      udot3=(1.d0/fac)**6*C4
*     
*     Choose the step from smallest time-scale (! time0 < time possible).
      taux = min(abs(1.d0/udot1),abs(1.d0/udot2),abs(1.d0/udot3))
      nstep = int(1.d0 + 100.d0*sqrt(ABS(time - time0)/taux))
      nstep = min(nstep,100)
*     Include extra steps for long intervals and/or AGB type 5 or 6.
      IF (TIME - TIME0.GT.0.1d0) nstep = nstep + 20
      IF (KSTAR(J1).EQ.5.OR.KSTAR(J1).EQ.6) nstep = nstep + 10
*     Evaluate the equilibrium angular velocity.
      e2 = es0**2
      f2 = ((0.3125d0*e2 + 5.625d0)*e2 + 7.5d0)*e2 + 1.d0
      f5 = (0.375d0*e2 + 3.d0)*e2 + 1.d0
      omeq = f2*meanmotion/(f5*fac**1.5d0)
*     Increase number of steps on slow primary rotation.
      IF (omeq - spin1.GT.0.2d0*omeq) nstep = nstep + 10
      dtau1 = ABS(time - time0)/DBLE(nstep)
*     
*     Check circularization time after merger or large interval.
      IF ((IPHASE.EQ.7).OR.(TIME - TIME0.GT.1.d0)) THEN
         ICIRC = -1
         A0 = -0.5d0*BODY(I)/H(IPAIR)
         QPERI = A0*(1.d0 - es0)
         CALL TCIRC(QPERI,es0,I1,I2,ICIRC,TC)
         DT = MIN(0.1D0*TC,1.0D0)
*     Ensure careful integration with DT=0.1*TC for rapid evolution.
         IF((TC.LT.10.d0).AND.((KSTAR(J1).GT.1).OR.(ECC.GT.0.1d0))) THEN
            nstep = int(500.d0*(1.d0 + 100.d0/TC))
...
*     Ensure that no overshooting takes place.
 ...
 14         FORMAT (' SPIN WARNING   T NM K* # E A S-1 ',
     &           F8.1,I7,I4,I6,F7.3,1P,2E10.1)
         ...
         IWARN = IWARN + 1
      END IF
      IF ((spin20.LT.1.d0).and.(spin2.gt.1.d0)) THEN
         IF (rank.eq.0.and.MOD(IWARN,100).EQ.0) THEN
            WRITE (66,14) TTOT, NAME(J2), KSTAR(J2), nstep, ecc,
     &           RP0*SU/(1.d0-es0), spin2-1.d0
            CALL FLUSH(66)
         END IF
         spin2 = 1.d0
         IWARN = IWARN + 1
...
*     Obtain the tidal contributions from integration.
      A0 = RP0/(1.d0 - ES0)
      DH = -0.5d0*BODY(I)*(1.d0/SEMI - 1.d0/A0)
*     
...
*     Ensure the dominant star has synchronous rotation.
         ...
         ELSE
            SPIN(J1) = (rg2(1)*BODY(J1)*RADIUS(J1)**2 +
     &           0.21d0*MC1/SMU*RC1**2)*meanmotion
         END IF
*     ks MPI communication SPIN
         call ksparmpi(K_store,K_real8,K_SPIN,J1,0,SPIN(J1))
...
*     Ensure apocentre or arbitrary phase is replaced by pericentre.
      ...
      IF ((R(IPAIR).GT.SEMI).OR.(ERR.GT.1.0D-05)) THEN
...
*     Scale KS variables to yield the prescribed elements (set TDOT2 >= 0).
      ...
      DO 18 K = 1,4
         ...
         TDOT2(IPAIR) = TDOT2(IPAIR) + 2.d0*U(K,IPAIR)*UDOT(K,IPAIR)
 18   CONTINUE
...
*     Rectify large eccentricity deviation from integrated value.
      ECC2 = (1.d0 - R(IPAIR)/SEMI)**2 + TDOT2(IPAIR)**2/(BODY(I)*SEMI)
      ECCF = SQRT(ECC2)
      IF (ABS(ECC - ECCF).GT.0.01d0) THEN
         CALL KSRECT(IPAIR)
      END IF
*     
...
*     Enforce termination at constant angular momentum on Roche condition.
      IF (DTR.LE.(0.1d0/TSTAR)) THEN
         AF = SEMI*(1.d0 - ECC**2)/(1.d0 - ECCM**2)
         H(IPAIR) = -0.5d0*BODY(I)/AF
         DETMP = ZMU*(HI - H(IPAIR))
         ECOLL = ECOLL + DETMP
         EGRAV = EGRAV + DETMP
         KSTAR(I) = 10
...
*     Ensure unperturbed motion for small GAMMA.
         IF ((GAMMA(IPAIR).LT.1.0D-10).AND.(ES0.LT.0.95D0)) THEN
            LIST(1,I1) = 0
         END IF
         IF ((GAMMA(IPAIR).LT.5.0D-07).AND.(ES0.LT.0.3d0)) THEN
            LIST(1,I1) = 0
         END IF
         CALL RESOLV(IPAIR,1)
         IMOD = KSLOW(IPAIR)
...
*     Define mass ratio and evaluate Roche radius for the primary.
         ...
         Q1 = Q0**0.3333d0
         Q2 = Q1**2
         RL1 = 0.49d0*Q2/(0.6d0*Q2 + LOG(1.0D0 + Q1))*SEMI
*     
...
*     Include occasional diagnostics of spiral evolution (every output).
      IF (ABS(TIME - TADJ).LT.STEP(I1)) THEN
         IF((RP1.LT.3.d0).AND.((ECC.LT.0.1d0).OR.(KSTAR(J1).GE.3))) THEN
            NP = LIST(1,I1)
            TK = DAYS*SEMI*SQRT(SEMI/BODY(I))
            if(rank.eq.0)
...
*     Include enforced circularization for difficult cases.
      IF (((ECC.LT.0.6d0).AND.(GAMMA(IPAIR).LT.2.0D-07)).OR.
     &     ((ECC.LT.0.8d0).AND.(GAMMA(IPAIR).LT.1.0D-08)).OR.
     &     ((ECC.LT.0.95d0).AND.(GAMMA(IPAIR).LT.1.0D-09))) THEN
         ICIRC = 0
         CALL TCIRC(QPERI,ECC,I1,I2,ICIRC,TC) 
         IF ((TC.GT.200.d0).OR.((TC.GT.10.d0).AND.(ECC.LT.0.7d0))) THEN
            CALL KSPERI(IPAIR)
            CALL KSAPO(IPAIR)
            CALL DEFORM(IPAIR,ECC,ECCM)
            KSTAR(I) = 10
...
*     Set standard binary on large ECC and TC if small GAMMA (large omeq).
      IF ((ECC.GT.0.95d0).AND.(GAMMA(IPAIR).LT.1.0D-09)) THEN
         ICIRC = 0
         CALL TCIRC(QPERI,ECC,I1,I2,ICIRC,TC) 
