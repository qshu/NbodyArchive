...
*       Note that ghost may be saved in an old binary c.m. (search NTOT). 
      DO 10 I = 1,NTOT
          IF (((BODY(I))<1.D-14).AND.(NAME(I).EQ.NAMEG(IMERGE))) 
     &    JCOMP = I
   10 CONTINUE
*
...
*       Reduce evolution time-scale after any delay during ghost stage.
          IF (TEV(2*JPAIR-1).LT.TIME + 0.01d0*TCR) TEV(2*JPAIR-1) = TIME
          IF (TEV(2*JPAIR).LT.TIME + 0.01d0*TCR) TEV(2*JPAIR) = TIME
...
*       Reduce evolution time-scale after any delay during ghost stage.
          IF (TEV(2*NPAIRS-1).LT.(TIME+0.01d0*TCR)) TEV(2*NPAIRS-1)=TIME
