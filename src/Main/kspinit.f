...
*     Estimate an appropriate KS slow-down index for G < GMIN.
         ...
         IF (LIST(1,I1).EQ.0.AND.SEMI.GT.0.d0) THEN
            ...
            IF (KZ(26).GT.0.AND.STEP(ICM).GT.TK) THEN
               IMOD = 1 + INT(LOG(STEP(ICM)/TK)/0.69d0)
               IMOD = MIN(IMOD,5)
            END IF
         END IF
