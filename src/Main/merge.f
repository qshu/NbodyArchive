...
*       Check switching of inner and outer binary for improved restart.
      IF (JCOMP.GT.N) THEN
          SEMI2 = -0.5d0*BODY(JCOMP)/H(JCOMP-N)
...
*       Check for sufficient perturbers (RMAX = apocentre set in IMPACT).
      IF (SQRT(CMSEP2)*RMAX.GT.2.d0*RS(ICOMP).OR.NNB.LE.3) THEN
          FAC = NNBMAX/NNB
          FAC = MIN(FAC,4.0D0)
          RSI = FAC**0.3333d0*RS(ICOMP)
...
*     Remove ghost from NXTLST and add into ghost list
      IF(ABS(TIME+TOFF)>1.D-14) call delay_remove_tlist(JCOMP1,STEP,DTK)
      STEP(JCOMP1) = 2.d0*DTK(1)
      IF(ABS(TIME+TOFF)>1.D-14) call add_tlist(JCOMP1,STEP,DTK)
...
*       Subtract 1 for hyperbolic flybys to count stable systems only.
      IF (H(NPAIRS).GT.0.d0) NMERG = NMERG - 1
