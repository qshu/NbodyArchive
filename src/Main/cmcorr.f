...
*       Adjust the total energy to new kinetic energy & tidal potential.
      BE(3) = BE(3) + 0.5d0*(ERRX + ERRV)
      E(11) = E(11) - 0.5d0*(ERRX + ERRV)
*
...
*       Skip ghosts to avoid spurious prediction inside 1.0E+10.
              IF ((BODY(I))<1.D-14) GO TO 60
              DO 55 K = 1,3
                  DF = TIDAL(K)*CMR(K)
                  DD = TIDAL(K)*CMRDOT(K)
                  FR(K,I) = FR(K,I) - DF
                  F(K,I) = F(K,I) - 0.5d0*DF
                  D1R(K,I) = D1R(K,I) - DD
...
*
     ...
     &                              3.d0*FDOT(K,I))*DT + 2.d0*F(K,I))*DT
