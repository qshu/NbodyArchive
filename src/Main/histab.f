...
*       Determine the outer eccentricity.
    ...
    5 CONTINUE
      RIJ = SQRT(RIJ2)
      A1 = 2.d0/RIJ - VIJ2/(BODY(I) + BODY(J))
*
...
*
      SEMI1 = 1.d0/A1
      ECC2 = (1.d0 - RIJ/SEMI1)**2 + RDOT**2/(SEMI1*(BODY(I) + BODY(J)))
      ECC1 = SQRT(ECC2)
...
*       Evaluate the basic stability condition without fudge factor.
      ...
      IF (ECC1.LT.1.d0) THEN
          XFAC = (1.d0 + Q)*(1.d0 + ECC1)/SQRT(1.d0 - ECC1)
      ELSE
          XFAC = 40.d0*(1.d0 + Q)
      END IF
      PCRIT = 2.8d0*XFAC**0.4d0*SEMI
*
*       Exit if stability value falls outside practical limits.
      IF (((PCRIT.GT.1.5d0*PMIN).OR.(PCRIT.LT.0.5d0*PMIN))
     &  .AND.(J.LE.N)) THEN
          RSTAB = PCRIT
          GO TO 20
      END IF
...
*       Resolve weakly perturbed binary (prevent X(K,I1) = X(K,I2)).
      IF ((GAMMA(IPAIR).LT.GMIN).OR.(ABS(X(1,I1)-X(1,I2))<1.D-14)) THEN
          CALL RESOLV(IPAIR,1)
      END IF
*
