...
*       Save index of fast particle outside 3*<R> or VI < VMAX/4.
              IF((RI2.GT.(9.d0*RSCALE**2)).OR.(VI2.LT.VMAX2/16.d0)) THEN
                  LH = L
                  LI = I
                  RL2 = RI2
                  VL2 = VI2
...
*       Form a list of recently ejected candidates.
          IF (FI2.GT.DBLE(N).OR.STEP(I).LT.DTMIN.OR.
     &        STEPR(I).LT.20.d0*DTMIN) THEN
...
*      Include all VMAX2 members inside 2*RSCALE or with negative velocity.
          ...
          IF ((VI2.GT.VMAX2).AND.
     &       ((RI2.LT.4.d0*RSCALE**2).OR.(RD.LT.0.d0))) THEN
              DO 8 L = 2,NHI
                  IF (I.EQ.LISTV(L)) GO TO 10
    8         CONTINUE
*       Check maximum membership and possible ghost particle.
              IF (NHI.GE.MLV.OR.STEP(I).GT.1.d0) GO TO 10
              NHI = NHI + 1
              LISTV(1) = LISTV(1) + 1
              NFAST = NFAST + 1
              LISTV(NHI) = I
...
*       Evaluate two-body energy.
   ...
   25     CONTINUE
          RIJ = SQRT(RIJ2)
          SEMI = 2.d0/RIJ - VIJ2/(BODY(I1) + BODY(I2))
*       Accept outwards hyperbolic motion arising from recent interaction.
          IF (SEMI.LT.0.d0.AND.RIJ.LT.10.d0*RMIN.AND.RDOT.GT.0.d0) THEN
              NHI = NHI + 1
              LISTV(NHI) = I1
              NHI = NHI + 1
              LISTV(NHI) = I2
