...
*           CC = M1*MC1/(LAMB1*R1) + ALPHA1*M1*M2/(SEP*EFAC)
            ...
            IF(DM.LE.0.d0.OR.DM.GT.(M1-MC1))THEN
...
*
   20    IF(ABS(MC22)<1.D-14) MF = MAX(MF,MC1+M2)
         M2 = 0.D0
         M1 = MF
         KW2 = 15
*
...
* Include collision criterion at the end (note RC1 = 0 for main sequence).
      IF (ABS(RC1)<1.D-14) THEN
          IF (R1 + RCC2.GE.SEP*(1.d0 - ECC)) THEN
              COEL = .TRUE.
          END IF
      ELSE
          IF (RC1 + RCC2.GE.SEP*(1.d0 - ECC)) THEN
              COEL = .TRUE.
          END IF
      END IF
*
