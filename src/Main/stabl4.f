...
*       Check stability (AM 1997; inner triple or well separated quadruple).
      ITERM = 0
      IF (RB1.GT.5.d0*RB2.AND.E2.LT.1.d0) THEN
          Q1 = M(K3)/MB0
          XFAC = (1.d0 + Q1)*(1.d0 + E2)/SQRT(1.d0 - E2)
          PCRIT = 2.8d0*XFAC**0.4d0*SEMI0
          PMIN = SEMI2*(1.d0 - E2)
   ...
   15         FORMAT ('  STABT:    A0 A2 E0 E2 RATIO R0 R2 PCR PM ',
     &                             1P,2E10.2,0P,2F7.3,F6.2,1P,4E9.1)
          END IF
      ELSE IF (RB1.GT.5.d0*MAX(RB0,RB).AND.E1.LT.1.d0.AND.
     &         MIN(SEMI0,SEMI).GT.0.d0) THEN
*       Choose smallest binary as third body and ignore fudge factor.
          ...
          ELSE
              ...
              AIN = SEMI0
          END IF
          XFAC = (1.d0 + Q1)*(1.d0 + E1)/SQRT(1.d0 - E1)
          PCRIT = 2.8d0*XFAC**0.4d0*AIN
          PMIN = SEMI1*(1.d0 - E1)
          IF (PCRIT.LT.PMIN) THEN
              ITERM = -1
              if(rank.eq.0)
