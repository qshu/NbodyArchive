...
*       Obtain relevant distributions of KS binaries.
      DO 50 J = 1,NPAIRS
          I = N + J
          IF ((H(J)>=0.d0).OR.((BODY(I))<1.D-14)) GO TO 50
          J1 = 2*J - 1
          J2 = 2*J
*
...
*       Adopt logarithmic distribution of semi-major axis (factor 2).
          ...
          IF (SEMI.LT.A0) THEN
              K = 2 + INT(LOG10(A0/SEMI)/LOG10(2.d0))
              K = MIN(K,10)
              JLAST = MAX(JLAST,K)
              JD(K) = JD(K) + 1
          ELSE
...
*       Form eccentricity dispersion & histogram.
          ECC2 = (1.d0 - R(J)/SEMI)**2 + TDOT2(J)**2/(BODY(I)*SEMI)
          DISP = DISP + ECC2
          IF (ECC2.GT.EMAX) EMAX = ECC2
          K = 1 + INT(10.d0*SQRT(ECC2))
          IF (K.LE.10) JE(K) = JE(K) + 1
*
*       Set up logarithmic distribution of binding energy (factor 2).
          EB = -0.5d0*BODY(J1)*BODY(J2)/SEMI
          IF (EB.LT.EB0) THEN
              K = 2 + INT(LOG10(EB/EB0)/LOG10(2.d0))
              K = MIN(K,14)
              KLAST = MAX(KLAST,K)
              JEB(K) = JEB(K) + 1
              EBMAX = MIN(EBMAX,EB)
...
*       The previous line does not work - RSp Dec. 2019
...
*       Set fractional gain of binding energy (initial energy = -0.25).
      ...
      ELSE
          E(9) = EMERGE
          DB = -4.d0*(E(1) + E(2) + E(5) + E(7) + E(9) + ECOLL + EMDOT
     &                                                 + EKICK - EBIN0)
      END IF
*
*       Print eccentricity & energy distributions.
   ...
   60 FORMAT (/,' BINARIES',4X,'OR =',I5,'  EX =',I3,'  DB =',F7.3,
     &          '  SB =',F8.4,'  BB =',F8.4,'  CH =',F8.4,'  NC =',I4,
     &         '  NCH =',I4,'  NEWB =',I4,'  CIRC =',I4,'  N(A) =',10I4)
*
      IF (DISP.GT.0.D0) DISP = SQRT(DISP/DBLE(NPAIRS))
      EMAX = SQRT(EMAX)
      if(rank.eq.0)
