...
*     For diagnostic, thus safe for parallel
      SAVE  TTIDE,IONE
      DATA  ECCM,ECCM2,TTIDE,IONE  /0.002d0,0.00000399d0,0.0D0,0/
      INTEGER IP1
      DATA IP1 /0/
*
*
...
*
      ...
      IF (KZ(27).EQ.1) THEN
          ZF = 4.d0
          IF (ECC.GT.0.95d0) ZF = 50.d0
*       Skip if penetration is not significant (prevents frequent calls).
          IF (ABS(QPERI - ZF*R1).LT.0.01d0*QPERI) GO TO 50
...
*       Update KS variables for CHAOS or TERMINATED CHAOS (H > 0).
          IF ((KSTAR(I).EQ.-1).OR.(H(IPAIR).GT.0.d0)) THEN
...
*       Set new semi-major axis from angular momentum conservation.
              SEMI1 = AM0/(1.d0 - ECC1**2)
          END IF
*       Form the corresponding energy change.
          DH = 0.5d0*BODY(I)*(1.d0/SEMI - 1.d0/SEMI1)
          DE(1) = -ZMU*DH
          DE(2) = 0.d0
*
*       Special Treatment of GR for compact binaries RSp March 2019
...
*             DE(2) = 0.0
*         END IF
          SEMI1 = -0.5d0*BODY(I)/(H(IPAIR) + DH)
          ECC1 = 1.d0 - PERI/SEMI1
          ECC1 = MAX(ECC1,ECCM)
      END IF
*
*       Skip possible hyperbolic case.
      IF ((H(IPAIR) + DH).GT.0.d0) GO TO 50
*
...
*       Form new perturber list after significant energy loss.
      NP0 = LIST(1,I1)
      IF (ABS(SEMI1/SEMI).LT.0.5d0) THEN
          CALL KSLIST(IPAIR)
...
*       Produce diagnostic output for interesting new case (first time).
      IF ((ECC.GT.0.99d0).AND.(ABS(ECC - ECC1).GT.0.01d0)
     &     .AND.IONE.EQ.0) THEN
          if(rank.eq.0)
...
*       Check for hierarchical configuration with eccentric inner binary.
      IF ((KZ(27).EQ.2).AND.(ECC.GT.0.95d0).AND.(HI.LT.0.d0)) THEN
          NP1 = LIST(1,I1) + 1
          DO 30 L = 2,NP1
              J = LIST(L,I1)
*              CALL JPRED(J,TIME,TIME)
   ...
   25         CONTINUE
              ...
              A1 = 1.d0/A1
              IF ((1.d0/A1).GT.(0.1d0/SEMI)) THEN
                  ECC2 = (1.d0 - RIP/A1)**2 +
     &                                  RDOT**2/(A1*(BODY(I) + BODY(J)))
                  RP = A1*(1.d0 - SQRT(ECC2))
                  RA = SEMI*(1.d0 + ECC)
                  SR = RP/RA
                  ICIRC = -1
c$$$                  JCOMP = J
...
*       Set one unperturbed period for small apocentre perturbation (#27=1).
      GA = GAMMA(IPAIR)*(SEMI1*(1.d0 + ECC1)/R(IPAIR))**3
...
*       Count any hyperbolic captures.
      IF ((SEMI.LT.0.d0).AND.(SEMI1.GT.0.d0)) THEN
          NTIDE = NTIDE + 1
          QPS = QPERI/R1
          if(rank.eq.0)
...
*       See whether a low-eccentricity synchronous state has been reached.
      RCOLL = 0.75d0*(RADIUS(I1) + RADIUS(I2))
      IF ((ABS(SEMI1).LT.1.5d0*RCOLL).AND.(ECC.LT.ECCM).AND.
     &    (KSTAR(I).LT.10)) THEN
          KSTAR(I) = 10
*     ks MPI communication TEV
          call ksparmpi(K_store,K_int,K_KSTAR,I,0,KSTAR(I))
...
***       IF (TEV(J1) - TIME.GT.0.01*TEV(J1)) TEV(J1) = TIME
          DR = (0.99d0*4.d0*RADIUS(J1) - QPERI)/QPERI
          YF = MAX(ABS(DR),0.01D0)
          RADIUS(J1) = (1.d0 - MIN(YF,0.1D0))*RADIUS(J1)
