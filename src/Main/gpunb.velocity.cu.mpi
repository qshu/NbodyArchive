...
void GPUNB_open(int nbmax,int irank){
    ...
    MPI_Comm_rank( MPI_COMM_WORLD, &rank );
		...
		cudaSetDevice(devid[rank]);
                fprintf(stderr, " rank %d Set Dev %d\n", rank, devid[rank]);
    char hostname[150];
    memset(hostname,0,150);
    gethostname(hostname,150);
    int len;char pbusid[13];
    len = 13;
      cudaDeviceGetPCIBusId ( pbusid, len, devid[rank] );
      cudaDeviceProp prop;
      cudaGetDeviceProperties(&prop, devid[rank]);
      fprintf(stderr, "# GPU Set Device - rank: %d; HOST %s; NGPU %d; device: %d %s Bus Id %s \n", rank, hostname, numGPU, devid[rank], prop.name, pbusid);
                        int tid = 0;
