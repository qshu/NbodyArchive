...
*       Save energy for correction (Roche COAL but not via CMBODY & EXPEL).
              ...
              IQCOLL = 3
          END IF
          IF (H(IPAIR).GT.0.d0) THEN
              WHICH1 = ' HYPERB '
              NHYP = NHYP + 1
              IQCOLL = -1
              VINF = SQRT(2.d0*H(IPAIR))*VSTAR
              EB1 = BODY(2*IPAIR-1)*BODY(2*IPAIR)*H(IPAIR)/BODY(I)
          END IF
*
...
*       Form central distance (scaled by RC), central distance and period.
   ...
   10 CONTINUE
      ...
      ECC = 1.d0 - RCOLL/SEMI
      IF (IQCOLL.EQ.5) THEN
          EB1 = -0.5d0*BODY(I1)*BODY(I2)/SEMI
      END IF
*
...
*       Check that a mass-less primary has type 15 for kick velocity.
      IF(ZMNEW*ZMBAR.LT.1.d-10.AND.KW1.NE.15)THEN
         if(rank.eq.0)then
