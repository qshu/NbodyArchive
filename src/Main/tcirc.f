...
*     
     ...
     &     -2.314374d0,-4.127795d0/
      DATA  ECCM,ECCM1 /0.002d0,0.002001d0/
...
*     Set large circularization time for merged binary.
      IF ((RADIUS(I1)<1.D-14).OR.(RADIUS(I2)<1.D-14)) THEN
         TC = 1.0D+10
         GO TO 30
      END IF
*     
...
*     Include an appropriate numerical factor (Chris Tout 07/08).
      AR = RP/((1.d0 - ES0)*RADIUS(J1))
      FF = 1.d0
      IF ((KSTAR(J1).EQ.1).AND.(BODY(J1)*SMU.GT.1.25d0)) THEN
         FF = 100.d0
      ...
      END IF
      Q12 = BODY(J1)/BODY(J2)
      TAUC = FF*2.d0*Q12**2/(1.d0 + Q12)*AR**8
...
*     Obtain the new eccentricity.
      ...
      ECC = (-1.d0 + C(1)*Z - SQRT(C(2)*Z**2 + C(3)*Z + C(4)))
     &     /(C(5) + C(6)*Z)
*     
      ECC = MAX(ECC,ECCM)
      RP = RP*(1.d0 + ES0)/(1.d0 + ECC)
      ES0 = ECC
      GO TO 30
*     
*     Evaluate circularization time (in units of 10**6 yrs).
 10   TC = TSTAR*(1.d0 - FF)/CONST
*     
*     Activate tidal indicator if TC < 2x10**9 yrs or hyperbolic orbit.
      IF ((TC.LT.2000.d0).OR.(ES0.GT.1.d0)) THEN
         IP = KVEC(I1)
         ITIME = ITIME + 1
