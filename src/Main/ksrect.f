...
*       Skip rectification for small eccentricity or large perturbation.
     ...
     &    'ipair,semi,ecc2,ecc,kw,r,h,gamma=',ipair,semi,ecc2,ecc,
...
*    &    kstar(n+ipair),r(ipair),h(ipair),gamma(ipair)
     ...
     &              (X(3,ICM) - RDENS(3))**2)
      ...
      IF(ISNAN(RI).OR.ISNAN(VI))THEN
      print*,' Warning RI,VI NaN: X,XDOT,RDENS 1-3=',
     &    X(1:3,ICM),XDOT(1:3,ICM),RDENS(1:3)
      END IF
          EB = -0.5*BODY(I1)*BODY(I2)/SEMI
          PD = TWOPI*SEMI*SQRT(DABS(SEMI)/BODY(ICM))*TSTAR*365.24D6
          WRITE (6,17)  TIME+TOFF,NAME(I1),NAME(I2),
