...
*       Skip rectification for small eccentricity or large perturbation.
      ...
      ECC = SQRT(ECC2)
      IF (ECC.LE.0.01d0) GO TO 50
      IF (GAMMA(IPAIR).GT.0.1d0) GO TO 50
*
*       Include Roche rectification for large ECC (additional to roche.f).
      IF ((KSTAR(I).GT.10).AND.(ECC.GT.0.2d0)) THEN
          QP = SEMI*(1.d0 - ECC)
          ICIRC = -1
...
*     ks MPI communication
      ...
      END IF
*
      HI = (2.d0*UPR2 - BODY(N+IPAIR))/R(IPAIR)
      ERR = (HI - H(IPAIR))/HI
...
*       Avoid negative round-off value on second try (NB! no change in CK).
      IF ((ITER.EQ.2).AND.(A2.LT.0.d0)) A2 = 0.0D0
*
