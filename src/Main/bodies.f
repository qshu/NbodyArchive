...
*
   ...
   30     CONTINUE
     ...
     &          (XDOT(3,I) - XDOT(3,JMIN))**2
          EREL = 0.5d0*VR2 - (BODY(I) + BODY(JMIN))/RIJMIN
*       Only print significant binaries.
          IF (EREL.GT.-0.1d0*ECLOSE) GO TO 40
          SEMI = -0.5d0*(BODY(I) + BODY(JMIN))/EREL
...
*       Output of regularized binaries (frequency NFIX with KZ(6) = 4).
   50 DO 60 JPAIR = 1,NPAIRS
          IF (H(JPAIR).GE.0.d0) GO TO 60
          I = 2*JPAIR - 1
          ICM = N + JPAIR
          JMIN = I + 1
          IF (BODY(I).LE.0.0D0) GO TO 60
          SEMI = -0.5d0*(BODY(I) + BODY(JMIN))/H(JPAIR)
          ZN = SQRT((BODY(I) + BODY(JMIN))/SEMI**3)
