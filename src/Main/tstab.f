...
*       Skip double hierarchy (RESET2 uses standard stability criterion).
          ITERM = 1
      ELSE IF (PMIN1.GT.0.8d0*YFAC*PCRIT.OR.
     &        (ECC1.GT.0.99d0.AND.ECC1.LT.1.d0)) THEN
*       Estimate time-scale for long-lived hierarchy.
          NK = 1 + int(10.d0*ECC1/(1.d0 - ECC1))
...
*       Note that large NK implies termination by other means.
          IF (ECC1.LT.1.d0.AND.NK.LE.1) ITERM = 1
*       Modify PCRIT so it becomes < SEMI1*(1 - ECC1)*(1 - 2*PERT).
          IF (PCRIT.LT.PMIN1) PCRIT = 0.999d0*PMIN1/YFAC
      ELSE IF (ECC1.LT.1.d0) THEN
