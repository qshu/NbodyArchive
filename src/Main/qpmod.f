...
*
     ...
     &                ECOLL1,RCOLL,QPERI,ISTAR(NMX),ICOLL,ISYNC,NDISS1
      ...
      INTEGER IS(2)
      DATA  ECCM,ECCM2  /0.002d0,0.00000399d0/
*
*
*       Skip for small dissipation (ISYNC > 0 delays further calls).
      IF (KZ27.EQ.1) THEN
          R1 = MAX(SIZE(K1),SIZE(K2))
          IF (ABS(QPERI - 4.d0*R1).LT.0.01d0*QPERI) THEN
              ISYNC = 1
              ITERM = 0
              GO TO 90
          END IF
...
*       Include safety check on energy loss to prevent new SEMI < R.
          DH = -(DE(1) + DE(2))/MU
          IF (H + DH.LT.-0.5d0*MB*RINV(IM)) THEN
              DH = -0.5d0*MB*RINV(IM) - H
          END IF
          SEMI1 = -0.5d0*MB/(H + DH)
...
*       Adopt reduced energy loss in case of imaginary solution.
          ...
          ELSE
              A3 = A1
              if(rank.eq.0)WRITE (6,5)  A1, A2, SQRT(1.d0/A3), A3, C1
...
*       Print diagnostic if eccentricity > 0.99 or #30 > 1.
      IF (ECC.GT.0.99d0.OR.KZ30.GT.1) THEN
          if(rank.eq.0)
