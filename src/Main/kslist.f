...
*       Ensure extra perturbers at new regularizations (R may be small).
          IF ((IPHASE.GT.0).AND.(SEMI.GT.0.d0)) THEN
              RAP = SEMI*(1.d0 + ECC)
          END IF
      END IF
*
      RAP = MIN(RAP,20.d0*RMIN)
      RCRIT2 = 2.d0*RAP**2/BODY(I)
      RCRIT3 = RCRIT2*RAP/GMIN
*       Base fast search on maximum binary mass (2*BODY1).
      RCRIT2 = 2.d0*RCRIT2*BODY1*CMSEP2
*
...
*       Ensure at least one perturber first time (max 5 tries except CHAOS).
      IF ((NNB1.EQ.1).AND.(IPHASE.GT.0).AND.
     &  (NNB2.GT.1).AND.(TIME.GT.0.0)) THEN
          RCRIT2 = 2.d0*RCRIT2
          RCRIT3 = 2.d0*RCRIT3
          IT = IT + 1
*       Skip repeat for small size (next KSLIST requires many periods).
          IF (((SEMI*SU.GT.10.d0).AND.(IT.LE.5)).OR.(KSTAR(I).EQ.-1))
     &     GO TO 6
      END IF
*
