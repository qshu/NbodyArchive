...
*       Check for further unperturbed motion.
      IF ((NNB0.EQ.0).AND.(H(IPAIR).LT.0.d0)) THEN
...
*     Reduce the criterion from 0.25 to 0.1 to reduce the energy error
      IF (GI.GT.0.10d0) GO TO 2
      IF (GI.LT.0.03d0) THEN
          JCOMP = 0
          GO TO 20
      END IF
      CALL FLYBY(I,ITERM)
...
*       Set termination if strong perturber <= N forms dominant pair.
      IF (JCOMP.GT.0.OR.GI.GT.1.d0) THEN
...
*       Terminate the binary when close particle is KS pair
         ...
         IF (JCOMP.LE.N.OR.GI.GT.1.d0) IQ = .TRUE.
      END IF
*
*       Check termination of hyperbolic encounter (R > R0 or R > 2*RMIN).
   20 IF (HI.GT.0.0D0.AND.NAME(I).GT.0) THEN
          IF ((RI.GT.R0(IPAIR).AND.GI.GT.GMAX).OR.RI.GT.2.d0*RMIN.OR.
     &        (GI.GT.0.5d0.AND.TD2.GT.0.d0)) THEN
*       Skip termination delay in case of velocity kick (cf. routine KSTERM).
              IF (HI.LT.100.d0.OR.GI.GT.0.1d0.OR.RI.GT.5.d0*RMIN) THEN
                  IQ = .TRUE.
              END IF
          END IF
      END IF
...
*       Convert to physical time units modified by Stumpff coefficients.
      STEP(I1) = (((((TDOT6*DT12 + TDOT5*Z5)*0.2d0*DTU+0.5D0*TDOT4)*DTU
...
*     For massive black hole ks, try to avoid terminated with large seperation
      IF((KZ(24).EQ.1).AND.((BODY(I)/BODYM).GT.200.d0)
     &  .AND.(RI.GT.10.d0*RMIN)) THEN
         IQ = .true.
      END IF
...
*     IF (RI.GT.R0(IPAIR)) THEN
      IF (RI.GT.R0(IPAIR).AND.RI.GT.2.d0*RMIN.AND..NOT.IQ) THEN
*       See whether termination can be delayed for sufficient perturbers.
          IF (NNB0.LT.0.80d0*LIST(1,I).AND.GI.LT.0.1d0) GO TO 60
...
*       Determine pericentre for hyperbolic two-body motion.
              SEMI = -0.5D0*BODY(I)/HI
              ECC2 = (1.d0 - RI/SEMI)**2+ TDOT2(IPAIR)**2/(BODY(I)*SEMI)
   ...
   54                 FORMAT ('CLOSE   Time[NB] NAME(I1) NAME(I2) ',
     ...
     &                     'PERI[R*] ',1P,E26.17,0P,2I2,2I4,4F12.5)
                  ...
                  IF (QPERI.LT.RCAP) THEN
                      ...
                      FAC = 0.5d0*BODY(I)/BODY(J1)
*       Adopt collision criterion of Kochanek (Ap.J. 385, 604, 1992).
                      IF (KZ(27).LE.2) THEN
                          RCOLL = 1.7d0*FAC**0.3333d0*RADIUS(J1)
                      ELSE
                          RCOLL = 6.d0*BODY(I)/CLIGHT**2
                      END IF
                      RI2 = 0.d0
                      VI2 = 0.d0
                      DO 61 K = 1,3
...
*       Update termination length scale in case of initial soft binary.
          ...
          IF (EB.LT.EBH) R0(IPAIR) = MAX(RMIN,2.d0*SEMI)
      ELSE 
          ECC2 = (1.d0 - RI/SEMI)**2 + TDOT2(IPAIR)**2/(BODY(I)*SEMI)
          ECC = SQRT(ECC2)
          RP = SEMI*(1.d0 - ECC)*(1.d0 - 2.d0*GI)
*       Find merger index.
          IM = 0
          DO 72 K = 1,NMERGE
...
*       Update unperturbed binary if T - TOSC > 10 Myr (cf. IMPACT & DECIDE).
                          IF ((TIME - TOSC(K))*TSTAR.GT.10.d0) GO TO 90
                      END IF
...
*       Produce diagnostics for any circularizing perturbed binary.
      IF (KSTAR(I).EQ.-2.AND.GI.GT.0.01d0) THEN
          ECC = RI/SEMI - 1.d0
          QPS = SEMI*(1.d0 - ECC)/MAX(RADIUS(I1),RADIUS(I2))
          ZM = BODY(I1)/BODY(I2)
          if(rank.eq.0)
...
*       Set approximate value of next period with perturbation included.
      TK = TWOPI*SEMI*SQRT(SEMI*BODYIN)*(1.d0 + GI)
      IF (IMOD.GT.1) THEN
          TK = ZMOD*TK
      END IF
*
