...
*       Set zero eccentricity after common envelope stage (still large R).
              IF (R(IPAIR).GT.2.d0*SEMI(JPAIR)) THEN
                  ECC(JPAIR) = 0.d0
              END IF
...
C          RATIO = MIN(RATIO,99.9d0)
          IF (SEMI(JPAIR).LT.0.d0.AND.RATIO.GT.5.d0) GO TO 20
...
*       Ensure that errant massless remnant will escape.
                      ...
                      DO 26 L = 1,3
                          X0(L,J) = 1000.d0*RSCALE*X(L,J)/RI
                          X(L,J) = X0(L,J)
                          X0DOT(L,J) = SQRT(0.004d0*ZMASS/RSCALE)*
     &                                 XDOT(L,J)/VI
                          XDOT(L,J) = X0DOT(L,J)
   26                 CONTINUE
                  ELSE
