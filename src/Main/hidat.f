...
*       Perform stability check.
              IF(PMIN*(1.d0 - GAMMA(JPAIR)).LT.PCR.AND.E1.LT.0.96d0.AND.
     &            LIST(1,J1).GT.0) THEN
*                 ISTAB = JPAIR
                  if(rank.eq.0)
     &            WRITE (6,20) NAME(J1),NAME(J),180.d0*ANGLE/3.14d0,E1,
     &                         PMIN, PCR, GAMMA(JPAIR), R0(JPAIR)
   20             FORMAT (' MERGE UNSTAB    NAM INC E1 PM PCR G R0 ',
     &                                      2I6,F7.1,F7.3,1P,4E10.2)
              END IF
          ELSE IF ((BODY(J1))<1.D-14) THEN
...
*       Form outer semi-major axis and eccentricity.
              SEMI1 = -0.5d0*BODY(ICM)/H(JP)
              ECC1 = (1.d0 - R(JP)/SEMI1)**2 +
