...
*       Include the whole (L*F)' term in explicit derivatives of FU & H.
     ...
     &                            TDOT2(IPAIR)*A(K) + R(IPAIR)*A(K+4))
          HDOT3(IPAIR) = HDOT3(IPAIR) + 2.d0*FUDOT(K,IPAIR)*A(K) +
     &                                           4.d0*FU(K,IPAIR)*A(K+4)
          TDOT4 = TDOT4 + 2.d0*FUDOT(K,IPAIR)*U(K,IPAIR) +
     &                                    6.d0*FU(K,IPAIR)*UDOT(K,IPAIR)
...
*       Form L(U'') and add two of the three terms in (L*F)'' (11/1/98).
     ...
     &            2.d0*(A1(1,K)*FPDOT(1) + A1(2,K)*FPDOT(2) +
     &                                     A1(3,K)*FPDOT(3))
          HDOT3(IPAIR) = HDOT3(IPAIR) + 2.d0*UDOT(K,IPAIR)*Q2(K)
   66 CONTINUE
*
*       Form higher derivatives by boot-strapping (Q2 added 11/1/98).
      DO 70 K = 1,4
          FUDOT2(K,IPAIR) = 0.5d0*(H(IPAIR)*FU(K,IPAIR) +
     &                   HDOT2(IPAIR)*U(K,IPAIR) + TDOT3(IPAIR)*A(K)) +
     &                   TDOT2(IPAIR)*A(K+4) + HDOT(IPAIR)*UDOT(K,IPAIR)
          FUDOT3(K,IPAIR) = 0.5d0*(H(IPAIR)*FUDOT(K,IPAIR) + TDOT4*A(K)+ 
     &                        HDOT3(IPAIR)*U(K,IPAIR)) +
     &                   1.5d0*(HDOT(IPAIR)*FU(K,IPAIR) +
     &                        HDOT2(IPAIR)*UDOT(K,IPAIR) +
     &                        TDOT3(IPAIR)*A(K+4))
          FUDOT2(K,IPAIR) = FUDOT2(K,IPAIR) + 0.5d0*R(IPAIR)*Q2(K)
          FUDOT3(K,IPAIR) = FUDOT3(K,IPAIR) + 1.5d0*TDOT2(IPAIR)*Q2(K)
          HDOT3(IPAIR) = HDOT3(IPAIR) + 2.d0*UDOT(K,IPAIR)*Q2(K)
          HDOT4(IPAIR) = HDOT4(IPAIR) + 2.d0*FUDOT2(K,IPAIR)*A(K) +
     &                  6.d0*(FUDOT(K,IPAIR)*A(K+4) + FU(K,IPAIR)*Q2(K))
          TDOT5 = TDOT5 + 2.d0*FUDOT2(K,IPAIR)*U(K,IPAIR) +
     &            8.d0*FUDOT(K,IPAIR)*UDOT(K,IPAIR) + 6.0*FU(K,IPAIR)**2
          TDOT6 = TDOT6 + 2.d0*FUDOT3(K,IPAIR)*U(K,IPAIR) +
     &                    10.d0*FUDOT2(K,IPAIR)*UDOT(K,IPAIR) +
     &                    20.d0*FUDOT(K,IPAIR)*FU(K,IPAIR)
   70 CONTINUE
*
*       Check maximum square step (soft binaries & weak hyperbolic pairs).
      IF (ABS(H(IPAIR)).GT.ECLOSE) THEN
          A2 = 0.5d0/ABS(H(IPAIR))
      ELSE
          A2 = MIN(R(IPAIR)/BODY(I),0.5d0/ABS(H(IPAIR)))
      END IF
*
*       Assign a conservative value of the initial step (except IMOD > 1).
      FAC = 0.8d0
      IF ((IMOD.GT.1).OR.(H(IPAIR).GT.0.d0)) FAC = 1.d0
      DTU = FAC*ETAU*SQRT(A2)/(1.d0 + 1000.d0*GAMMA(IPAIR))**0.333d0
*
*       Include convergence criterion DH = H'*DTU + H''*DTU**2/2 = 0.001*|H|.
      IF ((IMOD.EQ.1).AND.(ABS(HDOT2(IPAIR))>1.D-14)) THEN
          DH = 1.0D-03*MAX(ABS(H(IPAIR)),ECLOSE)
...
*       Skip Stumpff part and use ZMOD*TK for unperturbed binary.
      IF ((LIST(1,I1).EQ.0).AND.(SEMI.GT.0.d0)) THEN
...
*       Ensure that c.m. step is not exceeded (H > 0 is OK).
      IF ((STEP(I1).GT.STEP(I)).AND.(H(IPAIR).LT.0.d0)) THEN
          DTU = 0.5d0*DTU
          GO TO 75
      END IF
*
