...
*     Include step reduction for large derivatives.
      IT = 0
 1    IF (ABS(udot(1))*dtau.GT.0.01d0*spin10) THEN
         dtau = 0.5D0*dtau
         nsteps = 2*nsteps
         IT = IT + 1
         IF (IT.LT.5) GO TO 1
 ...
 5    IF (ABS(udot(2))*dtau.GT.0.01d0*spin20) THEN
         dtau = 0.5D0*dtau
         nsteps = 2*nsteps
         IT = IT + 1
         IF (IT.LT.5) GO TO 5
...
*     semi=(semi*(1+m21)/m21/semi0**2)**2/fac
      semi=(semi*(1.d0+m21)/m21/semi0**2)**2
      oa = 1.d0/semi
      IF (IQ.EQ.0) THEN
         oa0 = oa
         IQ = 1
...
*     udot(2)=(oa/fac)**6*C4*(oa**1.5*f2-fac**1.5*f5*spin2)
      udot(1)=oa**6*C3*(oa**1.5d0-spin1) - C5*spin1
      udot(2)=oa**6*C4*(oa**1.5d0-spin2)

*     Quit on relative change exceeding 2 % (SJA 05/09).
      IF (ABS(oa - oa0).GT.0.02d0*oa0) IFLAG = 1
