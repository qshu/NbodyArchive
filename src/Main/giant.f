...
*     Unsafe for parallel! update variables: cm, used variables: namec
     ...
     &               NAMEC(NTMAX)
      ...
      DATA  A0,A1,A2,A3  /0.944525D0,-0.392030D0,6.01655D-2,-3.34790D-3/
      DATA  B0,B1,B2  /-0.3789D0,1.481D0,-0.1018D0/
      DATA  C0,C1,C2,C3  /1.452104d0,3.923872d0,-11.88722d0,13.46106d0/
...
*     KS mpi coummincation for CM
          CM(L,IC) = (0.3d0 + 0.1d0*DBLE(KSTAR(I) - 3))/ZMBAR
...
*     KS mpi coummincation for CM
          ...
          call ksparmpi(K_store,K_real8,K_CM_M,L,IC,CM(L,IC))
      END IF
      IF(KSTAR(I).EQ.9) CM(L,IC) = 0.5d0*BODY(I)
*
...
*       FAC = (GM)^{1/2}*M_{env}^{1/3)/(R^{5/6}*L^{1/3}) = 8.48D+03 for S.U.
      QL = 8.48D+03*SQRT(ZM)*(ZME/ZL)**0.33d0/RSI**0.833d0
*
