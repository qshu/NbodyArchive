...
*    &           0.75,0.9/
      DATA FLAGR/0.01d0,0.02d0,0.05d0,0.1d0,0.2d0,0.3d0,0.4d0,
     &           0.5d0,0.625d0,0.75d0,0.9d0/
      DATA N1/0/
*
*
...
*       Improve choice of NAM1 using relative deviation (max 10 tries).
      DM = (ZM1 - 0.5d0*ZMASS)/ZMASS
      IF (ABS(DM).GT.0.001d0.AND.ITER.LE.10) THEN
          ITER = ITER + 1
          IF (ABS(DM).GT.0.05d0) DM = DM*0.05d0/ABS(DM)
          NM = DM*DBLE(N-NPAIRS)
          IF (NM.NE.0) THEN
              NAM1 = NAM1 - NM
              IF (KZ(5).EQ.2) THEN
...
*       Obtain sum of inverse separations to evaluate mass segregation.
   ...
   11         CONTINUE
              POT1 = POT1 + 1.d0/SQRT(RIJ2)
   12     CONTINUE
   14 CONTINUE
*       Define mean harmonic distance.
      RP1 = DBLE(NP)*(NP - 1.d0)/(2.d0*POT1)
*
      ...
      DO 44 L = LX0,NP-1
          ...
          DO 42 LL = L+1,NP
              ...
              DO 41 K = 1,3
                  RIJ2 = RIJ2 + (X(K,I) - X(K,J))**2
   41         CONTINUE
              POT0 = POT0 + 1.d0/SQRT(RIJ2)
   42     CONTINUE
   44 CONTINUE
*       Define mean harmonic distance.
      RP0 = DBLE(NP0)*(NP0 - 1.d0)/(2.d0*POT0)
*
      ...
      DO 54 L = 1,NP-1
          ...
          DO 52 LL = L+1,NP
              ...
              DO 51 K = 1,3
                  RIJ2 = RIJ2 + (X(K,I) - X(K,J))**2
   51         CONTINUE
              POT2 = POT2 + 1.d0/SQRT(RIJ2)
   52     CONTINUE
   54 CONTINUE
      RP2 = DBLE(NP)*(NP - 1.d0)/(2.d0*POT2)
*
*       Sort square distances with respect to the centre C.
      CALL SORT1(NP,R2,JLIST)
