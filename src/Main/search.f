...
*      IF (STEP(J).GT.SMIN) GO TO 2
       IF (STEP(J).GT.4*STEP(I).AND.BODY(J).LT.10.d0*BODY(I)) GO TO 2
...
c$$$       end if
      ...
      IF (RIJ2.GT.2.d0*RMIN22) GO TO 2
*
      FIJ = BODY(J)/RIJ2
...
*       Only select approaching particles (include nearly circular case).
      RIJMIN = SQRT(RJMIN2)
      IF (RDOT.GT.0.02d0*SQRT((BODY(I) + BODY(JCOMP))*RIJMIN)) THEN
         TRACE_FLAG = 3
         GO TO 10
      END IF
*
...
*       Accept #I & JCOMP if the relative motion is dominant (GI < 0.25).
      GI = PERT*RJMIN2/BCM
      IF (GI.GT.0.25d0) THEN
*         IF (KZ(4).GT.0.AND.TIME-TLASTT.GT.4.44*TCR/DBLE(N))
*    &                                             CALL EVOLVE(JCOMP,0)
          TRACE_FLAG=4
          GO TO 10
      END IF
