...
*
      J = LIST(L,I)
*RSPR-Dec19**
*     LPRINT=(STEP(J).GT.SMIN).AND.
*    *   (STEP(J).LE.4*STEP(I).OR.BODY(J).GE.10*BODY(I))
*      IF(LPRINT)
*    *    WRITE(6,987)NAME(I),NAME(J),KSTAR(I),KSTAR(J),
*    *    SMIN,STEP(J),4*STEP(I),BODY(J),10*BODY(I)
*987   FORMAT(' SEARCH: KSTRY nij, kwij, smin, dtj,4*dti, mj,10*mi = ',
*    *      4I8,1P,5E13.5)
*RSPR-Dec19**
...
*
      DO 6 K = L,2,-1
          J = LIST(K,I)
*RSPR-Dec19**
*     LPRINT=(STEP(J).GT.SMIN).AND.(STEP(J).LE.8*STEP(I))
*     IF(LPRINT)
*    *    WRITE(6,988)NAME(I),NAME(J),KSTAR(I),KSTAR(J),
*    *    SMIN,STEP(J),8*STEP(I)
*988   FORMAT(' SEARCH: KSTRY nij, kwij, smin, dtj,8*dti = ',
*    *      4I8,1P,3E13.5)
*RSPR-Dec19**
