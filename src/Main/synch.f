...
*     Exit if binary is not identified but include updating of small ECC.
      IF (IC.EQ.0) THEN
         ...
         DTM = MIN(DTM,DTM2)
         DTM = MAX(DTM,0.1D0)
         TEV(I) = TIME + 0.1d0*MIN(DTM,DTR)
         A0 = -0.5d0*BODY(I)/H(IPAIR)
         ECC2 = (1.d0 - R(IPAIR)/A0)**2 + TDOT2(IPAIR)**2/(A0*BODY(I))
         ECC = SQRT(ECC2)
         ITIME = ITIME + 1
         IF (ITIME.GT.10) THEN
            if(rank.eq.0)
...
*     Check enforcement of circularization for small TCIRC.
         IF (ECC.LT.0.10d0.AND.LIST(1,I1).EQ.0) THEN
            ICIRC = -1
            QPERI = A0*(1.d0 - ECC)
...
*     Initialize chaos elements after possible common envelope or E < 0.1.
         IF (ECC.LE.0.003d0.OR.IC.EQ.0) THEN
            NCHAOS = NCHAOS + 1
            IC = NCHAOS
            NAMEC(IC) = NAME(I)
            RP(IC) = A0*(1.d0 - ECC)
            ...
            IF (NCHAOS.GT.NTMAX) THEN
               if(rank.eq.0)
     &              WRITE (6,4)  NAME(I1), NCHAOS, ECC, A0*(1.d0 - ECC)
...
*     Rectify elements after possible common envelope, Roche or GR process.
      QPERI = A0*(1.d0 - ECC)
      IF (ES0.GT.0.01d0.OR.ABS(QPERI-RP0).GT.0.02d0*QPERI) THEN
         DT = TIME - TOSC(IC)
         IF (ITIME.LT.50) THEN
            if(rank.eq.0)
...
*     Set standard binary or current elements on significant departure.
      IF (ECC.GT.0.01d0.OR.ES0.GT.0.01d0) THEN
         ECC = SQRT(ECC2)
...
*     Skip integration if both spins are synchronous.
      IF (ABS(DS1).LT.0.01d0.AND.ABS(DS2).LT.0.01d0) THEN
         KX = MAX(KSTAR(J1),KSTAR(J2))
         IF (ABS(DS20).GT.0.01d0.AND.KX.LT.10) THEN
            if(rank.eq.0)
...
*     Scale the spins by mean motion and define angular momentum.
      spin10=spin1/omeq
      spin20=spin2/omeq
      angmom0=(m21/(1.d0+m21))*semi0**2*sqrt(1.d0-es0**2)+rg2(1)*spin10+
...
*     Form combined moment of inertia factor for primary (RM 11/08).
         C5 = -1.0D+06*DMX*TSTAR/M1 + 2.d0*RDOT/RADIUS(J1)
         R1 = RM/(SU*RADIUS(J1))
         R2 = 1.d0
      ELSE
         R1 = 1.d0
...
*     Choose the step from smallest time-scale (! time0 < time possible).
      taux = min(abs(1.d0/udot1),abs(1.d0/udot2))
      nstep = 1 + int(100.d0*sqrt(ABS(time - time0)/taux))
      nstep = min(nstep,100)
*     Increase number of steps on slow primary rotation.
      IF (DS1.LT.-0.2d0) nstep = nstep + 10
      IF (DS1.LT.-0.8d0) nstep = nstep + 50
      IF (DS1.LT.-0.9d0) nstep = nstep + 50
      DT = TIME - TIME0
*     
*     Reduce integration interval for slow evolution or after merger.
      IF (DT.GT.0.5d0) THEN
         ...
         DTM = MAX(DTM,0.1D0)
         DT = 0.1d0*MIN(DTR,DTM,10.0D0)
         IF (ABS(DS2).GT.0.05d0.OR.DS10.LT.-0.99d0) THEN
            DT = 0.1d0*DT
            nstep = nstep + 50
         END IF
...
*     
      ITT = 0
      IF (KSTAR(J1).EQ.3) DT = 0.5d0*DT
 40   dtau1 = ABS(DT)/DBLE(nstep)
*     
...
*     
      ITT = ITT + 1
*     IF (ITT.GE.3) STOP
      IF (spin1.LT.0.d0.OR.spin2.LT.0.d0) THEN
         if(rank.eq.0)
...
*     STOP
      ...
      ITRY = 0
*     
      IF ((DS10.LT.0.d0.AND.DS1.GT.0.01d0).OR.
     &     (DS20.LT.0.d0.AND.DS2.GT.0.01d0)) THEN
         ITER = ITER + 1
         nstep = nstep + 50
         IF (ITER.GT.1) DT = 0.5d0*DT
 ...
 45      FORMAT (' LIMIT    SYNCH    # DS10, DS1, DS20, DS2, DT ',
     &        I4,1P,5E10.2)
      END IF
*     
      IF ((DS1.GT.0.d0.AND.DS10.LT.-0.01d0).OR.
     &     (DS2.GT.0.d0.AND.DS20.LT.-0.01d0)) THEN
         ITER = ITER + 1
         nstep = nstep + 50
         IF (ITER.GT.1) DT = 0.5d0*DT
         IF (ITER.LT.4) GO TO 40
...
*     Prevent new SEMI < R.
      IF (H(IPAIR) + DH.LT.-0.5d0*BODY(I)/R(IPAIR)) THEN
         DH = -0.5d0*BODY(I)/R(IPAIR) - H(IPAIR)
      END IF
*     
...
*     Correct for small eccentricity drift.
      IF (ABS(ECC - ES0).GT.0.001d0*ES0) THEN
...
*     
 ...
 80      FORMAT (' DANGER!    NAM E # dtau DA/A DC1 DC2 ',
     &        I6,F8.4,I5,1P,5E10.2)
      ...
      END IF
*     
      IF (ICOUNT.LE.100.AND.ABS(DS1).GT.0.01d0) THEN
         ICOUNT = ICOUNT + 1
         DT = TEV(I) - TIME
         if(rank.eq.0)
 ...
 85      FORMAT (' SYNCH    NM K* A DA/A om s1 s2 DT ',
     &        I8,2I4,F8.2,1P,5E10.2)
      ...
      END IF
*     
      IF (spin1.LT.0.d0) THEN
         if(rank.eq.0)
     &        WRITE (6,90)  NAME(J1), KSTAR(J1), spin1, TIME-TIME0
 90      FORMAT (' DANGER!    NEGATIVE SPIN    NM K* s1 T-T0 ',
     &        I6,I4,1P,2E10.2)
         IF (SPIN1.LT.0.d0) SPIN1 = 0.1d0*omeq
         IF (SPIN2.LT.0.d0) SPIN2 = 0.1d0*omeq
         ITRY = ITRY + 1
