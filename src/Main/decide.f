...
*       Adopt termination interval 1/2 of predicted t_{circ} (at const R*).
          DT1 = 0.5d0*(TC1 + 0.1d0)/TSTAR
          IF (ECC.LT.0.1d0.AND.MAX(KSTAR(I1),KSTAR(I2)).LE.1) THEN
              DT1 = 10.d0*DT1
          END IF
...
*       Compare eccentricity derivatives and check t_{circ} (standard case).
          IF ((ABS(EDT).GT.ABS(EDAV)).AND.(TC1.LT.50.d0)) THEN
...
*       Transform to exact apocentre unless done already.
          IF (ABS(TDOT2(IPAIR)).GT.1.0D-12) THEN
              IF (R(IPAIR).GT.SEMI.AND.TDOT2(IPAIR).LT.0.d0) THEN
                  CALL KSAPO(IPAIR)
              END IF
              CALL KSPERI(IPAIR)
              CALL KSAPO(IPAIR)
