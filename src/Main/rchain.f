...
*       Update smallest moment of inertia during forward integration.
          IF (TIME4.GT.TIME0.AND.JC.EQ.0) THEN
              ...
              ZMI2 = ZMI3
              ZMI3 = ZMI
              RUNAV = 0.9d0*RUNAV + 0.1d0*R(IMIN)
...
*       Check the termination criterion.
          ...
          IF (R(IM2).GT.RMAX4.OR.TIME4.GT.TMAX) THEN
              IF (R(IMIN).LT.0.01d0*RUNAV) GO TO 50
              IQ = -1
              GO TO 100
          END IF
*
...
*       Check rare case of zero regularized step.
          IF ((ISTEPS.LT.3).AND.(ABS(DS)<1.D-14)) DS = 1.0d-2*DSO
          IF (ABS(DS)<1.D-14) GO TO 120
   50 CONTINUE
*
  100 RETURN 
*
