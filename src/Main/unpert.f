          ...
          IF (KZ(27).GT.0) THEN
              IF (KSTAR(I).LT.11) THEN
                  IF (STEP(I1).LT.TK) THEN
                      IF (STEP(I1).LT.0.0001d0*STEP(I)) GO TO 9
                  END IF
                  IF (TIME - T0(I).GT.2.d0*STEP(I1)) THEN
...
*       Compare pericentre and effective capture distance.
          ECC2 = (1.d0 - R(IPAIR)/SEMI)**2 +
     &                                    TDOT2(IPAIR)**2/(BODY(I)*SEMI)
          ECC = SQRT(ECC2)
          RP = SEMI*(1.0D0 - ECC)
          RT = 4.d0*MAX(RADIUS(I1),RADIUS(I1+1))
*
*       Specify circularization index (skip SPIRAL but include CHAOS).
          ...
          IF ((KZ(27).EQ.1).AND.(TTOT.GT.TCALL)) THEN
              IF (RP.LT.RT) ICIRC = 1
              TCALL = TTOT + 0.01d0
          ELSE IF ((RP.LT.2.5d0*RT).AND.(KSTAR(I).EQ.0).AND.
     &        (TTOT.GT.TCALL)) THEN
...
*         IF (RP.LT.0.99*RT.AND.ECC.GT.0.002) THEN
          IF ((ICIRC.GT.0).AND.(ECC.GT.0.002d0)) THEN
...
*       Assign one Kepler period for active state and impose TDOT2 > 0.
              IF ((R(IPAIR).LT.0.99d0*RT).AND.(KSTAR(I).NE.10)) THEN
                  STEP(I1) = TK
                  TDOT2(IPAIR) = 1.0D-20
              ELSE
...
*       Note next unperturbed check at apocentre since T'' < 0 in KSAPO.
                  IF (((ABS(RT - RP)/RT).GT.0.1d0).AND.KZ(27).LE.1) THEN
                      if(rank.eq.0)
     &                WRITE(6,25)  ECC, SEMI, R(IPAIR), RP, RADIUS(I1)
   25                 FORMAT (' INACTIVE PHASE    E A R RP R* ',
     &                                            F7.3,1P,4E10.2)
                  END IF
                  IF ((ECC.LT.0.002d0).AND.(SEMI.LT.0.01d0*RMIN)) THEN
                      KSTAR(I) = 10
                      TEV(I) = TIME
...
*       Perform general two-body collision test.
   ...
   26     CONTINUE
          ...
          IF (QPERI.LT.(0.75d0*(RADIUS(I1) + RADIUS(I1+1)))) THEN
...
*       Check merger condition before continuing (skip small Roche steps).
   28 IF (KZ(15).GT.0.AND.IR.GE.0.AND.TIME+STEP(I1).GT.TBLOCK) THEN
          IF (STEP(I).LT.DTMIN) THEN
              CALL IMPACT(I)
          ELSE IF (JCL.GT.0.AND.STEP(I).LT.10.d0*DTMIN) THEN
...
*       Include a more generous test for massive quadruples.
          ELSE IF (JCL.GT.N) THEN
              FAC = 2.d0*(BODY(I) + BODY(JCL))/BODYM
