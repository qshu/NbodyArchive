...
*       Evaluate the general stability function (Mardling, Cambody 2008).
      IF (ECC1.LT.1.d0.AND.ECC.LT.1.d0) THEN
...
*       Obtain maximum inner eccentricity (Douglas Heggie, Sept. 1995).
      ...
      Z2 = MAX(Z2,0.0D0)
      EMAX = (Z + 1.d0 - 4.d0*A**2 + SQRT(Z2))/6.d0
      EMAX = MAX(EMAX,0.0001D0)
      EMAX = SQRT(EMAX)
*
*       Check hierarchical stability condition for bound close pair (RB > a).
      ITERM = 0
      ALPHA = 180.d0*ALPHA/3.14159265359d0
      IF (PMIN.GT.PCRIT.AND.SEMI.GT.0.d0.AND.SEMI1.GT.0.d0.AND.
     &    RB.GT.SEMI) THEN
          IF (RDOT3.GT.0.d0.AND.R3.GT.3.d0*SEMI*(1.d0 + ECC1)) THEN
          ITERM = -1
          if(rank.eq.0)
...
*       Include termination test for wide triple system (exclude ECC1 > 0.9).
      ELSE IF (PMIN.GT.3.d0*SEMI*(1.d0 + ECC).AND.SEMI.GT.0.d0.AND.
     &         ECC1.LT.0.9d0) THEN
*       Wait for favourable configuration (R > SEMI, R3 > 3*SEMI & RDOT3 > 0.
          IF (RB.GT.SEMI.AND.R3.GT.SEMI1.AND.RDOT3.GT.0.d0) THEN
              APO = SEMI*(1.d0 + ECC)
              if(rank.eq.0)
...
*       Enforce termination of long-lived configuration using two limits.
      ELSE IF (NSTEP1.GT.50000.AND.PMIN.GT.0.9d0*PCRIT.AND.
     &    R3.GT.SEMI1) THEN
          ITERM = -1
