...
*       Use negative DTU and treat STEP as positive (not used elsewhere).
              STEP(I1)=((ONE6*TDOT3(IPAIR)*DTU + 0.5d0*TDOT2(IPAIR))*DTU
     &                                                   + R(IPAIR))*DTU
...
*       Determine the regularized step by Newton-Raphson iteration (DT > 0).
   20     Y0 = DT - ZMOD*((ONE6*TDOT3(IPAIR)*DTU +
     &                           0.5d0*TDOT2(IPAIR))*DTU + R(IPAIR))*DTU
          YPR =-((0.5d0*TDOT3(IPAIR)*DTU + TDOT2(IPAIR))*DTU + R(IPAIR))
          YPR = ZMOD*YPR
          DTU = DTU - Y0/YPR
          DT1 = ((ONE6*TDOT3(IPAIR)*DTU + 0.5d0*TDOT2(IPAIR))*DTU +
     &                                                     R(IPAIR))*DTU
          DT1 = ZMOD*DT1
          ITER = ITER + 1
          IF ((ABS(DT - DT1).GT.1.0D-10*STEP(I1)).AND.(ITER.LT.5)) 
     &      GO TO 20
*
...
*       Specify transformation coefficients (Seppo Mikkola's procedure).
      IF (ZETA.GE.0.d0) THEN
          XC = SQRT(0.5D0 + 0.5D0*ZETA/ECC)
          YS = PSI/(ECC*XC*SQRT(BODY(ICM)))
      ELSE
*       Employ well behaved expressions for R > A (SM 29/5/97).
          XC =0.5d0*ABS(PSI)/(SQRT(SEMI)*ECC)/SQRT(0.5D0-0.5D0*ZETA/ECC)
*       Avoid division by small XC near apocentre (ZETA < 0 only).
          YS = 2.d0*SQRT(SEMI/BODY(ICM)*(0.5D0 - 0.5D0*ZETA/ECC))
          IF (PSI.LT.0.d0) YS = -YS
      END IF
*
      ZZ = BODY(ICM)/(4.d0*SEMI)
      R(IPAIR) = 0.0D0
      TDOT2(IPAIR) = 0.0D0
*
...
*       Predict c.m. coordinates & velocities.
      IF (ABS(DT).LT.STEP(ICM).AND.TIME.GT.0.d0) THEN
