...
*       Adopt low order for standard single particles (I1 <= N & NNB = 0).
      IF (NNB.EQ.0) THEN
         IF(ABS(TVPRED(I1)-TIME)<1.D-14) GO TO 40
          I = I1
          S = TIME - T0(I)
...
c$$$      end if
*     --09/25/13 19:31-lwang-end----------------------------------------*
          IF(ABS(TVPRED(I)-TIME)<1.D-14) GO TO 5
          S = TIME - T0(I)
...
*       Distinguish between low and high-order prediction of U & UDOT.
              IF (GAMMA(JPAIR).GT.1.0D-04) ZZ = 0.d0
...
*      IF ((DT.EQ.0.0D0.AND.IPHASE.LT.4).OR.BODY(I).EQ.0.0D0) GO TO 35
      IF ((BODY(I))<1.D-14) GO TO 35
      IF ((ABS(DT)<1.D-14).AND.(IPHASE.LT.4)) THEN
         IF ((ABS(X(1,I)-X0(1,I))<1.D-14)
     &    .AND.(ABS(XDOT(1,I)-X0DOT(1,I))<1.D-14)) go to 35
         X(1,I) = X0(1,I)
         X(2,I) = X0(2,I)
         X(3,I) = X0(3,I)
         XDOT(1,I) = X0DOT(1,I)
...
*
      DO 30 K = 1,3
          FK = ((ONE6*D3R(K,I)*A3 + 0.5d0*D2R(K,I))*A3 + D1R(K,I))*A3
     &                                              + D0R(K,I) + D0(K,I)
          F1DOTK = (D3R(K,I)*A4 + D2R(K,I))*A3 + D1R(K,I) + D1(K,I)
          F2DOTK = 0.5d0*(D3R(K,I)*A3 + D2R(K,I) + D2(K,I))
          F3DOTK = ONE6*(D3R(K,I) + D3(K,I))
          X(K,I) = ((((F3DOTK*A1 + ONE12*F2DOTK)*DT +
     &                 ONE6*F1DOTK)*DT + 0.5d0*FK)*DT + X0DOT(K,I))*DT +
     &                   X0(K,I)
          XDOT(K,I)  = (((F3DOTK*A2 + ONE3*F2DOTK)*DT +
     &                            0.5d0*F1DOTK)*DT + FK)*DT + X0DOT(K,I)
