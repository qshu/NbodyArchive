...
*       Evaluate old separation, square regularized velocity, t'' & ECC.
      ...
      DO 10 K = 1,4
          ...
          TD2 = TD2 + 2.d0*U(K,JPAIR)*UDOT(K,JPAIR)
   10 CONTINUE
...
*       Check for possible circularization (tidal or sequential).
      RP = SEMI2*(1.d0 - ECC)
      IF (KZ(27).GT.0.AND.RP.LT.4.d0*RADIUS(J).AND.ECC.GT.0.002d0) THEN
          TC = 1.0D+10
          IF (KZ(27).EQ.2) THEN
              J1 = 2*JPAIR - 1
              J2 = J1 + 1
...
*       Form KS coordinate & velocity scaling factors (general point is OK).
      SEMI2 = -0.5d0*ZMB/H(JPAIR)
      C2 = SQRT(SEMI2/SEMI0)
      V2 = 0.5d0*(ZMB + H(JPAIR)*RI*(SEMI2/SEMI0))
      C1 = SQRT(V2/V20)
*
