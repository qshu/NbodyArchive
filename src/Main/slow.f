...
*       Switch to #I2 if second binary is smaller or first pair is not bound.
              IF (SEMI2.GT.0.d0) THEN
                  IF (SEMI2.LT.SEMI.OR.SEMI.LT.0.d0) THEN
                      I1 = I2
                      SEMI = SEMI2
                      RM = RINV(I2)
                  END IF
*       Exit in case of two hyperbolic two-body motions.
              ELSE IF (SEMI.LT.0.d0) THEN
                  GO TO 100
              END IF
          END IF
*
...
*       See whether a closer particle pair is present (factor of 2).
          ...
          DO 60 I = 1,N-1
              RI2 = 1.d0/RINV(I)
              IF (I.NE.I1.AND.R1.LT.0.5d0*RI2) THEN
                  I2 = I
                  IF (R1.LT.0.1d0*SEMI.AND.RI2.LT.0.1d0*SEMI) THEN
                      GO TO 100
                  END IF
*       Compare closest separation with current slow-down binary.
                  IF (RI2.LT.0.5d0*SEMI.AND.R1.GT.0.5d0*SEMI) THEN
*       Evaluate semi-major axis directly (cf. small RI2 in EREL).
   ...
   40                 CONTINUE
                      ...
                      SEMI = 1.0D0/SEMI
                      GO TO 80
                  ELSE IF(RI2.LT.MIN(SEMI,R1).AND.R1.GT.0.5d0*SEMI) THEN
*       Determine second binary by regular expression (R1 not too small).
   ...
   50                 CONTINUE
                      ...
                      IF (SEMI2.GT.0.d0.AND.SEMI2.LT.SEMI) THEN
                          KSNEW = 1.0D0
                          GO TO 90
                      ELSE
...
*       Form relative perturbation at maximum apocentre.
      rap = 2.d0*SEMI
      pert = 2.d0*sum*rap**3/(mc(i1) + mc(i1+1))
*
...
*       Check slow-down switch and include any change in binding energy.
   90 if (ABS(ksnew-ksch(i1))>1.D-14) then
          if (ABS(ksnew-1.0d0)<1.D-14) then
              KSLOW = .false.
          else
              KSLOW = .true.
          end if
*       Add change in binary energy and save new slow-down index.
          ...
          DEB = eb*(1.d0/ksnew - 1.d0/Ksch(i1))
          Ksch(i1) = ksnew
      else
          DEB = 0.d0
      end if
...
*       Form modified mass factors.
      ...
      do i = 1,n-1
          if (ABS(Ksch(i)-1.0d0)>1.D-14) then
              tk1(i) = tk1(i)/Ksch(i)
