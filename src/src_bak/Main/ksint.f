...
c$$$                  SEMI = -0.5D0*BODY(I)/H(IPAIR)
c$$$                  ECC2 = (1.0 - R(IPAIR)/SEMI)**2 +
c$$$     &                                   TDOT2(IPAIR)**2/(BODY(I)*SEMI)
c$$$                  RP = SEMI*(1.0 - SQRT(ECC2))*(1.0 - 2.0D0*GAMMA(IPAIR))
...
c$$$                          IF ((TIME - TOSC(K))*TSTAR.GT.10.D0) THEN
c$$$                              T0(I1) = TIME
c$$$                              GO TO 90
c$$$                          END IF
c$$$                      END IF
...
*       Check termination of hyperbolic encounter (R > R0 or R > 2*RMIN).
   20 IF (HI.GT.0.D0.AND.NAME(I).GT.0) THEN
          IF ((RI.GT.R0(IPAIR).AND.GI.GT.GMAX).OR.RI.GT.2.0D0*RMIN.OR.
     &        (GI.GT.0.5D0.AND.TD2.GT.0.D0)) THEN
*       Skip termination delay in case of velocity kick (cf. routine KSTERM).
              IF (HI.LT.100.D0.OR.GI.GT.0.1.OR.RI.GT.5.0*RMIN) THEN
                  IQ = .TRUE.
              END IF
          END IF
      END IF
...
*
*       Include convergence criterion DH = H'*DTU + H''*DTU**2/2 = 0.D001*|H|.
      IF (GI.GT.1.0D-04) THEN
          DH = 1.0E-03*MAX(ABS(H(IPAIR)),ECLOSE)
          XF = 2.0D0*DH/ABS(HDOT2(IPAIR))
...
c$$$         call flush(6)
c$$$      end if
c$$$      if(step(i1).lt.1/2.0D0**25.and.step(i1).gt.0) then
...
*              IF (GI.GT.0.1.AND.RI.GT.RMIN) IQ = .TRUE.
              ...
              IF (MIN(BODY(I1),BODY(I2)).LT.0.D05*BODYM) THEN
                  IF (GI.GT.2.0D-04) IQ = .TRUE.
              END IF
          ELSE
              IF (TD2.GT.0.D0.AND.(GI.GT.GMAX.OR.RI.GT.RMIN)) IQ = .TRUE.
          END IF
          IF (.NOT.IQ) GO TO 60
      END IF
*
*     For massive black hole ks, try to avoid terminated with large seperation
      IF(KZ(24).EQ.1.AND.BODY(I)/BODYM.GT.200.AND.RI.GT.10.D0*RMIN) THEN
         IQ = .true.
      END IF
...
*     IF (RI.GT.R0(IPAIR)) THEN
      IF (RI.GT.R0(IPAIR).AND.RI.GT.2.0D0*RMIN.AND..NOT.IQ) THEN
...
*       End integration cycle for hyperbolic motion.
   60 IF (HI.GE.0.D0) THEN
          IF (RDOT*TD2.LT.0.D0) THEN
...
*     K = KZ(4)
*     DO 65 L = 1,K
*         IF ((G0 - GPRINT(L))*(GI - GPRINT(L)).LE.0.D0) THEN
*             IF (L.EQ.1) THEN
*                 W1 = -0.5D0*BODY(I)/HI
*                 W2 = W1*BODYIN
*                 TK = TWOPI*W1*SQRT(W2)
...
*  70 IF (GI.LT.GMIN.AND.TD2.LT.0.D0) THEN
*       Skip apocentre position itself.
*         IF (RDOT.LT.0.D0.AND.IMOD.EQ.1) THEN
...
*       Determine new perturbers for binary at apocentre turning point.
   70 IF (RDOT*TD2.GE.0.D0) GO TO 100
      SEMI = -0.5D0*BODY(I)/HI
*
*       Check minimum two-body separation just after pericentre.
      IF (RDOT.LT.0.D0) THEN
*       Obtain pericentre by Mikkola's algorithm (GAMMA < 0.D001).
          IF (GI.LT.0.D001) THEN
...
*       Update termination length scale in case of initial soft binary.
          ...
          IF (EB.LT.EBH) R0(IPAIR) = MAX(RMIN,2.0D0*SEMI)
      ELSE 
          ECC2 = (1.0 - RI/SEMI)**2 + TDOT2(IPAIR)**2/(BODY(I)*SEMI)
          ECC = SQRT(ECC2)
          RP = SEMI*(1.0 - ECC)*(1.0 - 2.0D0*GI)
*       Find merger index.
          IM = 0
          DO 72 K = 1,NMERGE
...
*       Increase tolerance near sensitive stability boundary (RM 10/2008).
              IF (EOUT.GT.0.90) THEN
                  DE = 0.5D0*(1.0 - EOUT)
                  DE = MIN(DE,0.D01D0)
*       Add extra amount 0.D011 to avoid switching.
                  IF (ECC.GT.0.9) DE = DE + 0.D011
                  EOUT = EOUT - DE
              END IF
...
*       Produce diagnostics for any circularizing perturbed binary.
      IF (KSTAR(I).EQ.-2.AND.GI.GT.0.D01) THEN
          ECC = RI/SEMI - 1.0
