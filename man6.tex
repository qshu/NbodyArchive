...
In this connection note that currently the {\ZZ{GRAPE-6}} code {\ZZ{NBODY4}}
...
On the other hand, {\ZZ{NBODY6}} contains new procedures for a general 3D
galactic potential.
...
routines and the structure of the old code {\ZZ {NBODY5}} are included in
...
correction factor.
We also mention that the code {\ZZ {NBODY4}} includes an averaging method
...
%criterion has been implemented in the latest code version.
%Note that the inclination is now evaluated in radians
%(\cf routine {\tt inclin.f}).
...
envelope evolution, as in {\ZZ {NBODY4}}, although the evolutionary
...
whereas {\ZZ {NBODY4}} also treats continuous circularization
