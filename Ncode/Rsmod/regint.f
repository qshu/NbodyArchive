...
*       Take volume between inner and outer radius equal to basic sphere.
    1 RCRIT2 = 1.59*RS2
      RCRIT2 = MAX(RCRIT2,RS2*(BODY1/BODYM)**(1.0/3.0))
      RCRIT2 = MIN(RCRIT2,10.0D0)
...
*
          FACM = (BODY(J)/BODYM)**(1.0/6.0)
          IF (RIJ2.GT.FACM*RS2) THEN
              GO TO 8
*       Retain particles with small steps (large derivative corrections).
*         IF (RIJ2.GT.RS2*FACM.AND.STEP(J).GT.10.0*SMIN) THEN
...
*
          FACM = (BODY(J)/BODYM)**(1.0/3.0)
          IF (RIJ2.GT.FACM*RS2) GO TO 34
