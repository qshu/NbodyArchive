...
*       Save the original stellar types and initialize coalescence flag.
          ...
          KW2 = KSTAR(J2)
          COALS = .FALSE.
*         write(3,*)' start roche ',name(j1),name(j2),kw1,kw2,
...
*
         ...
         MASS(1) = 0.D0
         KW1 = KSTAR(J2)
*       See whether Chandra's mass is exceeded (SN).
...
*           IF(IGR.LT.10.OR.ABS(DJGR)/JORB.GT.0.001)THEN
            IF(SEP.LT.1.1*(RAD(1) + RAD(2)))THEN
...
*
      WRITE (6,882)  ECC, FAC
  882 FORMAT (' CHECK!    E FAC  ',F10.6,F8.3)
         IF(ECC.GT.0.001D0.OR.FAC.LT.0.98D0)THEN
...
* Allow eccentricity to change by 10% at most with a minimum of 0.01.
                  DTM0 = 0.1D0*ECC/ABS(DELET)
                  DTMMIN = 0.01D0/ABS(DELET)
                  TC = ECC/ABS(DELET)
                  TC = TC/1.0D+06
                  TTID = TC
               ELSE
...
* Allow jspin to change by 30% at most.
               DTM0 = MIN(DTM0,0.3D0*JSPIN(1)/ABS(DJSPIN(1)))
...
*
  ...
  711       FORMAT(' NEW PARAMS    ECC SPIN1/OORB A P R1 RL1 ',
     &                             F7.3,F9.3,F8.2,1P,3E10.2)
      SPIN0 = SPIN(J1)
            SPIN(J1) = JSPIN(1)/SPNFAC
            SPIN(J2) = JSPIN(2)/SPNFAC
      WRITE (6,889)  SPIN0, SPIN(J1)
  889 FORMAT (' SPINS   ',1P,2E10.2)
