...
*
      ixxxx = 0
      if(iq.ne.0)ixxxx=1
      if(iq.eq.2.or.iq.eq.-1)step(ifirst) = step(ifirst)/2.d0
...
*     if(ixxxx.gt.0)then
*     ixxxx = ixxxx + 1 
*     print*,' time,iq,icall,ifirst=',time,iq,icall,ifirst,ifirst+1
*     print*,' steps=',step(ifirst),step(ifirst+1)
*     print*,' timenw=',timenw(ifirst),timenw(ifirst+1)
*     end if
*
      ICALL = ICALL + 1
...
*
*     if(ixxxx.gt.0)then
*     print*,' nxtlen,block=',nxtlen,(nxtlst(k),k=1,nxtlen)
