...
*       Delay Kozai cycle for EMAX > 0.9 and TC > 2000.
      ...
      END IF
*
*       Ignore unfavourable angles (outside 30 degrees).
      ALPH = 360.0*ZI/TWOPI
*     DECC = EMAX - ECC
*     IF (ABS(ALPH-90.0).GT.30.0.AND.DECC.GT.0.05) THEN
*         IQ = 2
*         GO TO 40
*     END IF
*
...
*       Obtain quadrupole and tidal constants for each new binary.
      ...
      IF (NAME(I).NE.NAMEI.OR.DTM.LE.DT0) THEN
          ...
          DEDT = 0.0
          DTPREV = 1.0
*       Define termination time for marginal stability (cf. TSTAB).
          PCRIT = stability(CM(1,IM),CM(2,IM),BODY(JCOMP),ECC,ECC1,ZI)
          PCRIT = PCTIT*SEMI
