...
*       Produce special diagnostics for new binaries (unit 8).
     ...
     &              (X(3,I) - RDENS(3))*XDOT(3,I))/SQRT(RI2)
              K = 0
              IF (NAME(I).LT.0) K = -1
              GM = GAMMA(J)*(SEMI*(1.0 + SQRT(ECC2))/R(J))**3
              WRITE (8,40)  TIME, NAME(J1), NAME(J2), LIST(2,J2), K,
     &                      BODY(J1), BODY(J2), EB, SEMI, SQRT(ECC2),
     &                      GM, SQRT(RI2), VR
   40         FORMAT (' T =',F7.1,'  NAME = ',2I5,2I3,'  M =',2F8.4,
     &                       '  EB =',F10.5,'  A =',F8.5,'  E =',F5.2,
     &                      '  GM =',F6.3,'  RI =',F6.2,'  VR =',F4.1)
              CALL FLUSH(8)
          END IF
   50 CONTINUE
*
*       Set fractional gain of binding energy (initial energy = -0.25).
      IF (TIME.LE.0.0D0.AND.NPAIRS.EQ.0) THEN
          JOR = NBIN0
          DB = 0.0
      ELSE
          E(9) = EMERGE
          DB = -4.0*(E(1) + E(2) + E(5) + E(7) + E(9) + ECOLL - EBIN0)
      END IF
*
*       Print eccentricity & energy distributions.
     ...
     &              (JD(J),J=1,JLAST)
   60 FORMAT (/,' OR =',I4,'  EX =',I3,'  DB =',F16.3,'  SB =',F8.4,
...
*
   ...
   65 FORMAT (' <E> =',F5.2,'  EMAX =',F6.3,'  NPOP =',I4,I3,2I6,I4,3I3,
     &                                                 '  EB/KT =',14I4)
*
*       Obtain total energy budget by summing all components.
   70 ETOT = 0.0D0
      DO 80 J = 1,10
          ETOT = ETOT + E(J)
   80 CONTINUE
*
   ...
   90 FORMAT (' ENERGIES   ',10F15.5,'  ETOT =',F15.6)
*
      RETURN
*
      END
