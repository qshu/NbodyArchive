      SUBROUTINE CMFIRR(I,I1,NNB,KLIST,FIRR,FD)
*
*
...
*       Set perturber number and KS indices.
      ...
      BODYIN = 1.0/BODY(I)
*
*       Choose original perturber list for NP << neighbour number.
      LX = NP
*       Note KLIST(1) = 0 called from NBINT/P points to perturber list.
      IF (KLIST(1).GT.0) THEN
          NEWNB =  NNB/2
          IF (NP.GT.NEWNB) LX = NNB
      ELSE
          NEWNB = 0
      END IF
*
*       Form irregular force components due to perturbed KS pairs.
      DO 50 LL = 2,LX+1
          IF (KLIST(1).EQ.0.OR.NP.LT.NEWNB) THEN
*       Use perturber list instead of new neighbour list also for NP << NNB.
              J = LIST(LL,I1)
          ELSE
              J = KLIST(LL)
   10         IF (J.LT.JP) GO TO 50
              IF (J.GT.JP) THEN
*       Exit if no more perturbers.
                  IF (KP.GT.NP + 1) GO TO 60
                  KP = KP + 1
                  JP = LIST(KP,I1)
                  GO TO 10
              END IF
