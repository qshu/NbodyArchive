...
*       Count higher-order systems.
    ...
    2     CONTINUE
          if(rank.eq.0)
     &    WRITE (13,3)  NPAIRS, NRUN, N, NC, NMERGE, MULT, NEWHI, TTOT
...
*       Determine merger & ghost index (delay ghost c.m.).
     ...
     &                                 TDOT2(JPAIR)**2/(BODY(ICM)*SEMI1)
          END IF
*
*       Consider any double hierarchies first (c.m. mass > 0 also here).
...
*       Obtain inclination between inner relative motion and outer orbit.
   ...
   15         CONTINUE
              ...
              PCRIT = stability(M1,M2,M3,E0,E1,ANGLE)*SEMI
              NAM2 = NAME(J)
*       Perform stability check.
              IF (PMIN*(1.0 - GAMMA(JPAIR)).LT.PCR.AND.E1.LT.0.96) THEN
                  ISTAB = JPAIR
          ANGLE = ANGLE*180.0/3.14159
                  if(rank.eq.0) 
     &            WRITE (6,20)  NAME(J1), NAME(J), ANGLE, E1, PMIN,
     &                          PCR, GAMMA(JPAIR), R0(JPAIR)
   20             FORMAT (' MERGE UNSTAB    NAM INC E1 PM PCR G R0 ',
     &                                      2I6,F7.3,F7.3,1P,4E10.2)
              END IF
