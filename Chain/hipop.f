...
*       Select semi-major axis from uniform distribution in log(A) or SEMI0.
   ...
   10     IF (RANGE.GT.0.0) THEN
          ...
          END IF
*
*       Check stability criterion (maximum of 10 tries; zero inclination).
          PCRIT = stability(BODY(I1),BODY(I2),BS(I),ECC,ECC1,0.0D0)
          PCRIT = PCRIT*SEMI
          ITER = ITER + 1
          IF (PMIN.LT.PCRIT.AND.ITER.LT.10.AND.SEMI.LT.SEMI0) GO TO 10
*
