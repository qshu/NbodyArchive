...
!!!!!!!!!!!!!!!!!
!       IF((IIIv/1000)*1000.EQ.IIIv) THEN
!        WRITE(*,222)'PETUR',TIME,K,SQRT(RIJ2),
!&SQRT(XDOT(1,K)**2.0+XDOT(2,K)**2.0+XDOT(3,K)**2.0)
!  222   FORMAT(A5,F25.12,I6,2F35.25)
!	END IF
!*
!!!!!!!!!!!!!!!!!
          A1 = X(1,K) - XI(4)
...
*       See whether c.m. approximation applies (ignore unperturbed case).
          J = K - N
          IF (RIJ2.GT.rs2fac*CMSEP2*R(J)**2) THEN
              V1 = XDOT(1,K) - VI(1)
              V2 = XDOT(2,K) - VI(2)
              V3 = XDOT(3,K) - VI(3)
              A9 = 3.0D0*(A1*V1 + A2*V2 + A3*V3)/RIJ2
