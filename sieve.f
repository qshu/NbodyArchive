From sverre@mail.ast.cam.ac.uk Thu Nov 17 12:47:34 1994
Return-Path: <sverre@mail.ast.cam.ac.uk>
Received: from cast0.ast.cam.ac.uk by pluto.astrophysik.uni-kiel.de (4.1/SMI-4.1)
	id AA14010; Thu, 17 Nov 94 12:47:31 +0100
Received: from ioas21.ast.cam.ac.uk [131.111.68.72] 
	by cast0.ast.cam.ac.uk with smtp 
	(Smail-3.1.28.1) id m0r85Io-0001XFC; Thu, 17 Nov 94 11:46 GMT
Received: by ioas21.ast.cam.ac.uk 
	(Smail-3.1.28.1) id m0r85Io-0000MhC; Thu, 17 Nov 94 11:46 GMT
Message-Id: <m0r85Io-0000MhC@ioas21.ast.cam.ac.uk>
Date: Thu, 17 Nov 94 11:46 GMT
From: sverre@mail.ast.cam.ac.uk (Sverre Aarseth)
To: supas028@pluto.uni-kiel.de
Subject: routine sieve.f
Status: RO
...
*       Check all mutual c.m. distances for overlap (inner loop to LL - 1).
      DO 70 LL = 2,NCBLK
          J = ICBLK(LL)
          JPAIR = J - N
*       Set maximum size using (small) apocentre distance or R0 & 2*RMIN.
          APO = -BODY(J)/H(JPAIR)
          IF (APO.GT.0.0.AND.APO.LT.0.2*RMIN) THEN
              RJ = APO
          ELSE
              RJ = MAX(R0(JPAIR),2.0*RMIN)
          END IF
*
...
*       Compare c.m. separation with overlapping distance.
              IF (RIJ2.LT.RS2) THEN
                  ...
                  DO 40 L = 2,NP1
                      JP = LIST(L,J1)
*       Split search into two loops (first list and any subsequent).
   20                 DO 25 K = K1,LAST
                          ...
                          END IF
   25                 CONTINUE
*       Skip next loop if last perturber is not contained in JPERT.
