...
*       Use previous merger index to identify ghost from earlier level.
    ...
    4     CONTINUE
          ...
          if(rank.eq.0)
     &    WRITE (6,5)  WHICH1, TIME+TOFF, NAME(2*IPAIR-1),
     &                 NAME(2*IPAIR), NAMEG(IMERGE), RI, ECC, EB, SEMI1,
     &                 PMIN, G1
    5     FORMAT (' END',A11,'   T NM1 NM2 NM3 RI E1 EB1 A1 PM G1 ',
     &                           F9.2,3I6,2F6.2,F6.1,1P,3E10.2)
      END IF
*
*       Check presence of [[B,B],S] quintuplet.
      IF (NAMEG(IMERGE).GT.0.AND.NAMEG(IMERGE).LE.NZERO) THEN
          ...
          IF (NAME(JG).GT.NZERO) THEN
              ZM = BODY(I)*SMU
              if(rank.eq.0)
     &        WRITE (6,9)  TIME+TOFF, ZM, NAME(2*IPAIR), NAME(JG),
     &                     NAME(I2)
...
*       Determine original merger indices of the two inner binaries.
   ...
   16     CONTINUE
     ...
     &                  NAMEG(JM), AI, AJ, R(IPAIR), SEMI1, PMIN, G1
   18     FORMAT (/,' END HITRIP    T MT NM AI AJ R1 A1 PM G1 ',
...
*       Copy KS variables for inner binary (small TDOT2 near apo/peri).
   ...
   30 CONTINUE
*
*       Add #JCOMP to all neighbour lists containing ICM.
      JLIST(1) = JCOMP
...
*       Include standard inclination factor in the stability criterion.
      ...
      R0(IPAIR) = YFAC*RSTAB
*
*       Determine index of closest perturber (exclude outer component).
      JMIN = 0
      NNB1 = LIST(1,I1) + 1
      RS2 = 1.0
      DO 46 L = 2,NNB1
   ...
   45     CONTINUE
          IF (RIJ2.LT.RS2.AND.J.NE.JCOMP1) THEN
              RS2 = RIJ2
              JMIN = J
          END IF
   46 CONTINUE
*
*       Obtain improved force polynomials for close perturber.
      IF (JMIN.GT.0) THEN
          T0(JMIN) = TIME
          DO 47 K = 1,3
...
*
     ...
     &                  DP
   48     FORMAT (' END OUTER MERGER',I4,'  H =',F7.1,'  M =',2F7.4,
