      SUBROUTINE FPCORR(I,NBLOSS,NBGAIN,XI,XIDOT,FIRR,FREG,FD,FDR,
     &   KLIST)
*
*
...
*
     ...
     &        A(12),F1DOT(3),F2DOT(3),F3DOT(4)
      REAL*8 FIRR(3),FREG(3),FD(3),FDR(3),FKSR(3),FDKSR(3)
*
...
*       Initialize the derivative corrections.
      DO 70 K = 1,3
          ...
          SAVE2(K) = 0.0
          SAVE3(K) = 0.0
              FKSR(K) = 0.5D0*(FREG(K) + FIRR(K))
              FDKSR(K) = ONE6*(FDR(K) + FD(K))
   70 CONTINUE
*
...
*       Accumulate derivative corrections.
   ...
   80 J = JKSRL(L)
      IF (J.LT.IFIRST) THEN
*       Use c.m. values of X, XDOT, F & FDOT for single KS components.
          JCM = N + KVEC(J)
          STEPJ = STEP(JCM)
          S = TIME - T0(JCM)
          S3 = 3.0*S
*
     ...
     &                                            X0(K,JCM) - XI(K)
              A(K+3) = (FDOT(K,JCM)*S3 + 2.0*F(K,JCM))*S + 
     &                                           X0DOT(K,JCM) - XIDOT(K)
              A(K+6) = 2.0*(F(K,JCM) - FKSR(K))
              A(K+9) = 6.0*(FDOT(K,JCM) - FDKSR(K))
   82     CONTINUE
*
      ...
      ELSE
          ...
          S3 = 3.0*S
*
*       Predict XDOT, F & FDOT of body #J to order FDOT (XDOT may be old).
          DO 84 K = 1,3
              A(K) = ((FDOT(K,J)*S + F(K,J))*S + X0DOT(K,J))*S + 
     &                                            X0(K,J) - XI(K)
              A(K+3) = (FDOT(K,J)*S3 + 2.0*F(K,J))*S + X0DOT(K,J) -
     &                                                          XIDOT(K)
              A(K+6) = 2.0*(FDOT(K,J)*S3 + F(K,J) -  FKSR(K))
              A(K+9) = 6.0*(FDOT(K,J) - FDKSR(K))
   84     CONTINUE
      END IF
*
