...
*       Absorb the perturber (single particle or binary).
          IF (KZ(30).NE.0) CALL ABSORB(ISUB)
*
...
*       Enforce escape for distant member with positive radial velocity.
     ...
     &                              + X4(3,IESC)*XDOT4(3,IESC)
      IF (NCH.EQ.4.AND.RSUM.GT.20.0*RMIN.AND.RD.GT.0.0D0) THEN
          ...
          VINF = 0.0
          IF (NSTEP1.GT.NEXT) THEN
          WRITE (6,41)  NSTEP1, IESC, JESC, GPERT,
     &                  (1.0/RINV(K),K=1,NCH-1)
   41     FORMAT (' WIDE ESCAPE    # IESC JESC G R  ',
     &                             I8,2I4,F8.4,1P,4E10.2)
          NEXT = NSTEP1 + 10
          GO TO 900         ! Ensure termination for wide system and RD > 0.
          END IF
          GO TO 40
      END IF
*
...
*       Try single escape for wide binary.
          ...
          ELSE
              ...
              ELSE
                  VINF = 0.0
              END IF
              IF (KZ(30).GT.1.OR.VINF.GT.5.0) THEN
...
*       Skip small radial velocity (0.01 circular value).
              ...
              VINF = 0.0
              RX = -BODY(ICH)/HI
              IF (RI.LT.10.0*RMIN.AND.GPERT.LT.5.0D-03) THEN
                  KCASE = 0
                  GO TO 60
              END IF
          ELSE
...
*       Ensure single body is removed in case of wide binary.
              ...
              KCASE = 1
              GO TO 900   ! Skip past complications for NN = 4.
          END IF
      ELSE
          KCASE = 0
          GO TO 60
...
*
   ...
   55         FORMAT (' REDUCING    T NCH NAM RI RD ',
...
*       Include safety limit to enforce termination.
      ...
      END IF
*
*       Enforce binary removal for dominant middle distance (NN = 4).
...
*       Repeat specification of indices in case set to zero.
          IF (ISORT(1).EQ.2) THEN    ! This condition is ensured. (SJA 10/2018)
              IF (R1.LT.R3) THEN
                  IESC = 1
                  JESC = 2
              ELSE
              ...
              END IF
          END IF
          WRITE (6,75)  KCASE, IESC, JESC, R1, R2, R3, GPERT
   75     FORMAT (' FOUR-BODY TERM!    KC IE JE R1 R2 R3 G ',
     &                                 3I4,1P,4E9.1)
      ELSE IF (NN.EQ.4.AND.((RX.GT.3.0*RMIN.AND.GPERT.GT.0.01).OR.
...
*       Check for removal of first or last distant single. (SJA 12/2016)
          ...
          IF (ISORT(1).EQ.1.OR.ISORT(1).EQ.2) THEN
              IESC = INAME(1)
              IF (1.0/RINV(1).LT.RMIN) JESC = INAME(2)
          ELSE IF (ISORT(1).EQ.NN-1) THEN
              IESC = INAME(NN)
          ELSE IF (1.0/RINV(NN-1).LT.RMIN.AND.
     &             1.0/RINV(2).GT.10.0*RMIN) THEN
              IESC = INAME(NN-1)
              JESC = INAME(NN)
          END IF
          IF (IESC.GT.0) THEN     ! Refinement 1/2017 avoids ABSORB repeats.
              RD = X4(1,IESC)*XDOT4(1,IESC) + X4(2,IESC)*XDOT4(2,IESC)
     &                                      + X4(3,IESC)*XDOT4(3,IESC)
*       Delay two similar small separations (large perturbation).
              R1 = 1.0/RINV(1)
              R2 = 1.0/RINV(2)
              R3 = 1.0/RINV(3)
              IF (ISORT(1).EQ.3) THEN
                  DR = (R1 - R2)/(R1 + R2)
                  IF (ABS(DR).LT.0.33) RD = 0.0
              ELSE IF (ISORT(1).EQ.1) THEN
                  DR = (R2 - R3)/(R2 + R3)
                  IF (ABS(DR).LT.0.33) RD = 0.0
              END IF
*       Ensure positive radial velocity for single escaper (include ABSORB).
              IF (RD.GT.0.0.OR.RX.GT.20.0*RMIN) THEN
                  KCASE = -3
                  WRITE (6,80)  KCASE, IESC, JESC, RX, GPERT, RD/RX
   80             FORMAT (' FOUR-BODY TERM!    KC IE JE RX GP RDOT ',
     &                                         3I4,1P,2E10.2,0P,F7.2)
              ELSE
                  KCASE = 0
              END IF
          END IF
...
*       Include provisional check of three-body system using perturbation.
     ...
     &                                      + X4(3,IESC)*XDOT4(3,IESC)
              IF (GPERT.GT.0.001.AND.RD.GT.0.0) THEN
                  KCASE = -2
*       Delay termination to avoid another early chain with RD > 0.
                  IF (RX.LT.25.0*RMIN.AND.GPERT.LT.0.01) KCASE = 0
              ELSE
                  KCASE = 0
              END IF
...
*       Include trigger for rare case of NN = 6 (it happened as 3*KS).
   ...
   85     FORMAT (' SIX CHAIN    IS GP R ',I4,F7.3,1P,5E10.2)
      ...
      END IF
*
*       Combine skipping inert binary for NN = 5 & 6.
      IF (NN.GE.5.AND.KCASE.LT.0) THEN
          IF (IESC.GT.0) THEN
              IF (NAMEC(IESC).EQ.NAMEC(10)) THEN
                  KCASE = 0
...
*
*       Delay for weakly perturbed higher-order systems inside 15*RMIN.
      IF (KCASE.LT.0.AND.NN.GE.4) THEN
          IF (GPERT.LT.0.001.AND.RSUM.LT.15.0*RMIN) KCASE = 0
          IF (KCASE.LT.0.AND.NN.EQ.4.AND.1.0/RINV(2).GT.5.0*RX) THEN
              IF (IESC.GT.0) THEN
                  IF (NAMEC(IESC).EQ.NAMEC(10)) THEN
                      IF (IESC.EQ.4) THEN
                          IESC = INAME(1)
