...
# make pgf			           (single CPU version with pgf-Comp.)
...
#----------------------------------------------------------------------------
# Workstation single CPU pgf77 Linux -O4 -tp p6 -pc64
#----------------------------------------------------------------------------
pgf:
	$(MAKE) $(RESULT) "FC = pgf77" "FFLAGS = -tp p6 -pc64" \
...
hipop.f histab.f hmdot.f hmdot2.f hotsys.f \
...
nbrem.f nbrest.f nbsort.f nbtide.f nlmod.f orbit.f output.f peri.f permit.f \
pfac.f quad.f ran2.f reflct.f regint.f remove.f rename.f \
...
chain.f chstab.f const.f cstab2.f cstab3.f cstab4.f cstab5.f derqp.f difsy1.f \
erel.f hpsort.f inclin.f invert.f ksphys.f physks.f qforce.f qpmod.f \
