...
LFLAGS = -o $(RESULT)
LFLAGS2 =
CUFLAGS= -arch=sm_30 -O3 -D CUDA_5
...
standard: 
	...
	$(MAKE) $(RESULT) "FC = gfortran" "FFLAGS =  -O3 -fPIC -mcmodel=medium" "SOURCE = energy.f $(SOURCE)"
gpu:
	cp mpif.null.h mpif.h
	$(MAKE) nbody6.gpu "CUDA_PATH=/usr/local/cuda" "SDK_PATH=/usr/local/cuda_sdk/C" "FC = gfortran" "LFLAGS = -o nbody6.gpu -L$(CUDA_PATH)/lib64 -lcudart" \
		"FFLAGS = -fPIC -O3 -D GPU" \
		"SOURCE = energy.f util_gpu.f  fpoly0.F $(SOURCE)"
gpu_debug:
	cp mpif.null.h mpif.h
	$(MAKE) nbody6.gpu "CUDA_PATH=/usr/local/cuda" "SDK_PATH=/usr/local/cuda_sdk/C" "FC = gfortran" "LFLAGS = -o nbody6.gpu -L$(CUDA_PATH)/lib64 -lcudart" \
		"FFLAGS = -fPIC -g -O0 -D GPU -fbounds-check -Wall -fbacktrace -finit-real=nan" \
		"SOURCE = energy.f util_gpu.f  fpoly0.F $(SOURCE)"
...
gpu_laohu:
	...
	$(MAKE) nbody6.gpu "CUDA_PATH=/export/" "SDK_PATH=/export/NVIDIA_CUDA_SDK" "FC = gfortran" "LFLAGS = -o nbody6.gpu -L/export/CUDA/4.0/lib64 -lcudart" \
		"FFLAGS = -fPIC -O3 -D GPU" "CUFLAGS= -arch=sm_13 -O3"\
		"SOURCE = energy.f util_gpu.f  fpoly0.F $(SOURCE)"

gpu_milkyway:
	cp mpif.null.h mpif.h
	$(MAKE) nbody6.gpu "SDK_PATH=" "FC = gfortran" "CUFLAGS= -D CUDA_5" "LFLAGS = -o nbody6.gpu -lcudart" \
		"FFLAGS =  -fPIC -O3 -D GPU" "CUFLAGS= -arch=sm_20 -O3"\
		"SOURCE = energy.f util_gpu.f  fpoly0.F $(SOURCE)"
...
mpich:
	rm -f mpif.h
	$(MAKE) $(RESULT) "FC = mpif77" "FFLAGS =  -O4 -D PUREMPI -D PARALLEL -fPIC " \
		"SOURCE = energy_mpi.F fpoly1_mpi.f fpoly2_mpi.f $(SOURCE)"
mpich_debug:
	rm -f mpif.h
	$(MAKE) $(RESULT) "FC = mpif77" "FFLAGS = -g -O0 -fPIC -D PUREMPI -D PARALLEL -fbounds-check -Wall -fbacktrace -finit-real=nan" \
		"SOURCE = energy_mpi.F fpoly1_mpi.f fpoly2_mpi.f $(SOURCE)"
mpich_laohu:
	rm -f mpif.h
	$(MAKE) $(RESULT) "FC = /usr/mpi/gcc/openmpi-1.3.2-qlc/bin/mpif77" "FFLAGS = -fPIC -O4 -D PUREMPI -D PARALLEL" \
		"SOURCE = energy_mpi.F fpoly1_mpi.f fpoly2_mpi.f $(SOURCE)"
...
mpich_ensemble:
	rm -f mpif.h
	$(MAKE) $(RESULT) "FC = mpif77" "FFLAGS =  -O4 -D ENSEMBLE -fPIC " \
		"SOURCE = energy_mpi.F fpoly1_mpi.f fpoly2_mpi.f $(SOURCE)"
mpichgpu:
	rm -f mpif.h
	$(MAKE) nbody6.gpu "CUDA_PATH=/usr/local/cuda" "SDK_PATH=/usr/local/cuda/samples" "FC = mpif77" "LFLAGS = -o nbody6.gpu -L$(CUDA_PATH)/lib64 -lcudart" \
		"FFLAGS = -fPIC -mcmodel=medium -O4 -D PUREMPI -D PARALLEL -D GPU" \
		"SOURCE = energy_mpi.F phicor.f fpoly0.F fpoly2_mpi.f util_gpu.f  $(SOURCE)"
mpichgpu_kepler:
	rm -f mpif.h
	$(MAKE) nbody6.gpu "CUDA_PATH=/opt/local/cuda-5.0" "SDK_PATH=/opt/local/cuda-5.0/samples" "FC = mpif77" "LFLAGS = -o nbody6.gpu -L/opt/local/cuda-5.0/lib64 -lcudart" \
		"FFLAGS = -mcmodel=medium -fPIC -O4 -D PUREMPI -D PARALLEL -D GPU" \
		"SOURCE = energy_mpi.F phicor.f fpoly0.F fpoly2_mpi.f util_gpu.f  $(SOURCE)"
...
mpichgpu_laohu:
	...
	$(MAKE) nbody6.gpu "CUDA_PATH=/export/" "SDK_PATH=/export/NVIDIA_CUDA_SDK" "FC = /usr/mpi/gcc/openmpi-1.3.2-qlc/bin/mpif77" "LFLAGS = -o nbody6.gpu -L/export/CUDA/4.0/lib64 -lcudart" \
		"FFLAGS = -mcmodel=medium -O4 -D PUREMPI -D PARALLEL -D GPU -fPIC" "CUFLAGS= -O3"\
		"SOURCE = energy_mpi.F phicor.f fpoly0.F fpoly2_mpi.f  util_gpu.f  $(SOURCE)"

mpichgpu_milkyway_g:
	rm -f mpif.h
	$(MAKE) nbody6.gpu "FC = mpif77" "SDK_PATH=" "CUFLAGS= -g -D CUDA_5" "LFLAGS = -o nbody6.gpu -lcudart" \
		"FFLAGS = -fPIC -mcmodel=medium -g -O0 -D PUREMPI -D PARALLEL -D GPU -fbounds-check -Wall -fbacktrace -finit-real=nan"\
		"SOURCE = energy_mpi.F phicor.f fpoly0.F fpoly2_mpi.f util_gpu.f  $(SOURCE)"

mpichgpu_milkyway:
	rm -f mpif.h
	$(MAKE) nbody6.gpu "FC = mpif77" "SDK_PATH=" "CUFLAGS= -arch=sm_20 -Xptxas -v -O3 -D CUDA_5" "LFLAGS = -o nbody6.gpu -lcudart" \
		"FFLAGS = -mcmodel=medium -fPIC -O4 -D PUREMPI -D PARALLEL -D GPU -Wall"\
		"SOURCE = energy_mpi.F phicor.f fpoly0.F fpoly2_mpi.f util_gpu.f  $(SOURCE)"

mpichgpu_milkyway_tau:
	rm -f mpif.h
	$(MAKE) nbody6.gpu "FC = tau_f77.sh" "SDK_PATH=" "CUFLAGS= -arch=sm_20 -Xptxas -v -O3 -D CUDA_5" "LFLAGS = -o nbody6.gpu -lcudart" \
		"FFLAGS = -fPIC -mcmodel=medium -O4 -D PUREMPI -D PARALLEL -D GPU"\
		"SOURCE = energy_mpi.F phicor.f fpoly0.F fpoly2_mpi.f util_gpu.f  $(SOURCE)"

mpichgpu_lwpc:
	rm -f mpif.h
	$(MAKE) nbody6.gpu "CUDA_PATH=/usr/local/cuda" "CUFLAGS=-arch=sm_30 -v -O3 -D CUDA_5 " "SDK_PATH=/usr/local/cuda/samples" "FC = mpif77" "LFLAGS = -o nbody6.gpu -L/usr/local/cuda/lib64 -lcudart" \
		"FFLAGS = -pg -O3 -fPIC -ffast-math -D PUREMPI -D PARALLEL -D GPU -Wall" \
		"SOURCE = energy_mpi.F phicor.f fpoly0.F fpoly2_mpi.f util_gpu.f  $(SOURCE)"
...
mpichgpu_lwpcg:
	...
	$(MAKE) nbody6.gpu "CUDA_PATH=/usr/local/cuda" "CUFLAGS=-g -arch=sm_30 -Xptxas -v -D CUDA_5 " "SDK_PATH=/usr/local/cuda/samples" "FC = mpif77" "LFLAGS = -o nbody6.gpu -L/usr/local/cuda/lib64 -lcudart" \
		"FFLAGS =  -fPIC -g -O0 -ffast-math -D PUREMPI -D PARALLEL -D GPU -fbounds-check -Wall -fbacktrace -finit-real=nan" \
		"SOURCE = energy_mpi.F phicor.f fpoly0.F fpoly2_mpi.f util_gpu.f  $(SOURCE)"
...
INC = params.h common6.h timing.h commonc.h common2.h 
...
reduce.f reinit.f renew.f setsys.f tchain.f xcpred.f xtpert.f xbpred.f \
xbpredall.f jpred.f fpoly1_ks.f fpoly2_ks.f cmfirr_cor.f cmfirr_ucor.f \
encorr_mdot.F


CUDASOURCE = gpunb.gpu.cu gpupot.gpu.cu gpupot.mdot.cu
...
start.o: energy.f energy_mpi.F phicor.f fpoly1.f fpoly2.f fpoly1_mpi.f fpoly2_mpi.f fpoly0.F

gpunb.velocity.o: gpunb.velocity.cu cuda_pointer.h cuda_share.h
	nvcc -c $(CUFLAGS) -Xcompiler "-fPIC -fopenmp -O3 -Wall" -I$(SDK_PATH)/common/inc -I. -I$(HOME)/include gpunb.velocity.cu

gpunb.gpu.o: gpunb.gpu.cu cuda_pointer.h cuda_share.h
	nvcc -c $(CUFLAGS) -Xcompiler "-fPIC -O3 -Wall" -I$(SDK_PATH)/common/inc -I. -I$(HOME)/include gpunb.gpu.cu

gpupot.gpu.o: gpupot.gpu.cu cuda_pointer.h cuda_share.h
	nvcc -c $(CUFLAGS) -Xcompiler "-fPIC -O3 -Wall" -I$(SDK_PATH)/common/inc -I. -I$(HOME)/include gpupot.gpu.cu 

gpupot.mdot.o: gpupot.mdot.cu cuda_pointer.h cuda_share.h
	nvcc -c $(CUFLAGS) -Xcompiler "-fPIC -O3 -Wall" -I$(SDK_PATH)/common/inc -I. -I$(HOME)/include gpupot.mdot.cu
