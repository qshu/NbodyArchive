...
pgfp4:
	cp mpif.null.h mpif.h
	$(MAKE) $(RESULT) "FC = pgf77" "FFLAGS = -O4 -tp p7 -pc 64 -fastsse" \
		"SOURCE = energy.f $(SOURCE)"
#----------------------------------------------------------------------------
...
mpichpgf:
	cp mpif.mpich.h mpif.h
	$(MAKE) $(RESULT) "LOAD = mpif77-pgi" "FC = pgf77" "FFLAGS = -O4 -tp p7 -pc 64 -fastsse -D PUREMPI -D PARALLEL" \
		"SOURCE = energy_mpi.f fpoly1_mpi.f fpoly2_mpi.f $(SOURCE)"
mpichpgf_ensemble:
	cp mpif.mpich.h mpif.h
	$(MAKE) $(RESULT) "LOAD = mpif77-pgi" "FC = pgf77" "FFLAGS = -O4 -tp p7 -pc 64 -fastsse -D ENSEMBLE" \
		"SOURCE = energy_mpi.f fpoly1_mpi.f fpoly2_mpi.f $(SOURCE)"
#----------------------------------------------------------------------------
...
gcinit.f gcint.f galcen.f gcheck.f gclmass.f
...
clean: 
	\rm *.o adjust.f  cloud0.f  escape.f     hotsys.f  intide.f  modify.f  output.f bindat.f  cputim.f  file_init.f  input.f   ksint.f   mydump.f  scale.f binpop.f  data.f    hipop.f      intgrt.f  lagr.f    nbody6.f  start.f gcinit.f
