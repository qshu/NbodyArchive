...
mpich_laohu:
	...
	$(MAKE) $(RESULT) "FC = /usr/mpi/gcc/openmpi-1.3.2-qlc/bin/mpif77" "FFLAGS = -O4 -D PUREMPI -D PARALLEL" \
		"SOURCE = energy_mpi.f fpoly1_mpi.f fpoly2_mpi.f $(SOURCE)"
mpich_ensemble:
	cp mpif.mpich.h mpif.rainer.h
...
mpichgpu_laohu:
	cp mpif.laohu.h mpif.rainer.h
	$(MAKE) nbody6.gpu "CUDA_PATH=/export/cuda" "SDK_PATH=/export/NVIDIA_CUDA_SDK" "FC = /usr/mpi/gcc/openmpi-1.3.2-qlc/bin/mpif77" "LFLAGS = -o nbody6.gpu -L/export/cuda/lib -lcudart" \
		"FFLAGS = -O4 -D PUREMPI -D PARALLEL -D GPU" \
		"SOURCE = energy_mpi.f fpoly1_mpi.f fpoly2_mpi.f  util_gpu.f $(SOURCE)"
#----------------------------------------------------------------------------
