...
standard: 
	...
	$(MAKE) $(RESULT) "FC = gfortran" "FFLAGS = -g -fbounds-check" "SOURCE = energy.f $(SOURCE)"
#----------------------------------------------------------------------------
...
mpichgpu:
	cp mpif.mpich.h mpif.rainer.h
	$(MAKE) $(RESULT) "FC = mpif77" "LFLAGS = $(CUDA_PATH)/lib64 -lcudart" \
		"FFLAGS = -O4 -D PUREMPI -D PARALLEL -DGPU" \
...
gpunb.gpu.o: gpunb.gpu.cu
	nvcc -I $(SDK_PATH)/common/inc gpunb.gpu.cu
