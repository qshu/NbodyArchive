...
gpu:
	cp mpif.null.h mpif.rainer.h
	$(MAKE) nbody6.gpu "FC = gfortran" "LFLAGS = -o nbody6.gpu -L$(CUDA_PATH)/lib64 -lcudart -lgomp" \
		"FFLAGS = -O3 -D GPU" \
...
gpunb.gpu.o: gpunb.gpu.cu
	/usr/local/cuda/bin/nvcc -Xcompiler "-fopenmp -O" -c -g -I$(SDK_PATH)/common/inc gpunb.gpu.cu
