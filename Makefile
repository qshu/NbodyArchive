...
# make mpichgpu                    (many CPU, many GPU, beta test)
# make mpich_laohu                 (special mpich for laohu)
# make mpichgpu_laohu              (special mpichgpu for laohu)
...
standard: 
	...
	$(MAKE) $(RESULT) "FC = gfortran" "FFLAGS = -O3 -g" "SOURCE = energy.f $(SOURCE)"
test:
	cp mpif.null.h mpif.rainer.h
	$(MAKE) $(RESULT) "FC = gfortran" "FFLAGS = -O3 -g -DVISUAL" "SOURCE = energy.f $(SOURCE)"
gpu:
	cp mpif.null.h mpif.rainer.h
