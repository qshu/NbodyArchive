...
# make pgfp4-64                    (single CPU pgf77-Comp. EM64 Pentium 4 optimised)
...
pgfp4:
	...
	$(MAKE) $(RESULT) "FC = pgf77" "FFLAGS = -O4 -tp p7 -pc 64 -fast -fastsse" \
	       "SOURCE = energy.f $(SOURCE)"
#----------------------------------------------------------------------------
# Workstation single CPU pgf77 Linux (P4-64)
#----------------------------------------------------------------------------
pgfp4-64:
	cp mpif.null.h mpif.h
	$(MAKE) $(RESULT) "FC = pgf77" "FFLAGS = -O4 -tp p7-64 -pc 64 -fast -fastsse" \
	   "SOURCE = energy.f $(SOURCE)"
#----------------------------------------------------------------------------
...
dragmco_init.f masscentrobject.f dragforce.f orbitelements.f \
dragstarpcm.f masscentrobject1.f dragforce1.f inputdisc.f smbh_adjust.f \
inflsmbh.f bhout2.F disrupt.F nbpotmbh.f \
