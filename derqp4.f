...
*
*       Specify KS index and set close pair indices for I1 & I2 or I3 & I4.
      RCOLL = 100.0
      IM0 = 1
      DO 20 IM = 1,3
          K = 1 + 4*(IM - 1)
...
*
          IF (QPERI.GT.2.7*MAX(SIZE(K1),SIZE(K2))) THEN
              QPERI1 = QPERI
              ICALL = 0
              GO TO 20
          END IF
*       Save indices for smallest pericentre and switch off indicator.
   ...
   20 CONTINUE
*
*       Save smallest pericentre and reset corresponding indices.
      QPERI = RCOLL
      IM = IM0
      K = 1 + 4*(IM - 1)
      K1 = K10
...
*       Check for tidal two-body interaction or stellar collision.
      IF (RCOLL.LT.2.7*MAX(SIZE(K1),SIZE(K2))) THEN
*
*       Determine indices of two least dominant interactions.
          IM1 = IM + 1
          IF (IM1.GT.3) IM1 = 1
          IM2 = IM1 + 1
          IF (IM2.GT.3) IM2 = 1
...
*       Convert QPR to standard KS with T' = R and form radial velocity R'.
          ...
          R3 = R3 + Q(J)**2
   25     CONTINUE
          IF (R(IM).GT.MIN(R(IM1),R(IM2))) THEN
              GO TO 50
*             IF (ICOLL.EQ.0.AND.RPR.LT.0.0) GO TO 50
          END IF
*
...
*       Begin iteration if R(IM) is smallest or wide pair is past pericentre.
          IF (R(IM).LT.MIN(R(IM1),R(IM2)).OR.RPR.GT.0.0D0) THEN
              DSFAC = 0.2*ABS(DS/DSC)
              IF (R(IM).LT.MIN(R(IM1),R(IM2))) DSFAC = 1.0
              DS = DSFAC*DSC
          ELSE
