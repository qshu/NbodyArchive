...
*
      IF(IPAIR.LT.20)THEN
       PRINT*,NTOT,' c.m.  X , XDOT ',X(1,NTOT),XDOT(1,NTOT)
       PRINT*,ICOMP,JCOMP,' X',X(1,ICOMP),X(1,JCOMP),
     &   ' XDOT ',XDOT(1,ICOMP),XDOT(1,JCOMP)
       PRINT*,' masses ',BODY(ICOMP),BODY(JCOMP)
       END IF
...
*       Check optional output for new KS.
   ...
   60     FORMAT (/,' NEW KSREG    TIME =',F8.2,2I8,F12.3,1PE10.1,
     &                                0PF7.2,F9.2,2I8,F8.3,1PE10.1,2I5)
      IF(IPAIR.LT.20)PRINT*,ICOMP,JCOMP,' X ',X(1,ICOMP),X(1,JCOMP),
     &   ' XDOT ',XDOT(1,ICOMP),XDOT(1,JCOMP)
      END IF
*
