...
*
      ...
      REAL*8  M_T,ETA_REDM,SEP,SEP_2,SEP_3
*
*
...
*
* --------------------------------------------------
* Determine if KS pair experiences a PN perturbation
* --------------------------------------------------
*
* Note that the pair is always perturbed in PN case so set-up
* of unperturbed binaries is skipped.
* See define.f for meaning of each KZ(J).
*
      IF (KZ(41).LT.5) THEN
*         IF (((KZ(42).EQ.1).OR.((KZ(42.EQ.2).AND.((NAME(ICOMP).EQ.1
*     &        .AND.NAME(JCOMP).EQ.2).OR.(NAME(ICOMP).EQ.2.AND.
*     &        NAME(JCOMP).EQ.1))).OR.(KZ(42).EQ.3.AND.(NAME(ICOMP).EQ.1
*     &        .OR.NAME(JCOMP).EQ.1)))))
*
         IF ((KZ(43).EQ.0).OR.(KZ(43).EQ.1.AND.(VOVERC.GT.BETAREL)))
     &     THEN 
*
* Report that the binary was relativistic upon KS initalization
            WRITE (*,6104) TIME+TOFF,VOVERC,NAME(ICOMP),NAME(JCOMP)
 6104       FORMAT ('Relativistic upon KS Initalization -->  Time = ',
     &           F20.8,' VOVERC = ',F20.8,' I1 = ',I8,' I2 = ',I8)
*
* Calculate an energy correction using an unphysical PN pseudo potential
            SEP_2 = Q(1)*Q(1) + Q(2)*Q(2) + Q(3)*Q(3)
            SEP = SQRT(SEP_2)
            SEP_3 = SEP*SEP_2
            M_T = BODY(ICOMP) + BODY(JCOMP)
            ETA_REDM = BODY(ICOMP)*BODY(JCOMP)/(M_T*M_T)
*
            RELENERGY1(IPAIR) = ((2.0 + ETA_REDM)*M_T*M_T*M_T
     &           /(SEP_2))*VOVERC*VOVERC
            RELENERGY2(IPAIR) = -(12.0+29.0*ETA_REDM)*M_T*M_T*M_T*M_T
     &           /(4.0*SEP_3)*VOVERC*VOVERC*VOVERC*VOVERC
           GO TO 1231
         END IF
      END IF
*
...
*
* ------------------------------------------------
* Coninue in PN case with only perturbed particles
* ------------------------------------------------
*
 1231 CONTINUE
*
...
*
   ...
   60     FORMAT (/,' NEW KSREG    TIME =',1PE10.2,2I6,0PF12.3,1PE10.1,
     &                                 0PF7.2,E10.2,I5,F8.3,1PE10.1,2I5)
      END IF
*
