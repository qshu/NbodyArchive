...
*       Open unit #4 the first time.
      IF (NB.GT.0.AND.FIRST) THEN
          OPEN (UNIT=44,STATUS='UNKNOWN',FORM='FORMATTED',FILE='DEGEN')
          FIRST = .FALSE.
*
*       Print cluster scaling parameters at start of the run.
          if(rank.eq.0)then
          WRITE (44,2)  RBAR, BODYM*ZMBAR, BODY1*ZMBAR, TSCALE,
     &                 NBIN0, NZERO
...
*         END IF
          ...
          IF (IPRINT.EQ.0.AND.ICASE.EQ.0) THEN
              if(rank.eq.0) WRITE (44,*)
          END IF
          if(rank.eq.0)then
          WRITE (44,10)  ICASE, TPHYS, SEMI(JPAIR), ECC(JPAIR), RATIO,
