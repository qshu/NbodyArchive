...
*       Determine the regularized step by Newton-Raphson iteration (DT < 0).
      ...
      DTU = DT/R(IPAIR)
      ITER = 0
   40 Y0 = DT - ((ONE6*TDOT3(IPAIR)*DTU + 0.5*TDOT2(IPAIR))*DTU +
     &                                                     R(IPAIR))*DTU
     ...
     &                                                     R(IPAIR))*DTU
      ITER = ITER + 1
      IF (ABS(DT - DT1).GT.1.0E-10*STEP(I1).AND.ITER.LT.5) GO TO 40
*
*       Integrate small step back to apocentre using temporary indicator # 0.
      ...
      CALL KSINT(I1)
*
*       Reverse the regularized velocities for forward integration.
      DO 50 K = 1,4
          UDOT(K,IPAIR) = -UDOT(K,IPAIR)
   50 CONTINUE
*
