...
*     IF (KCASE.EQ.1.AND.I1.NE.NTOT) THEN
*       Form total c.m. force & first derivative and also AC components.
*         A1 = BODY(ICOMP)
*         A2 = BODY(JCOMP)
*         DO 90 K = 1,3
*             D2(K,NTOT) = (A1*D2(K,ICOMP) + A2*D2(K,JCOMP))/BODY(NTOT)
*             D2R(K,NTOT) = (A1*D2R(K,ICOMP) + A2*D2R(K,JCOMP))/
*    &                                                        BODY(NTOT)
*             D3(K,NTOT) = (A1*D3(K,ICOMP) + A2*D3(K,JCOMP))/BODY(NTOT)
*             D3R(K,NTOT) = (A1*D3R(K,ICOMP) + A2*D3R(K,JCOMP))/
*    &                                                        BODY(NTOT)
*  90     CONTINUE
*     END IF
*     IF(KCASE.EQ.1)THEN
*     PRINT*,' 1 NTOT,D2,D2R,D3,D3R=',NTOT,D2(1,NTOT),D2R(1,NTOT),
*    *                                     D3(1,NTOT),D3R(1,NTOT)
*     END IF
*
