...
*       Impose unperturbed motion (small final orbit).
          LIST(1,2*IPAIR-1) = 0
*
...
*
*       Modify KS variables to yield nearly circular velocity with energy H.
          ECCM = 0.002
          RAP = R(IPAIR)*(1.0 + ECCM)
          CALL EXPAND(IPAIR,RAP)
          CALL RESOLV(IPAIR,1)
*
          WRITE (6,10)  NAME(I1), NAME(I2), KSTAR(I1), KSTAR(I2), KW1,
     &                  KW2, M1, M2, DM*ZMBAR, R1, R2, SEMI0, SEMI
   10     FORMAT (' NEW CE    NAM K0* K* M1 M2 DM R1 R2 A0 A ',
     &                        2I6,4I3,3F5.1,2F7.1,1P,2E10.2)
*
...
*       Evaluate binary disruption velocity and introduce velocity kick.
              ...
              CALL KICK(I1,1)
*
*       Re-initialize two-body energy from new velocities.
              VREL2 = 0.d0
              DO 15 K = 1,3
                  VREL2 = VREL2 + (XDOT(K,I1) - XDOT(K,I2))**2
   15         CONTINUE
              H(IPAIR) = 0.5d0*VREL2 - BODY(I)/R(IPAIR)
*
...
*       Form new c.m. variables and update GRAPE.
   ...
   20         CONTINUE
              CALL GPSEND
*
*       Add body #I at the end (counting from location #2).
              NNB2 = NNB + 2
              ILIST(NNB2) = I
...
*       Initialize new KS polynomials after velocity kick (H > 0 is OK).
   ...
   35             FORMAT (' NS BINARY    R/A A M V ',3F6.2,3F7.1)
              END IF
          ELSE
*       Set circular velocity indicator and update Roche look-up time.
              KSTAR(I) = 10
...
*       Correct neighbour forces and update system energy loss due to DM.
              CALL FCORR(I,DM,KW1)
*
              IF (DM*SMU.GT.0.1.OR.ECC0.GT.0.1) THEN
