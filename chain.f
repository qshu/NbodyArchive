...
*       Save step in case of backwards last iteration on CE or collision.
      SAVEIT = STEPIN
*     RUNAV = 0.0001*RGRAV
*
...
*       Set search distance for closest separation (exclude circular orbit).
          IF (IMCIRC.EQ.0) THEN
              I1 = INAME(IX)
              I2 = INAME(IX+1)
...
*       Switch on search indicator during iteration or just after pericentre.
      ...
      IF (RY.LT.SX.AND.NSTEP1.GT.NEXT) THEN
          IF (ZMI3.LT.ZMI2.AND.ZMI2.GT.ZMI1) THEN
              IF (ISYNC.EQ.0.AND.NCIRC.LT.25) THEN
                  ICALL = 1
...
*       Distinguish between tidal energy loss and collision (ICOLL holds IM).
      ...
      RFAC = 10.0
*
*       Determine maximum periastron factor for capture (option #27 > 0).
      IF (EBS.GT.0.0.AND.KZ27.GT.0) THEN
          SEMI = -0.5*M(K1)*M(K2)/EBS
          HI = -0.5*(M(K1) + M(K2))/SEMI
          VINF = SQRT(2.0*HI)*VSTAR1
          RFAC = RPMAX(SIZE(K1),SIZE(K2),M(K1),M(K2),
     &                 ISTAR(K1),ISTAR(K2),VINF)
      END IF
*
*       Check option for artificial collision.
      IF (KZ27.EQ.-1) THEN
*       Check collision criterion of Kochanek (Ap.J. 385, 684, 1992)
          J1 = K1
          J2 = K2
...
*       Convert to new scaling using dummy variable (VSTAR1 = RMSTAR).
          RCR = VSTAR1*RCR
          IF (QPERI.LT.RCR) THEN
              CALL TOUCH2(K1,K2,RCR)
          END IF
          GO TO 50
      END IF
*
*       Check for capture, circularization or collision.
      IF (QPERI.LT.RFAC*MAX(SIZE(K1),SIZE(K2))) THEN
*
*       Check collision criterion of Kochanek (Ap.J. 385, 684, 1992)
          J1 = K1
          J2 = K2
...
*       Check termination criteria (T > TMAX or RSUM > RMAXC).
     ...
     &                      (1.0/RINV(K),K=1,N-1)
   55         FORMAT (' CHAIN:  # T DTR R ',I5,F14.8,1P,6E9.1)
          END IF
