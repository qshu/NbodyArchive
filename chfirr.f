...
*
     ...
     &        FPSUM(3),FPD(3*NCMAX),FPDSUM(3)
      LOGICAL UPDATEF(0:MAXPE-1)
      COMMON/XCUPD/  UPDATEF
*
...
*       Check updating perturber list and X & XDOT of chain members.
      IF (IR.EQ.0) THEN
*       Chain-Variables will be updated on PE #rank
          UPDATEF(RANK) = .TRUE.
          CALL CHLIST(ICH)
          CALL XCPRED(0)
      END IF
...
*       See whether c.m. approximation applies (skip unperturbed case).
          IF (RIJ2.GT.rs2fac*CMSEP2*R(JP)**2.OR.LIST(1,J1).EQ.0) GO TO 5
...
*       Decide appropriate summation (c.m. approximation or components).
          IF (K.LE.N) THEN
              IF (RIJ2.GT.rs2fac*RPERT2) GO TO 55
              GO TO 45
          ELSE IF (RIJ2.LT.rs2fac*CMSEP2*R(K-N)**2) THEN
              KDUM = 2*(K - N) - 1
