...
*       Calculate total mass of five nearest neighbours.
   ...
   45     CONTINUE
*
          RHODBL(I) = XMASS/(RLIST(I6)*SQRT(RLIST(I6)))
*       For multi-mass store also five neighbour particle density
          XNDBL(I) = RHODBL(I)/XMASS
*       Assign zero weight if not enough neighbours.
          IF (N6.LT.6) RHODBL(I) = 0.0D0
          RHOS = MAX(RHOS,RHODBL(I))
   50 CONTINUE
*
...
*
      ...
      DO 70 L = 1,NC
          I = JLIST(L)
          DO 65 K = 1,3
              RDENS(K) = RDENS(K) + RHODBL(I)*X(K,I)
   65     CONTINUE
          RHO1 = RHO1 + RHODBL(I)
   70 CONTINUE
*
...
*       Obtain density radius & averaged density.
      ...
      DO 80 L = 1,NC
          ...
          RC = RC + RHODBL(I)**2*RID2
          RHO2 = RHO2 + RHODBL(I)**2
   80 CONTINUE
*
