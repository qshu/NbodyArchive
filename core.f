...
*       Estimate D6 to limit the candidates (additional factor of 2).
          ...
          YI = X(2,I)
          ZI = X(3,I)
          ITERATION = 0 !!Kupi Gabor
   20     N6 = 0
*
	ITERATION = ITERATION + 1
!!!!!       WRITE(*,*)'coreBBBB',TIME,NNB,ITERATION
!!!!!       CALL FLUSH(6)
     ...
     &                                  (ZI - X(3,J))**2
*
*       Remember: The two last particles of input list are now excluded!
              IF (NAME(J).NE.(NZERO-1).AND.NAME(J).NE.NZERO) THEN
                  IF (RIJ2.LT.D6) THEN
                      N6 = N6 + 1
                      ILIST(N6) = J
                      RLIST(N6) = RIJ2
   ...
   25     CONTINUE
*
!!!!!       WRITE(*,*)'coreCCCC',TIME,N6,NNB
!!!!!       CALL FLUSH(6)

         IF(ITERATION.GE.10) GO TO 30
          IF (N6.LT.6) THEN
