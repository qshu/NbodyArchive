...
#
# $Id: stats.sh 2956 2007-10-04 11:44:54Z tbruese $
...
RDF_TEMPLATE=stats/jobstat.rdf.tpl
RDF_DOCUMENT=stats/jobstat.rdf
...
function submit_job_rdf()
{
  ...
  if [ $? -eq 0 ]; then
    echo "SUCCESS submitting job statistics."
  else
    echo "FAILED submitting job statistics."
  fi
}
