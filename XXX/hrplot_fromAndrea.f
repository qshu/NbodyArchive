...
*
*
      if(rank.eq.0) WRITE (82,1)  NPAIRS,TPHYS,TSTAR,
     &                            RDENS(1),RDENS(2),RDENS(3)
    1 FORMAT (I8,F9.1,F10.6,3(xE18.12))
      NS = N - 2*NPAIRS
      IMERGE = 0
      if(rank.eq.0) WRITE (83,1)  NS, TPHYS, TSTAR,
     &                            RDENS(1),RDENS(2),RDENS(3)
*
      DO 20 I = 1,N
          M0 = BODY0(I)*ZMBAR
...
c          AGE = TEV0(I)*TSTAR - EPOCH(I)
c          AGE = TPLOT*TSTAR - EPOCH(J1)  
...
*       Create output file for single stars (skip rare chain subsystem).
     ...
     &                                      (X(3,I) - RDENS(3))**2
              ...
              R1 = LOG10(RM)
              ZL1 = LOG10(LUM)
              if(rank.eq.0) WRITE (83,10) NAME(I),KW,RI,M1,LUM,RM,
     &              TTOT,TPLOT,TEV0(I),EPOCH(I),X(1,I),X(2,I),X(3,I)
   10         FORMAT (I6,I3,F6.1,3(xE18.12),xF10.1,6(xE18.12))
          END IF
   20 CONTINUE
*
*       Update plot interval (10 Myr initially) and next output time.
      ...
      IF (TIME.EQ.0.0D0) THEN
          ...
          TPLOT = TPLOT + DTPLOT
      ELSE
c*         DTPLOT = SQRT(2.0)*DTPLOT
c          TPLOT = 2.0*TIME/TSTAR      
c          TPLOT = TPLOT + DTPLOT/TSTAR
          TPLOT = TPLOT + DTPLOT  
c          Vorschlag von Kristin
      END IF
