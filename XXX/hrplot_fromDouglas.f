...
*
      DO 20 I = 1,N
          M0 = BODY0(I)*ZMBAR
          M1 = BODY(I)*ZMBAR
      IF (I.LT.IFIRST.AND.NAME(I).EQ.INAME) THEN
      ICM = KVEC(I) + N
      WRITE (6,50)  I, KSTAR(I), NAME(I), NAME(ICM), M1
   50 FORMAT (' BEGIN:    I K* NAMI NAMC M1  ',4I6,F7.2)
      END IF
...
*       Obtain stellar parameters at current epoch.
     ...
     &                RM,LUM,KW,MC,RCC)
      ...
      WRITE (6,55)  KW, M0, M1, AGE, TM, TN, RM 
   55 FORMAT (' CURRENT:   KW M0 M1 AGE TM TN RM  ',I4,2F7.2,1P,4E10.2)
      END IF
          IF (I.LT.IFIRST) THEN
              JPAIR = KVEC(I)
              J2 = 2*JPAIR
...
*       Copy masses and obtain evolution parameters for first component.
     ...
     &                        RM,LUM,KW,MC,RCC)
      IF (NAME(I).EQ.INAME) THEN
      WRITE (6,60) NAME(J1), IM, KW, M1, RM
   60 FORMAT (' GHOST CHECK:   NAM KW M1 RM  ',2I6,F7.2,1P,2E10.2)
      END IF
              END IF
              RJ = R(JPAIR)
              HJ = H(JPAIR)
...
*       Re-define index of second component and obtain parameters of M1.
     ...
     &                        RM,LUM,KW,MC,RCC)
      IF (NAME(I).EQ.INAME) THEN
      WRITE (6,65)  NAME(N+JPAIR), IMERGE, KW, AGE, M0, M1, RM
   65 FORMAT (' MERGER CHECK:   NAMC KW AGE M0 M1 RM  ',
     &                          I7,I4,F9.1,2F7.2,1P,2E10.2)
      END IF
              END IF
              M0 = BODY0(J2)*ZMBAR
              KW2 = KSTAR(J2)
