...
*
* -------------------------------------------------------
* Calculate VOVERC to determine if binary is relativistic
* -------------------------------------------------------
*
* This is for initialization and output purposes only, VOVERC is
* actually calculated for each KS timestep in ksint.f
*
      IF (KZ(41).LT.5) THEN
         CMVI(1) = (BODY(ICOMP)*XDOT(1,ICOMP)+BODY(JCOMP)*XDOT(1,JCOMP))
     &        /(BODY(ICOMP)+BODY(JCOMP))
         CMVI(2) = (BODY(ICOMP)*XDOT(2,ICOMP)+BODY(JCOMP)*XDOT(2,JCOMP))
     &        /(BODY(ICOMP)+BODY(JCOMP))
         CMVI(3) = (BODY(ICOMP)*XDOT(3,ICOMP)+BODY(JCOMP)*XDOT(3,JCOMP))
     &        /(BODY(ICOMP)+BODY(JCOMP))
*
         VCM(1) = XDOT(1,ICOMP)-CMVI(1)
         VCM(2) = XDOT(2,ICOMP)-CMVI(2)
         VCM(3) = XDOT(3,ICOMP)-CMVI(3)
         VCM(4) = XDOT(1,JCOMP)-CMVI(1)
         VCM(5) = XDOT(2,JCOMP)-CMVI(2)
         VCM(6) = XDOT(3,JCOMP)-CMVI(3)
...
*
         WRITE (*,*) ' '
         WRITE (*,34) "KSREG VOVERC",VOVERC,NAME(ICOMP),NAME(JCOMP),
     &        ICOMP,JCOMP
 34      FORMAT (A12,F35.25,5I6)
      END IF
*
