...
*
     ...
     &               NAMEC(NTMAX)
      ...
      LOGICAL IQ
*     PN variables
      REAL*8 FRELP(3),FRELD(3),DTUREL,DTUCLASS,VBIN,VWHOLE,GREL
      REAL*8 RTOCENTER
*     APPLY = 0: Do not calculate derivs and don't apply, only check acc
*     APPLY = 1: Calculate everything and apply
*     APPLY = 2: Calculate only derivs because acc already there
*     from the criterion check and then apply
      INTEGER APPLY, RELORNOT
...
*
* OBtain distance to density center for output
      RTOCENTER = (X(1,I1)-RDENS(1))**2+(X(2,I1)-RDENS(2))**2 + 
     & (X(3,I1)-RDENS(3))**2
      RTOCENTER = SQRT(RTOCENTER)
*       Check for further unperturbed motion.
      IF (NNB0.EQ.0.AND.H(IPAIR).LT.0.0) THEN
*     Patrick: skip this to switch on ksrel
         GO TO 777
...
*       Perform KS prediction of U, UDOT & H.
 777  CALL KSPRED(IPAIR,I1,I,BODYIN,UI,UIDOT,XI,VI)
*
*       Obtain VOVERC (relative velocity VBIN) for PN activation
      VBIN = (VI(1)-VI(4))*(VI(1)-VI(4))+(VI(2)-VI(5))*(VI(2)-VI(5))+
     & (VI(3)-VI(6))*(VI(3)-VI(6))
      VBIN = SQRT(VBIN)
      VOVERC = VBIN/CLIGHT

C      WRITE (*,*) 'KSINT', NAME(I1), NAME(I2), VOVERC
...
* ---------------------------------------
*
C      WRITE(*,*) 'RELSUM', RELSUM
            RIJ2 = (XI(1)-XI(4))*(XI(1)-XI(4))
     &         + (XI(2)-XI(5))*(XI(2)-XI(5))+(XI(3)-XI(6))*(XI(3)-XI(6))
            RIJ = SQRT(RIJ2)
*
* Criterion is crude, simple Schwarzchild radius overlap.
* For 3PN 3.5PN use 4 times SchwSchild because of divergence of PN
            IF (RIJ.LT.(8.0*(BODY(I1)+BODY(I2)))/(CLIGHT*CLIGHT)) THEN
*
 798           WRITE(*,556) TIME+TOFF,NAME(I1),NAME(I2),BODY(I1)+
     &              BODY(I2),RIJ, 2.0*(BODY(I1)+BODY(I2))/CLIGHT/CLIGHT,
     &              VWHOLE,MAX(BODY(I1)/BODY(I2),
...
C      IF ((NAME(I1).EQ.1.AND.NAME(I2).EQ.2).OR.(NAME(I1).EQ.2.AND.
C     &           NAME(I2).EQ.1)) THEN
C         STOP
C      END IF
               CALL FLUSH(6)
...
C               CALL KSPERI(IPAIR)
               KSPAIR = IPAIR
*
* Call KSREL with Apply = 3 to get final spin for new body
* in AFIN.
               APPLY = 3
               CALL KSREL(IPAIR,I1,XI,VI,FRELP,FRELD,VWHOLE,APPLY)
* Perform relativistic merger
C               CALL CMBODY(QPERI,2)
               CALL MERGREL(IPAIR)
*
...
*     &        THEN
         IF (VOVERC.GT.BETAREL) THEN
            APPLY = 1
*            WRITE (*,*) 'VOVERC is greater than BETAREL!'
*     Get Forces and apply later
            CALL KSREL(IPAIR,I1,XI,VI,FRELP,FRELD,VWHOLE,APPLY)
            GREL = GAMREL(1,IPAIR)+GAMREL(2,IPAIR)+GAMREL(3,IPAIR)
     & +GAMREL(4,IPAIR)+GAMREL(5,IPAIR)+GAMREL(6,IPAIR)+
     & +GAMREL(7,IPAIR)+GAMREL(8,IPAIR)
C            IF (GREL.GT.0.5) THEN
*     Just merge for that big of a perturbation
C               GO TO 798
C            END IF
C         ELSE IF (VOVERC.GT.(BETAREL/5.)) THEN
*            WRITE (*,*) 'VOVERC is greater than BETAREL/5!'
C            APPLY = 0
*     Calculate GAMREL values for switch-on criterion
C            CALL KSREL(IPAIR,I1,XI,VI,FRELP,FRELD,VWHOLE,APPLY)
*     Now GAMREL(8,KMAX) holds current PN strengths
*     First approach: Sum up GAMREL()
C            GREL = GAMREL(1,IPAIR)+GAMREL(2,IPAIR)+GAMREL(3,IPAIR)
C     & +GAMREL(4,IPAIR)+GAMREL(5,IPAIR)+GAMREL(6,IPAIR)+
C     & +GAMREL(7,IPAIR)+GAMREL(8,IPAIR)
*            WRITE (*,*) 'GAM REL', GREL
C            IF (GREL.GT.GAMMAREL) THEN
C               APPLY = 2
C               CALL KSREL(IPAIR,I1,XI,VI,FRELP,FRELD,VWHOLE,APPLY)
C            END IF
         ELSE
...
*     Apply corrections if...
            IF (APPLY.GT.0) THEN
               IF (NSTEPU.EQ.0.OR.MOD(NSTEPU,500).EQ.0) THEN
C         IF (RELENERGY(IPAIR).GT.1.0E-03.AND.APPLY.GT.0) THEN
                  CALL ENERGY
                  ETOT = ZKIN - POT + ETIDE
           ETOT = ETOT + EBIN + ESUB + EMERGE + ECOLL + EMDOT + ECDOT
                  WRITE (10,476) TIME+TOFF, RIJ, GREL, EBIN
     &                 , RELENERGY(IPAIR),PREVRELENERGY,ETOT
     &          ,NAME(I1),NAME(I2)
 476              FORMAT (7E17.9,2I5)
C     &                 FP(1), FP(2), FP(3), FD(1), FD(2), FD(3)
C 476              FORMAT (1E17.10, 1E14.7, 2I5, 6E15.7)
                  CALL FLUSH(10)
               END IF

               DO 337 KK = 1,3
                  FP(KK) = FP(KK) + FRELP(KK)
                  FD(KK) = FD(KK) + FRELD(KK)
C                  WRITE (*,*) 'RELATIVISTIC PAIR ', IPAIR
  337          CONTINUE
...
C      RELTSTEP(IPAIR) = STEP(I1)
*     Compute relativistic correction with the newly found timestep
*     Seems to make more sense at this Point (Patrick)
      IF (APPLY.GT.0) THEN
         RELENERGY(IPAIR) = RELENERGY(IPAIR) + DIFFS(3)*STEP(I1)
     &        + DIFFS(4)*STEP(I1)*STEP(I1)
      END IF
*
