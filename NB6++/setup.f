...
*       Scale coordinates & velocities to analytical expectation values.
   ...
   50 CONTINUE
*
*     Patrick: Set up two most massive stars in a close binary in the
*     center
C      SEMI = 0.0003
C      VELOC = SQRT((BODY(1)+BODY(2))/SEMI)
C      X(1,1) = -SEMI*BODY(2)/(BODY(1)+BODY(2))
C      X(1,2) = SEMI*BODY(1)/(BODY(1)+BODY(2))
C      X(2,1) = 0.
C      X(3,1) = 0.
C      X(2,2) = 0.
C      X(3,2) = 0.
C      XDOT(1,1) = 0.
C      XDOT(2,1) = VELOC/(1+SQRT(BODY(1)/BODY(2)))
C      XDOT(3,1) = 0.
C      XDOT(1,2) = 0.
C      XDOT(2,2) = -VELOC/(1+SQRT(BODY(2)/BODY(1)))
C      XDOT(3,2) = 0.
*
*
