...
*
   ...
   15     FORMAT (/,' END KSREG    TIME =',F8.2,1P,2E9.1,0PF8.3,1PE10.1,
...
*
      IF (KZ(41).LT.5.AND.RFLAG(IPAIR).EQ.1) THEN
         PREVRELENERGY = PREVRELENERGY - RELENERGY(IPAIR)
...
*
         WRITE (*,*) ' '
         WRITE (6,273) TIME+TOFF,RELENERGY(IPAIR),PREVRELENERGY
 273     FORMAT ('Rel Energy: TIME = ',f12.4,' PN ENERGY = ',e15.6,
     &           ' PREVRELENERGY = ',e15.6)
*
         ICOLLISION = 0
*
...
*       Check predicted neighbour number and form volume ratio.
      NBP = MIN(ALPHA*SQRT(FLOAT(NNB)*RS(ICM))/
     &                               (RS(ICM)**2),FLOAT(NNBOPT))
      NBP = MAX(NBP,1)
