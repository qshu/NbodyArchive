...
*
      INCLUDE 'common6.h'
      REAL*4  A1(3),A2(3),XREL(3),VREL(3),EI(3),HI(3),HO(3),
     &        TK0(10),TLAST(10),TTERM(10)
      INTEGER  LAST(10),IPRINT(10)
      LOGICAL  FIRST
      SAVE  FIRST,TLAST,TTERM,RAP,TK0,LAST,IPRINT,NL
      DATA  FIRST /.TRUE./
      DATA  LAST,IPRINT  /20*0/
*
*
*       Open unit #10 the first time.
      IF (FIRST) THEN
          OPEN (UNIT=10,FORM='FORMATTED',FILE='HIARCH')  ! STATUS='NEW' 
          OPEN (UNIT=11,FILE='HIARCH2')                  ! New 
          FIRST = .FALSE.
*
...
*       Set c.m. and component indices.
      ...
      I2 = I1 + 1
*
*       Skip printing line #2 on first termination after restart.
...
*
          WRITE (10,10)  TIME, SEMI, SEMI1, ECC1, PMIN, TK1, Q,
     &                   PCRIT/RAP, BODY(I)/BODYM, Q1, II,
     &                   NAME(I1), NAME(I2), NAME(JCOMP), KCM
*
          WRITE (11,10)  TIME, SEMI, SEMI1, ECC1, PMIN, TK1, Q,
     &                   PCRIT/RAP, BODY(I)/BODYM, Q1, II,
     &                   NAME(I1), NAME(I2), NAME(JCOMP), KCM
...
*
          ...
          WRITE (11,11)  SQRT(EI2), EMAX, KSTAR(I1), KSTAR(I2),KSTAR(I),
...
*
      ELSE IF (IPHASE.EQ.7) THEN
          ...
          NK = (TIME - TLAST(IL))/TK0(IL)
          NK1 = (TIME - TLAST(IL))/TK
          NK = MIN(NK,99999999)
          NK1 = MIN(NK1,9999)
          NK = MAX(NK,0)
          RI = 0.0
...
*
          WRITE (10,20)  TIME, SQRT(RI)/RC, SEMI, ECC, PMIN, TK1,
     &                   RR, GAMMA(IPAIR), IT, NK, NK1, NPAIRS, N
   20     FORMAT (' #2',F8.1,1P,2E10.2,0P,F6.2,F7.2,F8.1,2F6.2,I4,
     &                  I10,2I5,I6)
          CALL FLUSH(10)
          WRITE (11,20)  TIME, SQRT(RI)/RC, SEMI, ECC, PMIN, TK1,
     &                   RR, GAMMA(IPAIR), IT, NK, NK1, NPAIRS, N
      END IF
*
*       Restrict memory of previous cases to 9 by removing the oldest one.
   30 IF (NL.GE.10) THEN
          LT = 1
*       Skip any current mergers (defined by TTERM = 0).
