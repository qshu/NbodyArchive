...
*       Copy four-body system from the first single particle locations.
          M(L) = BODY(J)
*       Define global particle indices.
          NAME4(L) = J
          SIZE(L) = RADIUS(J)
          CM(7) = CM(7) + M(L)
          DO 3 K = 1,3
...
*       Calculate internal energy and include in total subsystem energy.
      ...
      ESUB = ESUB + ENERGY
*
*       Save global indices & particle names of subsystem.
      DO 10 L = 1,4
          J = NAME4(L)
          JLIST(L) = J
          NAME4(L) = NAME(J)
...
*       Transform to global velocities using corrected c.m. velocity.
      DO 130 L = 1,4
          ...
          DO 125 K = 1,3
              ...
              X0DOT(K,J) = XDOT(K,J)
  125     CONTINUE
      WRITE (6,126) J,LL,(X(K,J),K=1,3),(XDOT(K,J),K=1,3)
  126 FORMAT (' J LL X XD ',2I4,3F9.4,1X,3F6.2)
  130 CONTINUE
*
*       Predict coordinates & velocities of perturbers to order F3DOT.
      DO 140 L = 1,NP
          J = JPERT(L)
          CALL XVPRED(J,0)
