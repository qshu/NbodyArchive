...
*
*       Stabilize NNB between 1 & 0.9*NNBOPT by square root of contrast.
...
*       Stabilize NNB on NNBOPT if too few perturbers.
          IF (A2.GT.RS(I)) THEN
              A3 = MAX(1.0 - FLOAT(NNB)/FLOAT(NNBOPT),0.0D0)
...
*
c  110 IF (TTMP.LT.STEP(I)) THEN
  110 IF (TTMP .LT. 8.0D0*STEP(I)) THEN
          STEP(I) = 0.5D0*STEP(I)
