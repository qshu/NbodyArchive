...
*       Check for hierarchical configuration.
    ...
    4             FORMAT (' HIERARCHY:   KS NMJ H E A0 A1 RP E1 SR',
     &                              2I6,F7.0,F7.3,1P,3E9.1,0PF6.2,F6.1)
              END IF
...
*       Check initialization for original fifth body.
      IF (NSYS.EQ.5) THEN
          CALL FPOLY1(I5,I5,0)
          CALL FPOLY2(I5,I5,0)
      END IF
...
*       Specify new membership (< 0 for SETSYS) and remove ghost from chain.
          ...
          DO 50 L = 2,NSYS-1
              JLIST(L) = JLIST(L+1)
   50     CONTINUE
*       Copy well defined binding energy and skip energy correction.
          EB = EBS
          CHCOLL = CHCOLL + EB
          GO TO 80
...
*
     ...
     &                                      (X(3,I) - X(3,J))**2
              ...
              IF (L.EQ.1.AND.LL.EQ.2) RB = SQRT(RIJ2)
   65     CONTINUE
   70 CONTINUE
*
...
*       Update energy loss & collision counters.
   80 ECOLL = ECOLL + EB
      ...
      NCOLL = NCOLL + 1
*
*       Set IPHASE < 0 & TLIST = TIME to ensure new NLIST (NB! both needed).
      IPHASE = -1
      TLIST = TIME
*
      WRITE (6,90)  WHICH1, NSYS, NAME1, NAME2, ZM, RCOLL, RB, EB, DP
   90 FORMAT (/,A8,'COLLISION    NSYS =',I3,'  NAME =',2I6,
     &             '  M =',F7.4,'  RCOLL =',1P,E8.1,'  R =',E8.1,
     &             '  EB =',E9.1,'  DP =',E10.2)
*
      RETURN
*
