...
*
      INTEGER ITIMER(NMAX),NCOROT,NCNROT
      SAVE
      COMMON/DRAGDAT/Q_DRAG,C3,HZ,CMBLHOLE,EPS1
      SAVE
      COMMON/DRAG/K11,K111,K1111,INCLDISTR(NMAX),
     &            EXCEDISTR(NMAX),EXCEDISTR1(NMAX),TIMEOLD1(NMAX),
     &            TIMEOLD2(NMAX),XOLD1(ID,NMAX),XOLD2(ID,NMAX),
     &            PARTENERGY(NMAX),INCLDISTR1(NMAX),AXISDISTR(NMAX),
     &            RSTAR1(NMAX),RSTAR2(NMAX),RSTAR3(NMAX),DFR1(NMAX),
     &            DFR2(NMAX),DFROLD1(NMAX),DFROLD2(NMAX),XOLDM(ID,NMAX),
     &            XDOTCD(ID,NMAX),
     &            XCD1(ID,NMAX),COUNTER3(NMAX),ROLD(NMAX),
     &            EXCENTRB(NMAX),
     &            EXCENTRC(NMAX),EXCEDISTR2(NMAX),
     &            EXCEDISTR3(NMAX),
     &            ECC_L(NMAX),
     &            ECC_3P(NMAX),ECC_PA(NMAX),
     &            INCLDISTR0(NMAX),EXCEDISTR0(NMAX),EXCEDISTR10(NMAX),
     &            INCLDISTR10(NMAX),EXCEDISTR20(NMAX),
     &            EXCEDISTR30(NMAX)
*
      COMMON/ORBITELEMENT/ XOLD(ID,NMAX),XDOTOLD(ID,NMAX)
*
      COMMON/DRAG1/ EDISS1,EXCENTR22(NMAX),EXCENTR222(NMAX),
     &              AMPLIT(NMAX),
     &              RSTARMAX(NMAX),RSTARMIN(NMAX),
     &              VMIN(NMAX),VMAX(NMAX),NFILE,ANGMAX(NMAX),
     &              ANGMIN(NMAX),
     &              EDISS11,NUMBER1(NMAX),VOLD1(ID,NMAX),VOLD2(ID,NMAX)
*         
      COMMON/BLACKHL/ EBLCKHL,EBLCKHL1,BLACKFORCE
*
