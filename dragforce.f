       SUBROUTINE DRAGFORCE(I,FIRR,FD,KDR)
*
*
...
*              Types of BODY(I),X(K,I),XDOT(K,I) is Defined in "comon6.h" 
     ...
     &        DISCMOM,STARMOM,ANGLE1,INCLIN2(NMAX)
*
      LOGICAL LPR
      LPR=NAME(I).EQ.250
*
...
*
      VXDISC=-CD3*X(2,I)/((R2DISC)**(3.0/4.0))
      VYDISC=CD3*X(1,I)/((R2DISC)**(3.0/4.0))
...
*      Excenricity & Inclination of Orbits
      ...
      EXCENTR1(I)=(RSTARMAX(I)-RSTARMIN(I))/(RSTARMAX(I)+RSTARMIN(I))
*     PRINT*,'EXCENTRICITY=',EXCENTR1(I)
...
*      
      DISCMOM=(VXDISC*X(2,I)-VYDISC*X(1,I))
      STARMOM= (XDOT(2,I)*X(3,I)-XDOT(3,I)*X(2,I))**2-
     &         (XDOT(1,I)*X(3,I)-XDOT(3,I)*X(1,I))**2+
     &         (XDOT(1,I)*X(2,I)-XDOT(2,I)*X(1,I))**2
      ANGLE1= (XDOT(1,I)*X(2,I)-XDOT(2,I)*X(1,I))*DISCMOM/
     &        (SQRT(STARMOM)*SQRT((DISCMOM**2)))
      IF(ANGLE1.GT.0.0) THEN
*     IF(VXDISC*XDOT(1,I).GT.0.0 .AND. VYDISC*XDOT(2,I).GT.0.0) THEN
      INCLIN2(I)=(360/TWOPI)*ACOS(ANGLE1)
...
*
      COUNTER1(I)=O
*     PRINT*,'INCLINATION=',INCLIN1(I),INCLIN2(I)
      END IF
*
*     IF(LPR) THEN
*     WRITE(51,511) TIME,(FDRAG(K), FDDRAG(K),X(K,I),XDOT(K,I),K=1,3)
*     WRITE(52,512) TIME,INCLIN1(I),EXCENTR1(I)
*511  FORMAT(1X,1P,13(D15.5,1X))
*512  FORMAT(1X,1P,3(D15.5,1X))
*     END IF
*      
*              Auxiliary Calculations
      DVXRE=-(CD3**2.0)*X(1,I)/(R2DISC**(3.0/2.0))-
     &       (FIRR(1)+FDRAG(1))
      DVYRE=-(CD3**2.0)*X(2,I)/(R2DISC**(3.0/2.0))-
     &       (FIRR(2)+FDRAG(2))
      DVZRE=-(FIRR(3)+FDRAG(3))
