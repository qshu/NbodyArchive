...
*       Ignore case of three-body system here (JLIST(4) = 0).
          ...
          END IF
*
*       Determine current quantized time (note: TBLOCK-TPREV may be large).
          TM = TBLOCK
*       Skip the chain members (note I3 may have any value for NSYS = 3).
          JFIRST = IFIRST + NSYS
          IF (I3.GT.JFIRST) JFIRST = JFIRST - 1
          DO 9 J = JFIRST,NTOT
              IF (J.EQ.I3) GO TO 9
              TM = MIN(T0(J) + STEP(J),TM)
    9     CONTINUE
*       Adopt smallest end point (no need to subtract STEP).
          TIME = MIN(TM,TBLOCK)
      END IF
*
