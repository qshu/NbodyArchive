...
*       Apply safety test near small pericentre or for unperturbed motion.
          IF (DTU.GT.1.2*DTAU(J)) THEN
              DTU = 1.2*DTAU(J)
          END IF
          IF (LIST(1,J1).EQ.0) DTU = 0.0
*
          IF (LIST(1,J1).GT.0) THEN
          IF(DTCHK1.GT.1.20D0*DTAU(J))PRINT*,'=======DTCHK1/DTAU===',
     *     DTCHK1/DTAU(J)
          IF(DTCHK2.GT.1.20D0*DTAU(J))PRINT*,'=======DTCHK2/DTAU===',
     *     DTCHK2/DTAU(J)
          IF(DTCHK2.GT.1.20D0*DTAU(J).OR.DTCHK1.GT.1.20D0*DTAU(J))THEN
          WRITE(55,*)' KSRES2: ',2*IZZ-1,' Bin ',J1,' STEP,T0,TIME,DT=',
     *  STEP(J1),T0(J1),TIME,TIME-T0(J1)
          PRINT*,IZZ,' Bin ',J1,' STEP,T0,TIME,DT=',
     *  STEP(J1),T0(J1),TIME,TIME-T0(J1)
