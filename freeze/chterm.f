...
*       Predict current coordinates & velocities to F3DOT before termination.
      CALL XVPRED(ICM,-1)
*
*       Predict X & XDOT of neighbours to order FDOT.
      NNB1 = LIST(1,ICH) + 1
      DO 25 L = 2,NNB1
          J = LIST(L,ICH)
          CALL XVPRED(J,-2)
...
*
*       Copy name of first perturber in case of exchange in KSREG.
...
*
   ...
   90         FORMAT (' CHTERM:   SECOND BINARY   I3 I4 DT DTR R H G ',
     &                                            2I5,1P,5E9.1)
      ...
      END IF
*
*       Replace old perturber if it has has been exchanged in KSREG.
      IF (LISTC(1).GT.0.AND.LISTC(2).LT.IFIRST) THEN
          DO 92 J = IFIRST,N
              IF (NAME(J).EQ.NAMC) THEN
                  LISTC(2) = J
              END IF
   92     CONTINUE
      END IF
