...
*
*       Do not allow multiple absoptions (NAMES(NMX,5) = 0 each new chain).
...
*
          RSUM = RSUM + RIJ
          IF (KZ(30).GT.1) THEN
              WRITE (6,7)  JCLOSE, NAME(JCLOSE), GPERT, RIJ, RSUM, PMIN
    7         FORMAT (' ABSORB:    JCLOSE NMJ GPERT RIJ RSUM PMIN ',
     &                             2I6,F6.2,1P,3E9.1)
          END IF
*
*       Switch off any slow-down before increasing the chain (cf. new step).
          ...
          END IF
*
*       Treat extreme binary as inert if another is present (cf. slow-down).
          IF (JCLOSE.GT.N.AND.KZ(26).EQ.2) THEN
    ...
    8         CONTINUE
              ...
              IF (RM.LT.SMALL.AND.SEMI.LT.SMALL) THEN
                  WRITE (6,9)  NAME(JCLOSE), GPERT, RIJ,
     &                                       (1.0/RINV(K),K=1,NCH-1)
    9             FORMAT (' ABSORB INERT    NM G RIJ R ',
     &                                      I6,F6.2,1P,5E10.2)
...
*       Do not permit large length contrast even for bound orbit (NN = 4).
              ...
              IF (ER.LT.0.0.AND.RX.LT.2.0*RMIN) THEN
                  ...
                  GO TO 60
              END IF
              IF (RI.GT.MAX(0.5*RMAXS(ISUB),RMIN)) THEN
...
*       Delay termination for eccentric binary inside R = -m1*m2/2*E > a.
                      ...
                      J1 = INAME(IB)
                      J2 = INAME(IB+1)
*       Use definition of energy: E = (m1*m2 + (m1+m2)*m3)/RGRAV.
                      ZMU = BODYC(IESC)*(BODYC(J1) + BODYC(J2))/
     &                                  (BODYC(J1)*BODYC(J2))
                      RCRIT = 0.5*RGRAV/(1.0 + ZMU)
                      RY = 1.0/RINV(IB)
                      IF (RY.LT.RCRIT) THEN
                          WRITE (6,31)  NSTEP1, RY/RCRIT, RI, RDOT**2,
     &                                  2.0*BODY(ICH)/RI
                          KCASE = 0
                          GO TO 60
                      END IF
...
*       Skip if largest separation is not dominant.
              ...
              END IF
          ELSE
*       Note possibility a << RCRIT in strong interaction (hence RDOT test).
              IB = 3 - IM
              J1 = INAME(IB)
              J2 = INAME(IB+1)
              ZMU = BODYC(IESC)*(BODYC(J1) + BODYC(J2))/
     &                          (BODYC(J1)*BODYC(J2))
              RCRIT = 0.5*RGRAV/(1.0 + ZMU)
              RY = 1.0/RINV(IB)
              IF (RY.LT.RCRIT.AND.RDOT**2.LT.8.0*BODY(ICH)/RI) THEN
                  WRITE (6,31)  NSTEP1, RY/RCRIT, RI, RDOT**2,
     &                          2.0*BODY(ICH)/RI
   31             FORMAT (' CHAIN DELAY    # R/CRIT RI RD2 VP2 ',
     &                                     I5,F6.2,1P,3E9.1)
                  KCASE = 0
                  GO TO 60
              END IF
