...
*
      PRINT*,' WARNING IN CASE OF PARALLEL EXECUTION'
      JCOMP = IKSRL(LJ)
      K = 1
...
*       Now see whether body #J has already been included as neighbour.
  309 DO 310 L = 1,NNB
          IF (J.EQ.IKSRL(L+1)) GO TO 308
  310 CONTINUE
*       Include body #J in sequential location of neighbour list.
      L = NNB + 1
  312 IF (J.GT.IKSRL(L)) GO TO 314
      IKSRL(L+1) = IKSRL(L)
...
*       Advance list index by two for every identified pair.
      IF (IKSRL(L).GT.IFIRST + 3.OR.L.GT.NNB + 1) GO TO 330
*
      JPAIR = 0.5001*(IKSRL(L) + 1)
*       Appropriate pair index for either component (NB! do not use KVEC).
      IPAIR = 0.5001*(IKSRL(L+1) + 1)
...
*
      LJ = NNB + 1
  327 IF (J.GT.IKSRL(LJ)) GO TO 328
...
*       First use large increments to speed up the search.
      ...
      IF (IKSRL(LG).LT.JBODY.AND.LG.LT.NNB + 1) GO TO 336
*
      LG = LG + 1
  338 LG = LG - 1
      IF (IKSRL(LG).GT.JBODY.AND.LG.GT.2) GO TO 338
      IF (IKSRL(LG).EQ.JBODY) ICASE = ICASE + KTIME
...
*       Arguments for J & JCOMP are L or L - 1 depending on ICASE.
      ...
      A3 = X(3,JCOMP) - X(3,J)
      RIJ2 = A1*A1 + A2*A2 + A3*A3
*       Only accept body #J if distance to JCOMP is < 2*RMIN (skip J = I).
...
*
  ...
  344 CONTINUE
*
*       Finally include body #J in neighbour list and increase NNB.
      K = NNB + 1
  346 IF (J.GT.IKSRL(K)) GO TO 348
*       Move other members down by one until relevant location is free.
      IKSRL(K+1) = IKSRL(K)
      K = K - 1
      IF (K.GT.1) GO TO 346
...
*       Check list of high velocity intruders for early retention.
  ...
  351 J = LISTV(L)
      ...
      A4 = X0DOT(1,J) - X0DOT(1,I)
      A5 = X0DOT(2,J) - X0DOT(2,I)
      A6 = X0DOT(3,J) - X0DOT(3,I)
      A7 = A1*A4 + A2*A5 + A3*A6
      IF (A7.GT.0.0) GO TO 354
      P2 = RIJ2 - A7**2/(A4**2 + A5**2 + A6**2)
*       Accept if impact parameter < RS.
      IF (P2.GT.RS2) GO TO 354
*       Finally see if body #J has been included by other procedures.
      DO 353 K = 1,NNB
          IF (J.EQ.IKSRL(K+1)) GO TO 354
  353 CONTINUE
      ICM = -1
