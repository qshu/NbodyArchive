...
*       Compute mass dependency of neighbour radius:
      CALL rs2modf(rs2fac,BODY(J),BODY(JCOMP))
*       Only accept body #J as neighbour if distance to JCOMP is < 2*RMIN.
      IF (RIJ2.GT.(rs2fac*RMIN22)) GO TO 308
*
      IF (J.LE.N) GO TO 309
...
*
*       Compute mass dependency of neighbour radius:
      CALL rs2modf(rs2fac,BODY(JCOMP),BODY(J))
**       Only accept #J as neighbour if distance to JCOMP is < 2*RMIN.
      IF (RIJ2.LT.(rs2fac*RMIN22)) GO TO 324
  323 L = L - 1
...
*       Only accept body #J if distance to JCOMP is < 2*RMIN (skip J = I).
      IF (RIJ2.GT.(rs2fac*RMIN22).OR.J.EQ.I) GO TO 332
*
...
*       Check list of high velocity intruders for early retention.
  ...
  351 J = LISTV(L)
      ...
      IF (RIJ2.GT.4.0*rs2fac*RS2.OR.RIJ2.LT.(rs2fac*RS2)) GO TO 354
