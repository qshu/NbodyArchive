...
c
      real*8 arr(*)
      integer ind(*), n
      integer LSTK, sp, i, j, k, l, ij, m,kk
      parameter(LSTK = 32)
      integer iu(LSTK), il(LSTK)
      real*8 tij
c
      sp = 1
c      write(6,*) 'sort3 called with ', n
      if( n .eq. 1) return
      if( n .eq. 2) then
         if(arr(ind(1)) .gt. arr(ind(2))) then
            ij = ind(1)
            ind(1) = ind(2)
            ind(2) = ij
         endif
 ...
 5    continue
      if (i .lt. j) then
c     
c     first check if it is already sorted...
c
         do kk=i,j-1
            if(arr(ind(kk+1)).lt.arr(ind(kk))) goto 6
         enddo
c
c     already sorted - no need to go down
c
         sp = sp -1
         if (sp .eq. 0) goto 99
         i = il(sp)
