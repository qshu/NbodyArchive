...
*
      INCLUDE 'common4.h'
      INCLUDE 'grape6.h'
      INTEGER i,nnb,nlisti(1024),index(48)
      INTEGER iter,j,k,l,ll,nn,toolarge
      INTEGER ierr,iflag
      REAL*8 h2,rij2,ri2,resc2
      REAL*8 fgp(3,48),f1gp(3,48),xgp(3,48),vgp(3,48),phgp(48)
*
      nn = ntot - ifirst + 1
      iter = 0
      toolarge = 0
...
*       Decide between full N loop on host or fast neighbour list on GRAPE.
 1    continue
     ...
     &    toolarge.ne.0)then
 999     nnb = 0
*      Save index of all particles inside square distance H2.
         do j = ifirst,ntot
            rij2 = (x(1,i) - x(1,j))**2
...
*
      ...
      CALL g6calc_firsthalf(gpid,nn,48,index,xgp,vgp,
     &                      fgp,f1gp,phgp,eps2,gph2)
      gperr = g6calc_lasthalf(gpid,nn,48,index,xgp,vgp,
...
*
     ...
     &                ' in nblist'
      ...
      gperr = g6_get_neighbour_list(gpid,0,1024,nnb,nlisti)
      if(gperr.ne.0)then
         iflag = gperr
         if(gperr.lt.0)then
            WRITE(6,*)'GRAPE HARDWARE ERROR: RESET REQUIRED',time
...
*
      if(nnb.gt.128.or.nnb.ge.lmax-3)then
         ...
         if(nwarn.lt.1000)then
            WRITE(94,22)nnb,iphase,i,time,SQRT(h2),SQRT(ri2)
            CALL flush(94)
         endif
***
*
...
*       Adopt new neighbour distance from density fitting formula or 2*H.
         ...
         if(nwarn.lt.1000.and.n.ge.1000)then
            ...
            WRITE(94,25)i,nnb,time,SQRT(h2),SQRT(ri2),rks
         endif
         iter = iter + 1
         if(h2.gt.5.d0)then
            toolarge = 1
...
*       Include warning for exceeding maximum value.
 30   if(nnb.ge.lmax)then
         ...
         if(nwarn.lt.1000.and.n.ge.1000)then
            WRITE(94,35)iphase,nnb,i,time,SQRT(h2),rks
            CALL flush(94)
         endif
...
*
*       Copy temporary list to COMMON array (#2,NNB+1) excluding body #I.
      ll = 1
      do l = 1,nnb
         j = nlisti(l)
         if(j.ne.i)then
      ...
      enddo
*
*       Specify actual neighbour number and increase event counter.
      nnb = ll - 1
      nbcall = nbcall + 1
*
*     WRITE (6,50)  I, NNB, (ILIST(L),L=2,LL)
*  50 FORMAT (' NBLIST     I NNB IL ',I5,I4,20I4,2(/,5X,20I4))
*
      RETURN
      END
***
