...
*
     ...
     &              ZMASS/FLOAT(N), NCRIT1, (ILIST(J),J=1,JLAST)
   18 FORMAT (/,' ESCAPE    N =',2I6,I4,F8.4,F11.6,2F7.2,F7.3,F9.5,I7,
     &                                             2X,7I6,/,3(10X,20I6))
*
      IF (KZ(23).GT.2) THEN
          JLAST = MIN(2*NCORR,LMAX)
          WRITE (6,20)  (JPERT(J),J=1,JLAST)
   20     FORMAT (/,' ESCAPE ANGLES ',11(2I4,2X),9(/,18X,11(2I4,2X)))
      END IF
      IF (KZ(23).GT.1) THEN
          CALL FLUSH(11)
      END IF
...
*       Check updating of global index & neighbour list for chain c.m.
      ...
      END IF
*
*       Set phase & GRAPE indicator < 0 to denote change of particle number.
      IPHASE = -2
      ISEND = -1
*
   25 RETURN
...
*
*       Form optional output diagnostics (angles w.r.t. X-axis & XY-plane).
   40 IF (KZ(23).GT.2.AND.2*NCORR.LT.LMAX) THEN
          X1 = X(1,I) - RDENS(1)
          Y1 = X(2,I) - RDENS(2)
          Z1 = X(3,I) - RDENS(3)
          A3 = ABS(Y1/X1)
          A4 = ABS(Z1)/SQRT(X1**2 + Y1**2)
          JPERT(2*NCORR+1) = DATAN(A3)*180.0/3.14159
          JPERT(2*NCORR+2) = DATAN(A4)*180.0/3.14159
      END IF
      RESC = SQRT(RI2)
      STEPI = STEP(I)
*
*       Accumulate escaper names and save current name in case I > N.
      ...
      NAMEI = NAME(I)
      KSTARI = KSTAR(I)
      IF (NAME(I).LT.0) KSTARI = KSTAR(I) + 20
...
*       Obtain binding energy of body #I and update optional tidal radius.
      ...
      IF (KZ(14).GT.0) THEN
          CALL XTRNLV(I,I)
          ZK = ZK + HT
          IF(KZ(14).LT.3) RTIDE = (ZMASS/TIDAL(1))**0.3333
      END IF
      IF(KZ(14).EQ.-1) RTIDE = (ZMASS/TIDAL(1))**0.3333
      EI = ZK - BODY(I)*POTI
      IF (KZ(14).GE.3) THEN
          EI = EI - 0.5*TIDAL(4)*BODY(I)*(X(1,I)*XDOT(2,I) -
     &                                    X(2,I)*XDOT(1,I))
      END IF
*
...
*       Update total mass and save energy & number of single escapers.
      ...
      END IF
*
*       Include optional escape output on unit 11 (#23 > 1).
      IF (KZ(23).GT.1) THEN
          TESC = (TIME+TOFF)*TSTAR
          EESC = EI/BODY(I)
          VB2 = 2.0*ZKIN/ZMASS
*       Distinguish between tidal field and isolated system (ECRIT at RTIDE).
          ...
          IF(I.LE.N)THEN
             NAMEJ = 0
             KSTARJ = 99
             WRITE (11,46)  TESC, BESC, EESC, VKM, KSTARI, 
     &                      KSTARJ, KSTARJ, NAME(I), NAMEJ
          ELSE
             IP = I - N
             WRITE (11,46)  TESC, BESC, EESC, VKM, KSTARI, 
     &                      KSTAR(2*IP-1), KSTAR(2*IP), 
     &                      NAME(2*IP-1), NAME(2*IP) 
          ENDIF
   46     FORMAT (F8.1,F8.3,F6.1,F7.1,3I4,2I6)
      END IF
*
...
*       Update COMMON arrays to remove the escaper and correct F & FDOT.
      CALL REMOVE(I,1)
*
*       Delete escaper from perturber lists and reduce higher locations.
   60 DO 150 LL = 1,NPAIRS
          J = 2*LL - 1
          NNB = LIST(1,J)
...
*       Move up the remaining list members and reduce membership by one.
   ...
   80     CONTINUE
          LIST(1,J) = LIST(1,J) - 1
*       Reduce the steps to minimize error effect (skip block-step version).
...
*
*       Add an arbitrary body as perturber if list only contains escaper.
          LIST(1,J) = 1
          LIST(2,J) = MIN(JMIN,N)
          GO TO 150
*
...
*       Distinguish between actual and ghost binary (mergers come later).
      IF (BODY(2*IPAIR-1).GT.0.0D0) THEN
          NAME2 = NAME(2*IPAIR)
*
*       Include rare case of quintuplet or sextuplet.
...
*
      IF (NAMEI.GT.0) THEN
          NBESC = NBESC + 1
          IF (KSTARI.EQ.-2) NPOP(10) = NPOP(10) + 1
      ELSE
          NMESC = NMESC + 1
      END IF
*
...
*       Search for c.m. name one level below current (NAMEI < 0).
  ...
  220         CONTINUE
*       Use previous merger index to look for ghost binary at earlier level.
              I30 = I3HI
              JM0 = JM
