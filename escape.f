...
*       Check escape criterion for tidal case or isolated system.
          EI = 0.5*VI2 - POTI
          IF (TIDAL(1).GT.0.0.OR.EI.GT.0.0) GO TO 30
      END IF
*
      ELSE
...
*       See whether nearest body also satisfies the escape condition.
      IF (A2.GT.RESC2.AND.TIDAL(1).GT.0.0) GO TO 40
*
      IF(KZ(23).GE.3)THEN
...
*       Form optional output diagnostics.
   40 X1 = X(1,I) - RDENS(1)
      ...
      ILIST(2*NCORR+1) = ATAN(A3)*180.0/3.14159
      ILIST(2*NCORR+2) = ATAN(A4)*180.0/3.14159
...
*       Update tidal radius in case of tidal cutoff (R.Sp.)
      ...
      END IF
*
*       Save energy & number of single escapers.
      IF (I.LE.N) THEN
          E(4) = E(4) + EI
          NPOP(4) = NPOP(4) + 1
          NSESC = NSESC + 1
...
*       Distinguish between tidal field and isolated system (ECRIT at RTIDE).
          ...
          ELSE
              ...
              BESC = BODY(I)/BODYM
          END IF
*         OPEN (UNIT=11,STATUS='UNKNOWN',FORM='UNFORMATTED',FILE='ESC')
          WRITE (11,45)  TESC, BESC, EESC
   45     FORMAT (F8.1,2F6.1)
*         CLOSE(UNIT=11)
      END IF
*
...
*       Remove escaper and reduce the membership.
  ...
  185         CONTINUE
          ...
          END IF
*       Reduce higher particle locations by one.
          IF (LISTV(L).GT.I) LISTV(L) = LISTV(L) - 1
  190 CONTINUE
*
...
*       Skip correction if ghost is also merged binary (NAMEI = 0 below).
      IF (NAMEI.NE.0) THEN
          EB = H(IPAIR)*BODY(2*IPAIR-1)*BODY(2*IPAIR)/
     &                                 (BODY(2*IPAIR-1) + BODY(2*IPAIR))
      ELSE
          EB = 0.0
      END IF
*
*       Set scaled velocity and update the total energy.
      VI = SQRT(2.0*VI2*RSCALE/ZMASS)
      BE(3) = BE(3) - EB
*
*       Specify binary type (0: standard; -1: merged binary).
      ...
      IF (NAMEI.LE.0) M = -1
*
      WRITE (6,200)  I, IPAIR, NAME(2*IPAIR-1), NAME(2*IPAIR),
     &               LIST(2,2*IPAIR), M, BODY(2*IPAIR-1), BODY(2*IPAIR),
     &               EB, VI, EI
  200 FORMAT (/,' ESCAPE OF BINARY   I =',I5,'  IPAIR =',I3,
     &         '  NAME =',2I5,I4,I3,'  M =',2F8.4,'  EB =',F8.4,
     &         '  V/<V> =',F5.2,'  EI =',F8.5)
*
