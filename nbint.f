...
*       Adopt c.m. approximation for small total perturbation.
      IF (GAMMA(IPAIR).GE.GMIN) THEN !!!!!!!!Test Kupi Gabor ".GE.GMIN)" was
...
*
*
*     Call eccydo for Black Hole Eccentricity Calculation
      if(rank.eq.0)then
!        CALL ECCYDO
      endif
*
...
*       See whether c.m. approximation applies (ignore unperturbed case).
          J = K - N
          KDUM = 2*J - 1
          IF (RIJ2.GT.(rs2fac*CMSEP2*R(J)**2).OR.LIST(1,KDUM).EQ.0)
     &         GO TO 48
*
          K = KDUM
