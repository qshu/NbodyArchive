...
*
*       Obtain irregular forcet & first derivative.
      DO 5 K = 1,3
          XI(K) = X(K,I)
          XIDOT(K) = XDOT(K,I)
          FIRR(K) = 0.0D0
...
*       Set KS pair index.
      IPAIR = I - N
*
*          Perturbation on c.m. /Omarov Ch. 2006
*     CALL DRAGSTARPCM(I,FIRR,FD)      
*
...
*       Check option for external tidal field.
   ...
   75     CONTINUE
      ...
      END IF
*
*       Interaction with Accretion Disk & Massive Central Object
      CALL MASSCENTROBJECT(I,XI,XIDOT,FIRR,FD) 
*          FREG(K)=FR(K) 
      CALL DRAGFORCE(I,XI,XIDOT,FIRR,FD,FR,0)
...
*
      STEP(I) = TTMP
*
*     For Accretion Disk 2007 (note that here STEP(I) is already the new *
*                   quantized step)
      XZNEW(I)=XN(3,I)+STEP(I)*XNDOT(3,I)  
      IF(DABS(XN(3,I)).GT.HZ*2.2) THEN
      DO WHILE(XZNEW(I)*XN(3,I).LT.0.0)
      STEP(I)=0.5*STEP(I)
      XZNEW(I)=XN(3,I)+STEP(I)*XNDOT(3,I)
      END DO
      R_XY=(XI(1)**2.0+XI(2)**2.0)**0.5
      IF(DABS(XZNEW(I)).LT.HZ*1.8) THEN
      STEP(I) = 0.5 * (0.5 * (0.5 *STEP(I)))
      TTMP=STEP(I)
      END IF
      END IF
*
