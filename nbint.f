...
*       See whether c.m. approximation applies (ignore unperturbed case).
          J = K - N
          IF (RIJ2.GT.CMSEP2*R(J)**2) GO TO 48
*
          KDUM = 2*J - 1
          K = KDUM
...
*       Specify new time-step (standard criterion or fast expression).
        ...
        ELSE
          TTMP = STEPI(FIRR,FD,D2(1,I),D3(1,I),ETAI)
        END IF
      DT0 = TTMP
*
...
*       Select discrete value (increased by 2, decreased by 2 or unchanged).
      IF (TTMP.GT.2.0*TOLD) THEN
          IF (DMOD(TIME,2.0*TOLD).EQ.0.0D0) THEN
              TTMP = MIN(2.0*TOLD,1.D0)
          ELSE
              TTMP = TOLD
          END IF
      ELSE IF (TTMP.LT.TOLD) THEN
          TTMP = 0.5*TOLD
            IF (TTMP.GT.DT0) THEN
                TTMP = 0.5*TTMP
          END IF
